...
def _headline_promo_externo(p):
    """Headlines mais naturais e premium, com foco em curadoria humana."""
    ...
    # Tênis / calçados
    if any(w in nome for w in ["tenis", "sneaker", "calcado", "running", "corrida", "casual", "fresh foam", "court", "run", "olympikus"]):
        if "olympikus" in nome:
            return pick(["Ótima oferta nesse Olympikus 👟", "Olympikus confortável para o dia a dia 👟", "Esse Olympikus vale conferir 👟", "Olympikus com preço interessante 👟"])
        if "new balance" in nome or " nb " in f" {nome} ":
            return pick(["New Balance muito bonito 👟", "Ótima oferta nesse New Balance 👟", "Conforto e estilo nesse New Balance 👟", "Esse New Balance vale conferir 👟"])
        if "nike" in nome:
            return pick(["Ótima oferta nesse Nike 👟", "Nike clean para o dia a dia 👟", "Esse Nike ficou muito bonito 👟", "Nike com preço interessante 👟"])
        if "adidas" in nome:
            return pick(["Ótima oferta nesse Adidas 👟", "Adidas sem erro para o dia a dia 👟", "Esse Adidas combina com tudo 👟", "Adidas com preço interessante 👟"])
        if "puma" in nome:
            return pick(["Ótima oferta nesse Puma 👟", "Puma bonito e confortável 👟", "Esse Puma vale conferir 👟"])
        if "mizuno" in nome:
            return pick(["Mizuno confortável para o dia a dia 👟", "Ótima oferta nesse Mizuno 👟", "Mizuno com preço interessante 👟"])
        if "fila" in nome:
            return pick(["Ótima oferta nesse Fila 👟", "Fila bonito para usar muito 👟", "Esse Fila vale conferir 👟"])
    ...
    # Moda
    if any(w in nome for w in ["camiseta", "camisa", "regata", "polo"]):
        if "nike" in nome:
        ...
        return pick(["Peça básica para o dia a dia 👕", "Camiseta boa para usar muito 👕", "Ótima oferta nessa camiseta 👕"])

    if any(w in nome for w in ["short", "bermuda"]):
        return pick(["Bermuda confortável para o dia a dia 👊", "Short versátil com preço bom 👊", "Ótima oferta nessa bermuda 👊"])

    if any(w in nome for w in ["cueca", "boxer"]):
        return pick(["Kit básico que vale ter 👊", "Boa oferta nesse kit de cuecas 👊", "Para renovar a gaveta sem gastar muito 👊"])

    # Perfume / beleza
    if any(w in nome for w in ["perfume", "lattafa", "asad", "armani", "colonia", "la vie", "oud", "egeo", "boticario"]):
        return pick(["Perfume com preço interessante ✨", "Achado perfumado do dia ✨", "Boa oferta para quem gosta de perfume ✨"])

    # Fitness / suplementos
    if any(w in nome for w in ["creatina", "whey", "suplemento", "protein"]):
        return pick(["Boa oferta para quem treina 👊", "Suplemento com preço interessante 👊", "Achado fitness do dia 👊"])

    # Tech
    if any(w in nome for w in ["fone", "jbl", "smartwatch", "relogio", "xiaomi", "caixa de som", "bluetooth"]):
        return pick(["Achado tech com preço bom ⚡", "Boa oferta em tecnologia ⚡", "Gadget útil para o dia a dia ⚡"])

    # Mochila / bolsa
    if any(w in nome for w in ["mochila", "bolsa"]):
        if "nike" in nome:
            return pick(["Mochila Nike para o dia a dia 🎒", "Boa oferta nessa mochila Nike 🎒", "Nike bonita e útil para usar muito 🎒"])
        return pick(["Mochila prática para o dia a dia 🎒", "Boa oferta nessa mochila 🎒", "Achado útil para carregar tudo 🎒"])

    # Casa / escritório
    if any(w in nome for w in ["cadeira", "escritorio", "home office", "gamer"]):
        return pick(["Boa oferta para o home office 👊", "Cadeira confortável com preço interessante 👊", "Achado para trabalhar melhor 👊"])

    if any(w in nome for w in ["chinelo", "havaianas", "slide"]):
        return pick(["Chinelo confortável para o dia a dia 👊", "Boa oferta nesse chinelo 👊", "Conforto simples que todo mundo usa 👊"])

    return pick(["Achado interessante do dia 🔥", "Boa oferta para aproveitar hoje 👊", "Preço interessante para conferir 🔥", "Garimpo Brizzah para economizar 👊"])
