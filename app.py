# -*- coding: utf-8 -*-
# BRIZZAH V13 - Automático somente banco + aviso de preço variável
...
def _brz_aplicar_modo_30min():
    try:
        ...
        # V21: respeita a configuração "Produtos por envio" do painel.
        pass
    except Exception as e:
...
def _headline_promo_externo(p):
    """Manchetes curtas estilo grupo de promoções, com lógica premium por marca/categoria."""
    import random as _random
    nome = _brz_norm(p.get("name") or "")
    source = (p.get("source") or "").lower()

    # 👟 TÊNIS / CALÇADOS PREMIUM — melhora muito a legenda de Netshoes/ML
    if any(w in nome for w in ["tenis", "sneaker", "calcado", "running", "corrida", "casual", "fresh foam", "court", "run"]):
        if "new balance" in nome or " nb " in f" {nome} ":
            opts = [
                "NEW BALANCE TÁ EM OUTRO NÍVEL 👟",
                "ESSE NEW BALANCE FICOU ABSURDO 🔥",
                "NB PRA USAR MUITO NO DIA A DIA 👟",
                "CONFORTO E ESTILO NESSE NEW BALANCE 🔥",
                "NEW BALANCE LIMPO DEMAIS 👟",
            ]
        elif "nike" in nome:
            opts = [
                "NIKE CLEAN DEMAIS 🔥",
                "NIKE PRA USAR COM TUDO 👟",
                "ESSE NIKE FICOU PESADO 🔥",
                "NIKE COM PREÇO DE OPORTUNIDADE 👟",
                "A NIKE SABE FAZER TÊNIS BONITO 🔥",
            ]
        elif "adidas" in nome:
            opts = [
                "ADIDAS SEM ERRO 👟",
                "ADIDAS PRA USAR TODO DIA 🔥",
                "ESSE ADIDAS COMBINA COM TUDO 👟",
                "ADIDAS COM PREÇO BOM DEMAIS 🔥",
                "TÊNIS ADIDAS PRA NÃO ERRAR NO LOOK 👟",
            ]
        elif "puma" in nome:
            opts = [
                "PUMA BONITO E CONFORTÁVEL 👟",
                "ESSE PUMA TÁ COM PREÇO FORTE 🔥",
                "PUMA PRA BATER PERNA O DIA TODO 👟",
                "PUMA CASUAL SEM ERRO 🔥",
            ]
        elif "mizuno" in nome:
            opts = [
                "MIZUNO PRA QUEM CURTE CONFORTO 👟",
                "MIZUNO COM PREÇO QUE VALE OLHAR 🔥",
                "PRA CAMINHAR E TREINAR COM CONFORTO 👟",
                "MIZUNO FORTE PRO DIA A DIA 🔥",
            ]
        elif "fila" in nome:
            opts = [
                "FILA BONITO PRA USAR MUITO 👟",
                "ESSE FILA TÁ COM PREÇO BOM 🔥",
                "FILA CASUAL SEM ERRO 👟",
            ]
        elif any(w in nome for w in ["corrida", "running", "run", "fresh foam", "treino"]):
            opts = [
                "CONFORTO PRA CAMINHAR O DIA TODO 👟",
                "PRA TREINO E PRO DIA A DIA 🔥",
                "LEVEZA QUE SURPREENDE 👟",
                "TÊNIS PRA QUEM CURTE CONFORTO 🔥",
            ]
        else:
            opts = [
                "TÊNIS PRA USAR TODO DIA 👟",
                "CONFORTO QUE VICIA 🔥",
                "TÊNIS QUE COMBINA COM TUDO 👟",
                "PREÇO BOM NESSE TÊNIS 🔥",
                "ACHADO FORTE PRA QUEM CURTE CALÇADO BOM 👟",
            ]

    elif any(w in nome for w in ["camiseta", "camisa", "regata", "polo"]):
        if "nike" in nome:
            opts = ["NIKE BÁSICA QUE NÃO TEM ERRO 🔥", "CAMISETA NIKE PRA USAR MUITO 👕", "NIKE CLEAN PRO DIA A DIA 🔥"]
        elif "adidas" in nome:
            opts = ["ADIDAS BÁSICA SEM ERRO 👕", "CAMISETA ADIDAS PRA USAR MUITO 🔥", "ADIDAS CLEAN PRO DIA A DIA 👕"]
        else:
            opts = ["CAMISETA BOA PRA USAR MUITO 👕", "BÁSICA QUE COMBINA COM TUDO 🔥", "PEÇA CURINGA PRO DIA A DIA 👕"]

    elif any(w in nome for w in ["short", "bermuda"]):
        opts = [
            "SHORT VERSÁTIL PRO DIA A DIA 🔥",
            "BERMUDA PRA TREINO E PASSEIO 👊",
            "CONFORTO PRA USAR MUITO 🔥",
            "PREÇO BOM NESSA BERMUDA 👊",
        ]

    elif any(w in nome for w in ["cueca", "boxer"]):
        opts = [
            "CUECA NUNCA É DEMAIS 👊",
            "KIT BÁSICO QUE TODO MUNDO USA 🔥",
            "PRA RENOVAR A GAVETA SEM GASTAR MUITO 👊",
        ]

    elif any(w in nome for w in ["perfume", "lattafa", "asad", "armani", "colonia", "la vie", "oud", "egeo", "boticario"]):
        opts = [
    ...
    elif any(w in nome for w in ["fone", "jbl", "smartwatch", "relogio", "xiaomi", "caixa de som", "bluetooth"]):
        opts = [
        ...
        if "nike" in nome:
            opts = ["A NIKE SABE FAZER MOCHILA 🔥", "MOCHILA NIKE PRA USAR TODO DIA 🎒", "NIKE BONITA E ÚTIL PRO DIA A DIA 🔥"]
        else:
            opts = ["MOCHILA PRA USAR TODO DIA 🎒", "CABE MUITA COISA E O PREÇO AJUDA 🔥", "ACHADO PRA QUEM CARREGA O MUNDO NAS COSTAS 🎒"]

    elif any(w in nome for w in ["cadeira", "escritorio", "home office", "gamer"]):
        opts = [
    ...
    elif any(w in nome for w in ["chinelo", "havaianas", "slide"]):
        opts = [
    ...
    # Leve tempero por origem, sem poluir quando já escolheu uma headline boa.
    if source == "netshoes" and _random.random() < 0.15:
        opts.append("NETSHOES COM PREÇO DE GARIMPO 👊")
    if source == "mercadolivre" and _random.random() < 0.15:
...
def _caption_wa_externo(p):
    ...
    linhas += ["", f"{emoji} Origem: *{fonte}*", f"🔗 {link}", "", "⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.", "_Brizzah | Achados inteligentes_ 🔥"]
    return "\n".join(linhas)
...
def _caption_wa_externo(p):
    ...
    linhas += ["", f"Loja oficial {fonte}:", f"🔗 {link}", "", "⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.", "_Brizzah | Achados inteligentes_ 🔥"]
    return "\n".join(linhas)
...
def _gerar_caption_wa(produto):
    ...
    p += ["", f"🔗 {link}", "", "⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.", "_Brizzah | Achados inteligentes_ 🔥"]
    return "\n".join(p)
...
def _caption_wa_externo(p):
    ...
    if cupom: linhas.append(f"Cupom: *{cupom}* ⚠️")
    linhas += ["", f"{emoji} Origem: *{fonte}*", f"🔗 {link}", "", _brz_cta_grupo(), "", "⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.", "_Brizzah | Achados inteligentes_ 🔥"]
    return "\n".join(linhas)
...
def _caption_wa_externo(p):
    ...
    linhas += [
        ...
        "",
        "⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.",
...
def _gerar_caption_wa(produto):
    ...
    p.append("")
    p.append("⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.")
...
def executar_ciclo():
    # V21: não força mais products_per_cycle=1; respeita o painel.
...
# ============================================================
# BRIZZAH V12 — AUTOMÁTICO SOMENTE BANCO DE LINKS
# Shopee automática desligada no agendamento.
# Shopee continua disponível no botão manual "Buscar e postar agora".
# Aplicado por atualização ChatGPT em 2026-05-06
# ============================================================
try:
    import threading as _brz_v12_threading
    _BRZ_V12_CTX = _brz_v12_threading.local()
except Exception:
    _BRZ_V12_CTX = None


def _brz_v12_is_auto_banco_only():
    try:
        return bool(getattr(_BRZ_V12_CTX, "auto_banco_only", False))
    except Exception:
        return False


def _brz_v12_set_auto_banco_only(valor=True):
    try:
        setattr(_BRZ_V12_CTX, "auto_banco_only", bool(valor))
    except Exception:
        pass


# A rotação do AGENDAMENTO não inclui Shopee automática.
def _brz_v10_rotacao_seq():
    return ["netshoes", "mercadolivre", "netshoes", "mercadolivre", "amazon", "nike", "externo"]


def _brz_proxima_ordem_fontes():
    return ["netshoes", "mercadolivre", "amazon", "nike", "externo"]


# Guarda buscadores originais para uso manual.
try:
    _brz_v12_buscar_multiplas_plataformas_original = buscar_multiplas_plataformas
except Exception:
    _brz_v12_buscar_multiplas_plataformas_original = None
try:
    _brz_v12_buscar_pool_produtos_top_original = buscar_pool_produtos_top
except Exception:
    _brz_v12_buscar_pool_produtos_top_original = None
try:
    _brz_v12_shopee_search_original = shopee_search
except Exception:
    _brz_v12_shopee_search_original = None


def buscar_multiplas_plataformas(keyword, limit_cada=6):
    """V12: no agendamento, bloqueia busca automática externa/Shopee; no manual, mantém normal."""
    if _brz_v12_is_auto_banco_only():
        try: log("INFO", "[V12] Agendamento banco-only: busca automática de plataformas bloqueada")
        except Exception: pass
        return []
    if _brz_v12_buscar_multiplas_plataformas_original:
        return _brz_v12_buscar_multiplas_plataformas_original(keyword, limit_cada=limit_cada)
    return []


def buscar_pool_produtos_top(nicho_base="geral"):
    """V12: no agendamento, não usa pool Shopee; no manual, mantém normal."""
    if _brz_v12_is_auto_banco_only():
        try: log("INFO", "[V12] Agendamento banco-only: pool Shopee bloqueado")
        except Exception: pass
        return []
    if _brz_v12_buscar_pool_produtos_top_original:
        return _brz_v12_buscar_pool_produtos_top_original(nicho_base=nicho_base)
    return []


def shopee_search(keyword, limit=12):
    """V12: no agendamento, Shopee automática desligada; no botão manual, mantém Shopee."""
    if _brz_v12_is_auto_banco_only():
        try: log("INFO", "[V12] Agendamento banco-only: fallback Shopee bloqueado")
        except Exception: pass
        return []
    if _brz_v12_shopee_search_original:
        return _brz_v12_shopee_search_original(keyword, limit=limit)
    return []


# No banco do agendamento, prioriza links manuais e evita Shopee como fonte automática.
try:
    _brz_v12_buscar_external_products_aprovados_original = buscar_external_products_aprovados
except Exception:
    _brz_v12_buscar_external_products_aprovados_original = None


def buscar_external_products_aprovados(limit=30):
    produtos = []
    if _brz_v12_buscar_external_products_aprovados_original:
        produtos = _brz_v12_buscar_external_products_aprovados_original(limit=max(int(limit or 30), 80))
    if _brz_v12_is_auto_banco_only():
        # Automático: apenas banco premium não-Shopee. Shopee fica para o botão manual.
        filtrados = []
        for p in produtos or []:
            try:
                fonte = _brz_v10_source(p.get("source"))
                ...
                filtrados.append(p)
        filtrados.sort(key=_brz_v10_score, reverse=True)
        try: log("INFO", f"[V12] Banco-only automático | candidatos={len(produtos or [])} | sem_shopee={len(filtrados)}")
        except Exception: pass
        return filtrados[:max(1, int(limit or 1))]
    return produtos[:max(1, int(limit or 30))]


# Reescreve o verificador do agendamento para marcar contexto automático banco-only.
def verificar_agendamento():
    """V12: agendamento posta somente links do banco. Shopee automática não roda aqui."""
    if cfg("auto_enabled", "false") != "true":
        return
    ...
    print(f"[AGENDADOR V12] hora_br={agora_hm} | banco-only | ultimo={ultimo}")
    ...
    cfg_set("last_auto_run", chave)
    log("INFO", f"▶ AGENDAMENTO V12 DISPARADO às {agora_hm} — somente banco de links")

    def run_ciclo():
        for tentativa in range(1, 3):
            try:
                _brz_v12_set_auto_banco_only(True)
                resultado = executar_ciclo()
                if resultado and resultado > 0:
                    log("INFO", f"✅ Agendado V12 OK: {resultado} produto(s) do banco | tentativa {tentativa}")
                    return
                log("WARN", f"Agendado V12: nenhum produto do banco apto — tentativa {tentativa}/2")
            except Exception as e:
                log("ERROR", f"Agendado V12 erro tentativa {tentativa}: {str(e)[:100]}")
            finally:
                _brz_v12_set_auto_banco_only(False)
            if tentativa < 2:
                time.sleep(20)
...
try:
    log("INFO", "[V12] Ativo: agendamento somente banco de links; Shopee apenas no botão manual Buscar e postar")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V18 — RODÍZIO REAL DE PERFIL + ANTI REPETIÇÃO FORTE
# Aplicado em 2026-05-07
# Objetivo:
# - Não repetir mesmo anúncio, mesmo link, mesma marca/subcategoria e mesmo perfil.
# - Alternar automaticamente: moda feminina, moda masculina, tênis, perfume,
#   fitness, tech, casa e acessórios.
# - Manter automático somente banco de links.
# - Manter Shopee somente manual.
# - Substituir headlines fortes por legendas mais naturais.
# ============================================================

import urllib.parse as _brz_v18_urlparse
import random as _brz_v18_random

BRIZZAH_V18_DUP_LINK_DIAS = int(os.environ.get("BRIZZAH_DUP_LINK_DIAS", "30") or "30")
BRIZZAH_V18_DUP_NOME_DIAS = int(os.environ.get("BRIZZAH_DUP_NOME_DIAS", "14") or "14")
BRIZZAH_V18_BLOQUEIO_MARCA_CATEGORIA_HORAS = int(os.environ.get("BRIZZAH_BLOQUEIO_MARCA_CAT_HORAS", "8") or "8")
BRIZZAH_V18_RECENT_LIMIT = int(os.environ.get("BRIZZAH_RECENT_LIMIT", "12") or "12")


def _brz_v18_norm(txt):
    try:
        return _brz_v10_norm(txt)
    except Exception:
        try:
            return _brz_norm(txt)
        except Exception:
...
def _brz_v18_link_base(url):
    """Normaliza link para comparar duplicados, inclusive linksynergy/deeplink."""
    try:
        url = (url or "").strip()
        if not url:
            return ""
        p = _brz_v18_urlparse.urlparse(url)
        qs = _brz_v18_urlparse.parse_qs(p.query)
        # Linksynergy usa murl com o link real do produto.
        for key in ("murl", "url", "u", "redirect", "target"):
            val = qs.get(key, [""])[0]
            if val and val.startswith("http"):
                return _brz_v18_link_base(_brz_v18_urlparse.unquote(val))
        host = (p.netloc or "").lower().replace("www.", "")
        path = _brz_v18_urlparse.unquote(p.path or "").lower().strip("/")
        # remove variações muito específicas de query; o produto é host+path.
        return f"{host}/{path}".strip("/")
    except Exception:
        return _brz_v18_norm(url)


def _brz_v18_nome_chave(nome):
    """Cria assinatura do produto sem tamanho/cor simples para evitar duplicados parecidos."""
    import re as _re
    n = _brz_v18_norm(nome)
    # Remove tamanhos/numerações comuns
    n = _re.sub(r"\b(3[0-9]|4[0-9]|pp|p|m|g|gg|xg|xxg|unico|unissex)\b", " ", n)
    # Remove palavras pouco úteis
    stop = {
        "masculino","feminino","adulto","infantil","original","oficial","preto","branco",
        "azul","verde","vermelho","rosa","bege","cinza","marrom","casual","leve",
        "confortavel","dia","academia","treino","corrida","promoção","promocao","oferta"
    }
    parts = [x for x in n.split() if x not in stop and len(x) > 2]
    return " ".join(parts[:8]).strip()


def _brz_v18_categoria(p):
    n = _brz_v18_norm(" ".join([
        str((p or {}).get("name") or ""),
        str((p or {}).get("category") or ""),
        str((p or {}).get("brand") or ""),
        str((p or {}).get("notes") or "")
    ]))
    if any(x in n for x in ["perfume", "colonia", "deo parfum", "eau de parfum", "lattafa", "egeo", "granado", "boticario", "fragrancia", "parfum"]):
        return "perfume"
    if any(x in n for x in ["creatina", "whey", "suplemento", "protein", "coqueteleira", "dryfit", "academia", "treino", "legging", "fitness"]):
        return "fitness"
    if any(x in n for x in ["fone", "bluetooth", "jbl", "xiaomi", "g shock", "casio", "relogio", "smartwatch", "boombox", "caixa de som", "power bank", "carregador"]):
        return "tech"
    if any(x in n for x in ["tenis", "sneaker", "calcado", "sapato", "sandalia", "chinelo", "bota", "running", "fresh foam"]):
        if any(x in n for x in ["feminino", "feminina", "mulher", "menina"]):
            return "tenis_feminino"
        if any(x in n for x in ["masculino", "masculina", "homem", "menino"]):
            return "tenis_masculino"
        return "tenis"
    if any(x in n for x in ["camiseta", "camisa", "polo", "regata", "bermuda", "short", "cueca", "boxer", "calca", "jaqueta", "moletom", "meia"]):
        if any(x in n for x in ["feminino", "feminina", "mulher", "menina", "delas"]):
            return "moda_feminina"
        if any(x in n for x in ["masculino", "masculina", "homem", "cueca", "boxer"]):
            return "moda_masculina"
        return "moda"
    if any(x in n for x in ["mochila", "bolsa", "colar", "corrente", "pulseira", "brinco", "anel", "oculos", "cinto"]):
        return "acessorios"
    if any(x in n for x in ["casa", "cozinha", "airfryer", "panela", "toalha", "espelho", "organizador", "decoracao", "tapete", "mesa"]):
        return "casa"
    if any(x in n for x in ["infantil", "crianca", "bebe", "kids"]):
        return "infantil"
    if any(x in f" {n} " for x in [" pet ", " cachorro ", " gato ", " racao ", " coleira ", " caminha pet "]):
        return "pet"
    return "geral"


def _brz_v18_marca(p):
    n = _brz_v18_norm(" ".join([str((p or {}).get("name") or ""), str((p or {}).get("brand") or "")]))
    marcas = [
        "new balance", "olympikus", "adidas", "nike", "puma", "mizuno", "fila", "reebok",
        "asics", "casio", "g shock", "jbl", "xiaomi", "philips", "lattafa", "granado",
        "boticario", "oakley", "under armour", "universal nutrition"
    ]
    for m in marcas:
        if m in n:
            return m
...
def _brz_v18_ensure_tables():
    try:
        with get_db() as c:
            c.execute("""
                CREATE TABLE IF NOT EXISTS brizzah_post_history (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    external_product_id INTEGER,
                    link_key TEXT DEFAULT '',
            ...
            """)
            c.execute("CREATE INDEX IF NOT EXISTS idx_brizzah_post_history_created ON brizzah_post_history(created_at)")
            c.execute("CREATE INDEX IF NOT EXISTS idx_brizzah_post_history_link ON brizzah_post_history(link_key)")
            c.execute("CREATE INDEX IF NOT EXISTS idx_brizzah_post_history_name ON brizzah_post_history(name_key)")
            c.execute("DELETE FROM brizzah_post_history WHERE created_at < datetime('now','-45 days')")
    except Exception as e:
        try: log("WARN", f"[V18] Falha ensure history: {str(e)[:80]}")
        except Exception: pass


def _brz_v18_registrar_post(p):
    """Registra histórico por link/nome/categoria/marca para evitar repetição real."""
    try:
        _brz_v18_ensure_tables()
        link = p.get("affiliate_url") or p.get("product_url") or p.get("link") or ""
        link_key = _brz_v18_link_base(link)
        name_key = _brz_v18_nome_chave(p.get("name") or "")
        cat = _brz_v18_categoria(p)
        brand = _brz_v18_marca(p)
        source = _brz_v10_source(p.get("source")) if "_brz_v10_source" in globals() else str(p.get("source") or "")
        with get_db() as c:
            c.execute("""
                INSERT INTO brizzah_post_history
                (external_product_id, link_key, name_key, category_key, brand_key, source, product_name, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now','localtime'))
            """, (p.get("id") or 0, link_key, name_key, cat, brand, source, (p.get("name") or "")[:180]))
            c.execute("INSERT OR REPLACE INTO config(key,value) VALUES('_brz_v18_last_category',?)", (cat,))
            c.execute("INSERT OR REPLACE INTO config(key,value) VALUES('_brz_v18_last_brand',?)", (brand,))
            c.execute("INSERT OR REPLACE INTO config(key,value) VALUES('_brz_v18_last_name_key',?)", (name_key,))
    except Exception as e:
        try: log("WARN", f"[V18] Falha registrar histórico: {str(e)[:80]}")
        except Exception: pass


def _brz_v18_recente():
    """Lê últimos posts do histórico V18 e da tabela products para compatibilidade."""
    _brz_v18_ensure_tables()
    out = []
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT link_key, name_key, category_key, brand_key, source, product_name, created_at
                FROM brizzah_post_history
                ORDER BY id DESC LIMIT ?
            """, (BRIZZAH_V18_RECENT_LIMIT,)).fetchall()
            out.extend([dict(r) for r in rows])
            rows2 = c.execute("""
                SELECT name, affiliate_url, product_url, shop_name, posted_at
                FROM products
            ...
            for r in rows2:
                d = dict(r)
                fake = {"name": d.get("name"), "affiliate_url": d.get("affiliate_url"), "product_url": d.get("product_url"), "source": d.get("shop_name")}
                out.append({
                    "link_key": _brz_v18_link_base(d.get("affiliate_url") or d.get("product_url") or ""),
                    "name_key": _brz_v18_nome_chave(d.get("name") or ""),
                    "category_key": _brz_v18_categoria(fake),
                    "brand_key": _brz_v18_marca(fake),
                    "source": d.get("shop_name") or "",
                    "product_name": d.get("name") or "",
                    "created_at": d.get("posted_at") or "",
                })
    except Exception:
        pass
    return out
...
def _brz_v18_foi_repetido(p):
    """Bloqueia o mesmo produto e produtos muito parecidos."""
    link_key = _brz_v18_link_base(p.get("affiliate_url") or p.get("product_url") or p.get("link") or "")
    name_key = _brz_v18_nome_chave(p.get("name") or "")
    cat = _brz_v18_categoria(p)
    brand = _brz_v18_marca(p)
    recentes = _brz_v18_recente()

    # Bloqueio duro: mesmo link/nome.
    for r in recentes:
        if link_key and r.get("link_key") and link_key == r.get("link_key"):
            return True, "mesmo link recente"
        if name_key and r.get("name_key") and name_key == r.get("name_key"):
            return True, "mesmo nome recente"

    # Bloqueio de feed: não repetir perfil/marca nos últimos posts.
    ultimos = recentes[:5]
    if cat != "geral":
        if any(r.get("category_key") == cat for r in ultimos[:2]):
            return True, f"categoria repetida: {cat}"
    if brand:
        if any(r.get("brand_key") == brand for r in ultimos[:4]):
            return True, f"marca repetida: {brand}"
        if any(r.get("brand_key") == brand and r.get("category_key") == cat for r in ultimos[:8]):
            return True, f"marca+categoria repetida: {brand}/{cat}"
    return False, "ok"


def _brz_v18_ordem_categorias():
    return [
        "moda_feminina",
        "moda_masculina",
        "tenis_feminino",
...
def _brz_v18_categoria_da_vez():
    ordem = _brz_v18_ordem_categorias()
    try:
        idx = int(cfg("_brz_v18_categoria_idx", "0") or 0)
    except Exception:
        idx = 0
    recentes = _brz_v18_recente()[:3]
    for offset in range(len(ordem)):
        cat = ordem[(idx + offset) % len(ordem)]
        if not any(r.get("category_key") == cat for r in recentes):
            return cat
    return ordem[idx % len(ordem)]


def _brz_v18_avancar_categoria():
    try:
        idx = int(cfg("_brz_v18_categoria_idx", "0") or 0)
        cfg_set("_brz_v18_categoria_idx", str(idx + 1))
    except Exception:
        pass


def _brz_v18_score_feed(p, categoria_alvo=None):
    """Score com diversidade. Não deixa Nike/camiseta dominar só por marca."""
    base = 0
    try:
        base = int(_brz_v10_score(p))
    except Exception:
        try: base = int(score_externo(p))
        except Exception: base = 0

    cat = _brz_v18_categoria(p)
    brand = _brz_v18_marca(p)
    fonte = _brz_v10_source(p.get("source")) if "_brz_v10_source" in globals() else str(p.get("source") or "")
    preco = _safe_float(p.get("price"), 0)

    if categoria_alvo and cat == categoria_alvo:
        base += 120
    elif categoria_alvo and cat != categoria_alvo:
        base -= 10

    # Prioridade de fontes premium, mas sem deixar uma marca dominar.
    if fonte == "netshoes": base += 35
    elif fonte == "mercadolivre": base += 30
    elif fonte == "amazon": base += 18
    elif fonte == "shopee": base -= 100

    if preco and 29 <= preco <= 399:
        base += 12

    # Pequena penalidade para categorias que já aparecem muito no banco.
    recentes = _brz_v18_recente()
    if any(r.get("category_key") == cat for r in recentes[:3]):
        base -= 80
    if brand and any(r.get("brand_key") == brand for r in recentes[:5]):
        base -= 100

    return base + _brz_v18_random.randint(0, 9)


# Guarda a versão anterior para fallback.
try:
    _brz_v18_buscar_external_products_aprovados_anterior = buscar_external_products_aprovados
except Exception:
    _brz_v18_buscar_external_products_aprovados_anterior = None


def buscar_external_products_aprovados(limit=30):
    """
    V18: no automático, escolhe 1 produto por ciclo com rodízio real de categoria/marca.
    Retorna só 1 candidato no agendamento para impedir que o ciclo antigo reordene tudo
    e escolha sempre Nike/camiseta pelo score.
    """
    _brz_v18_ensure_tables()

    produtos = []
    # Busca direto do banco para não depender do score antigo que repetia a mesma marca.
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT * FROM external_products
        ...
        try: log("WARN", f"[V18] Falha busca direta externos: {str(e)[:80]}")
        except Exception: pass
        if _brz_v18_buscar_external_products_aprovados_anterior:
            produtos = _brz_v18_buscar_external_products_aprovados_anterior(limit=max(int(limit or 30), 100)) or []

    validos = []
    bloqueados = {}
    for p in produtos or []:
        try:
            p["source"] = _brz_v10_source(p.get("source")) if "_brz_v10_source" in globals() else str(p.get("source") or "")
            if _brz_v12_is_auto_banco_only() and p["source"] == "shopee":
                continue
            p["old_price"] = _brz_preco_antigo_confiavel(p.get("price"), p.get("old_price"), p.get("source"))
            if "_brz_v10_produto_ruim" in globals() and _brz_v10_produto_ruim(p):
                bloqueados["produto ruim"] = bloqueados.get("produto ruim", 0) + 1
                continue
            if p.get("id") and ja_postado_recentemente_externo(p.get("id"), horas=24*7):
                bloqueados["id recente"] = bloqueados.get("id recente", 0) + 1
                continue
            rep, motivo = _brz_v18_foi_repetido(p)
            if rep:
                bloqueados[motivo] = bloqueados.get(motivo, 0) + 1
                continue
            validos.append(p)
        except Exception:
            continue
    ...
    if not validos:
        # Último fallback: relaxa categoria/marca, mas mantém bloqueio de link/nome/id.
        for p in produtos or []:
            try:
                p["source"] = _brz_v10_source(p.get("source")) if "_brz_v10_source" in globals() else str(p.get("source") or "")
                if _brz_v12_is_auto_banco_only() and p["source"] == "shopee":
                ...
                if p.get("id") and ja_postado_recentemente_externo(p.get("id"), horas=24*7):
                    continue
                link_key = _brz_v18_link_base(p.get("affiliate_url") or p.get("product_url") or "")
                name_key = _brz_v18_nome_chave(p.get("name") or "")
                duros = _brz_v18_recente()
                if any((link_key and link_key == r.get("link_key")) or (name_key and name_key == r.get("name_key")) for r in duros):
                    continue
                validos.append(p)
            except Exception:
                continue
    ...
    if not validos:
        try: log("WARN", f"[V18] Nenhum produto apto no banco após filtros | bloqueios={bloqueados}")
        except Exception: pass
        return []

    alvo = _brz_v18_categoria_da_vez()
    pool_alvo = [p for p in validos if _brz_v18_categoria(p) == alvo]

    if pool_alvo:
        escolhido = max(pool_alvo, key=lambda p: _brz_v18_score_feed(p, alvo))
    else:
        # Escolhe uma categoria que não apareceu recentemente.
        recentes_cats = {r.get("category_key") for r in _brz_v18_recente()[:4]}
        pool_diverso = [p for p in validos if _brz_v18_categoria(p) not in recentes_cats] or validos
        escolhido = max(pool_diverso, key=lambda p: _brz_v18_score_feed(p, None))

    try:
        log("INFO", f"[V18] Escolhido com rodízio real | alvo={alvo} | cat={_brz_v18_categoria(escolhido)} | marca={_brz_v18_marca(escolhido)} | nome={(escolhido.get('name') or '')[:60]} | bloqueios={bloqueados}")
    except Exception:
        pass
...
# Guarda execução anterior para registrar histórico após sucesso.
try:
    _brz_v18_executar_ciclo_anterior = executar_ciclo
except Exception:
    _brz_v18_executar_ciclo_anterior = None


def executar_ciclo():
    """
    V18: executa ciclo antigo, mas registra o último produto externo postado no histórico
    e avança categoria para o próximo ciclo.
    """
    antes = None
    try:
        with get_db() as c:
            antes = c.execute("SELECT id FROM products ORDER BY id DESC LIMIT 1").fetchone()
            antes = int(antes["id"]) if antes else 0
    except Exception:
        antes = 0

    resultado = 0
    if _brz_v18_executar_ciclo_anterior:
        resultado = _brz_v18_executar_ciclo_anterior()
    else:
        resultado = 0
    ...
    try:
        ...
        with get_db() as c:
            rows = c.execute("""
                SELECT id, item_id, name, affiliate_url, product_url, shop_name
                FROM products
                WHERE id > ?
                ORDER BY id ASC
            """, (antes or 0,)).fetchall()
            depois = [dict(r) for r in rows]
            for d in depois:
                ext_id = 0
                try:
                    if str(d.get("item_id") or "").startswith("ext_"):
                        ext_id = int(str(d.get("item_id")).replace("ext_", ""))
                except Exception:
                    ext_id = 0
                p = {
                    "id": ext_id,
                    "name": d.get("name") or "",
                    "affiliate_url": d.get("affiliate_url") or "",
                    "product_url": d.get("product_url") or "",
                    "source": d.get("shop_name") or "",
                }
                _brz_v18_registrar_post(p)
        if resultado and resultado > 0:
            _brz_v18_avancar_categoria()
    except Exception as e:
        try: log("WARN", f"[V18] Falha pós-ciclo histórico: {str(e)[:80]}")
        except Exception: pass

    return resultado


# Headlines finais mais naturais, sem “surrar”, sem tom agressivo.
def _headline_promo_externo(p):
    import random as _random
    nome = _brz_v18_norm(p.get("name") if isinstance(p, dict) else str(p))
    cat = _brz_v18_categoria(p if isinstance(p, dict) else {"name": nome})
    marca = _brz_v18_marca(p if isinstance(p, dict) else {"name": nome})

    def pick(opts):
        return _random.choice(opts)

    if cat in ("tenis", "tenis_feminino", "tenis_masculino"):
        if marca == "olympikus":
            return pick(["Ótima oferta nesse Olympikus 👟", "Olympikus confortável para o dia a dia 👟", "Esse Olympikus vale conferir 👟"])
        if marca == "new balance":
            return pick(["Ótima oferta nesse New Balance 👟", "New Balance muito bonito 👟", "Conforto e estilo nesse New Balance 👟"])
        if marca == "nike":
            return pick(["Ótima oferta nesse Nike 👟", "Nike clean para o dia a dia 👟", "Esse Nike ficou muito bonito 👟"])
        if marca == "adidas":
            return pick(["Ótima oferta nesse Adidas 👟", "Adidas sem erro para o dia a dia 👟", "Esse Adidas combina com tudo 👟"])
        if marca == "puma":
            return pick(["Ótima oferta nesse Puma 👟", "Puma bonito e confortável 👟"])
        if marca == "mizuno":
            return pick(["Ótima oferta nesse Mizuno 👟", "Mizuno confortável para o dia a dia 👟"])
        if marca == "fila":
            return pick(["Ótima oferta nesse Fila 👟", "Fila bonito para usar muito 👟"])
        return pick(["Ótima oferta nesse tênis 👟", "Tênis confortável para o dia a dia 👟", "Esse tênis vale conferir 👟"])

    if cat in ("moda_feminina", "moda_masculina", "moda"):
        if "camiseta" in nome or "camisa" in nome:
            if marca == "nike":
                return pick(["Camiseta Nike para usar muito 👕", "Nike básica sem erro 👕", "Ótima oferta nessa Nike 👕"])
            if marca == "adidas":
                return pick(["Camiseta Adidas para o dia a dia 👕", "Adidas básica sem erro 👕", "Ótima oferta nessa Adidas 👕"])
            return pick(["Camiseta boa para usar muito 👕", "Peça básica para o dia a dia 👕", "Ótima oferta nessa camiseta 👕"])
        if "short" in nome or "bermuda" in nome:
            return pick(["Short versátil com preço bom 👊", "Bermuda confortável para o dia a dia 👊", "Ótima oferta nessa bermuda 👊"])
        if "cueca" in nome or "boxer" in nome:
            return pick(["Boa oferta nesse kit de cuecas 👊", "Kit básico que vale ter 👊", "Para renovar a gaveta sem gastar muito 👊"])
        return pick(["Achado de moda para usar muito 👕", "Peça curinga para o dia a dia 👕", "Boa oferta em moda 👕"])

    if cat == "perfume":
        return pick(["Perfume com preço interessante ✨", "Achado perfumado do dia ✨", "Boa oferta para quem gosta de perfume ✨"])
    if cat == "fitness":
        return pick(["Boa oferta para quem treina 👊", "Achado fitness do dia 👊", "Produto útil para treino e rotina 👊"])
    if cat == "tech":
        return pick(["Achado tech com preço bom ⚡", "Boa oferta em tecnologia ⚡", "Gadget útil para o dia a dia ⚡"])
    if cat == "casa":
        return pick(["Achado útil para casa 🏠", "Boa oferta para o dia a dia 🏠", "Produto prático com preço bom 🏠"])
    if cat == "acessorios":
        return pick(["Acessório bonito com preço bom ✨", "Achado útil para o dia a dia ✨", "Boa oferta nesse acessório ✨"])
    if cat == "pet":
        return pick(["Achado para pet com preço bom 🐾", "Boa oferta para quem tem pet 🐾"])

    return pick(["Achado interessante do dia 🔥", "Boa oferta para aproveitar hoje 👊", "Preço interessante para conferir 🔥"])


try:
    log("INFO", "[V18] Ativo: rodízio real por perfil/categoria/marca + anti repetição forte + headlines naturais")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V19 — REFRESH DE PREÇO/TÍTULO/IMAGEM ANTES DE POSTAR
# ============================================================
import time as _brz_v19_time
import json as _brz_v19_json
import re as _brz_v19_re

try:
    from bs4 import BeautifulSoup as _brz_v19_BS
except Exception:
    _brz_v19_BS = None

BRIZZAH_V19_REFRESH_ENABLED = os.environ.get("BRIZZAH_REFRESH_PRECO", "true").lower() == "true"
BRIZZAH_V19_REFRESH_CACHE_MIN = int(os.environ.get("BRIZZAH_REFRESH_CACHE_MIN", "180") or "180")
BRIZZAH_V19_HIDE_PRICE_IF_FAIL = os.environ.get("BRIZZAH_OCULTAR_PRECO_SE_REFRESH_FALHAR", "true").lower() == "true"


def _brz_v19_clean_price_to_float(v):
    try:
        if v is None:
            return 0
        if isinstance(v, (int, float)):
            return float(v)
        s = str(v).replace("\xa0", " ").replace("R$", "").strip()
        s = _brz_v19_re.sub(r"[^0-9,\.]", "", s)
        if not s:
            return 0
        if "," in s and "." in s:
            s = s.replace(".", "").replace(",", ".")
...
def _brz_v19_url_real(url):
    try:
        url = (url or "").strip()
        if not url:
            return ""
        try:
            destino, _nome = _url_destino_e_nome_slug_externo(url)
            if destino:
                return destino
        except Exception:
            pass
...
def _brz_v19_extract_meta(html, prop):
    try:
        pats = [
            r'<meta[^>]+property=["\']' + _brz_v19_re.escape(prop) + r'["\'][^>]+content=["\']([^"\']+)',
            r'<meta[^>]+name=["\']' + _brz_v19_re.escape(prop) + r'["\'][^>]+content=["\']([^"\']+)',
            r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+property=["\']' + _brz_v19_re.escape(prop) + r'["\']',
            r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+name=["\']' + _brz_v19_re.escape(prop) + r'["\']',
        ]
        for pat in pats:
            m = _brz_v19_re.search(pat, html or "", flags=_brz_v19_re.I | _brz_v19_re.S)
            if m:
                try:
                    return _html_unescape(m.group(1))
                except Exception:
...
def _brz_v19_extract_jsonld(html):
    out = {"name": "", "price": 0, "old_price": 0, "image_url": ""}
    try:
        scripts = _brz_v19_re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>', html or "", flags=_brz_v19_re.I | _brz_v19_re.S)
        fila = []
        for raw in scripts:
            try:
                ...
                pass
            try:
                data = _brz_v19_json.loads(raw)
                fila.extend(data if isinstance(data, list) else [data])
            except Exception:
                continue
        while fila:
            obj = fila.pop(0)
            ...
            if isinstance(obj.get("@graph"), list):
                fila.extend(obj.get("@graph"))
            typ = obj.get("@type")
            typ_s = " ".join(typ) if isinstance(typ, list) else str(typ or "")
            if "Product" not in typ_s and not obj.get("offers"):
                continue
            if obj.get("name") and not out["name"]:
                out["name"] = str(obj.get("name")).strip()
            img = obj.get("image")
            if isinstance(img, list):
                img = img[0] if img else ""
            if img and not out["image_url"]:
                out["image_url"] = _fix_img(str(img))
            offers = obj.get("offers") or {}
            if isinstance(offers, list):
                offers = offers[0] if offers else {}
            if isinstance(offers, dict):
                price = offers.get("price") or offers.get("lowPrice") or offers.get("highPrice")
                val = _brz_v19_clean_price_to_float(price)
                if val and not out["price"]:
                    out["price"] = val
    except Exception:
        pass
    return out
...
def _brz_v19_extract_price_generic(html):
    try:
        for m in [_brz_v19_extract_meta(html, "product:price:amount"), _brz_v19_extract_meta(html, "og:price:amount"), _brz_v19_extract_meta(html, "twitter:data1")]:
            val = _brz_v19_clean_price_to_float(m)
            if 5 <= val <= 50000:
                return val
        pats = [
            r'"price"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'"priceAmount"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'"currentPrice"\s*:\s*\{[^}]*"value"\s*:\s*([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'"salePrice"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})',
        ]
        candidatos = []
        for pat in pats:
            for m in _brz_v19_re.finditer(pat, html or "", flags=_brz_v19_re.I | _brz_v19_re.S):
                val = _brz_v19_clean_price_to_float(m.group(1))
                if 5 <= val <= 50000:
                    candidatos.append(val)
        if candidatos:
            return sorted(candidatos)[0]
...
def _brz_v19_extract_old_price_generic(html, price=0):
    try:
        pats = [
            r'"listPrice"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'"originalPrice"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'"wasPrice"\s*:\s*"?([0-9]{1,7}(?:[\.,][0-9]{2})?)',
            r'preço anterior[^0-9]{0,60}R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})',
            r'de\s*R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})',
        ]
        cand = []
        for pat in pats:
            for m in _brz_v19_re.finditer(pat, html or "", flags=_brz_v19_re.I | _brz_v19_re.S):
                val = _brz_v19_clean_price_to_float(m.group(1))
                if price and val > price and val <= 50000:
                    cand.append(val)
...
def _brz_v19_refresh_fetch(url):
    dados = {"ok": False, "name": "", "price": 0, "old_price": 0, "image_url": "", "final_url": url or ""}
    url = _brz_v19_url_real(url)
    if not url:
        return dados
    try:
        headers = {
        ...
        html = r.text or ""
        dados["final_url"] = r.url or url
        if not html or r.status_code >= 400:
            return dados
        jd = _brz_v19_extract_jsonld(html)
        title = jd.get("name") or _brz_v19_extract_meta(html, "og:title") or _brz_v19_extract_meta(html, "twitter:title")
        if not title:
            m = _brz_v19_re.search(r"<title[^>]*>(.*?)</title>", html, flags=_brz_v19_re.I | _brz_v19_re.S)
            if m:
                title = _brz_v19_re.sub(r"\s+", " ", m.group(1)).strip()
        img = jd.get("image_url") or _brz_v19_extract_meta(html, "og:image") or _brz_v19_extract_meta(html, "twitter:image") or _brz_v19_extract_meta(html, "image")
        price = jd.get("price") or _brz_v19_extract_price_generic(html)
        old_price = jd.get("old_price") or _brz_v19_extract_old_price_generic(html, price)
        if title:
            dados["name"] = _limpar_nome_produto_ext(title, "")
        if img:
            dados["image_url"] = _fix_img(img)
        if price:
            dados["price"] = float(price)
            dados["ok"] = True
        if old_price and price and old_price > price:
            dados["old_price"] = float(old_price)
    except Exception as e:
        try: log("WARN", f"[V19] Refresh falhou request: {str(e)[:80]}")
        except Exception: pass
    return dados


def _brz_v19_cache_key(prod_id):
    return f"_brz_v19_refresh_{prod_id}"


def _brz_v19_refresh_produto_externo(p, force=False):
    if not isinstance(p, dict) or not BRIZZAH_V19_REFRESH_ENABLED:
        return p
    p = dict(p)
    ...
    if not link:
        return p
    try:
        if prod_id and not force:
            raw = cfg(_brz_v19_cache_key(prod_id), "")
            if raw:
                info = _brz_v19_json.loads(raw)
                ts = float(info.get("ts") or 0)
                if _brz_v19_time.time() - ts < BRIZZAH_V19_REFRESH_CACHE_MIN * 60:
                    if info.get("ok"):
                        p.update({k: v for k, v in info.items() if k in ("name", "price", "old_price", "image_url", "product_url") and v})
                        p["_refresh_ok"] = True
                    elif BRIZZAH_V19_HIDE_PRICE_IF_FAIL:
                        p["price"] = 0; p["old_price"] = 0; p["_refresh_ok"] = False
                    return p
    except Exception:
        pass
    atual = _brz_v19_refresh_fetch(link)
    if atual.get("ok"):
        if atual.get("name") and len(atual.get("name")) >= 8:
            p["name"] = atual["name"]
        p["price"] = atual.get("price") or p.get("price") or 0
        if atual.get("old_price") and atual.get("old_price") > (p.get("price") or 0):
            p["old_price"] = atual.get("old_price")
        else:
            p["old_price"] = 0
        if atual.get("image_url"):
            p["image_url"] = atual["image_url"]
        if atual.get("final_url"):
            p["product_url"] = atual["final_url"]
        p["_refresh_ok"] = True
        try:
            if prod_id:
                with get_db() as c:
                    c.execute("""
                        UPDATE external_products
                        SET name=COALESCE(NULLIF(?,''), name), price=?, old_price=?, image_url=COALESCE(NULLIF(?,''), image_url), product_url=COALESCE(NULLIF(?,''), product_url)
                        WHERE id=?
                    """, (p.get("name") or "", _safe_float(p.get("price"), 0), _safe_float(p.get("old_price"), 0), p.get("image_url") or "", p.get("product_url") or "", prod_id))
                cfg_set(_brz_v19_cache_key(prod_id), _brz_v19_json.dumps({"ts": _brz_v19_time.time(), "ok": True, "name": p.get("name") or "", "price": p.get("price") or 0, "old_price": p.get("old_price") or 0, "image_url": p.get("image_url") or "", "product_url": p.get("product_url") or ""}, ensure_ascii=False))
        except Exception as e:
            try: log("WARN", f"[V19] Falha atualizando banco: {str(e)[:80]}")
            except Exception: pass
        return p
    p["_refresh_ok"] = False
    if BRIZZAH_V19_HIDE_PRICE_IF_FAIL:
        p["price"] = 0; p["old_price"] = 0
    try:
        if prod_id:
            cfg_set(_brz_v19_cache_key(prod_id), _brz_v19_json.dumps({"ts": _brz_v19_time.time(), "ok": False}, ensure_ascii=False))
        log("WARN", f"[V19] Não confirmei preço atual; legenda vai sem preço | id={prod_id} | nome={(p.get('name') or '')[:70]}")
    except Exception:
        pass
    return p

try:
    _brz_v19_caption_wa_anterior = _caption_wa_externo
except Exception:
    _brz_v19_caption_wa_anterior = None
try:
    _brz_v19_caption_ig_anterior = _caption_ig_externo
except Exception:
    _brz_v19_caption_ig_anterior = None

def _caption_wa_externo(p):
    p = _brz_v19_refresh_produto_externo(p)
    if _brz_v19_caption_wa_anterior:
        return _brz_v19_caption_wa_anterior(p)
    return str(p.get("name") or "Oferta selecionada")

def _caption_ig_externo(p):
    p = _brz_v19_refresh_produto_externo(p)
    if _brz_v19_caption_ig_anterior:
        return _brz_v19_caption_ig_anterior(p)
    return str(p.get("name") or "Oferta selecionada")

try:
    log("INFO", "[V19] Refresh de preço ativo: atualiza preço/título/imagem antes da postagem; oculta preço se não confirmar")
except Exception:
    pass
# ============================================================
# FIM BRIZZAH V19
...
# ============================================================
# BRIZZAH V20 — FEED INTELIGENTE DEFINITIVO REAL
# - Não repete link/produto enquanto houver outros disponíveis.
# - Evita mesma marca/categoria em sequência.
# - Escolhe semi-aleatório entre bons produtos.
# - Mantém refresh de preço do V19 antes da postagem.
# ============================================================
import random as _brz_v20_random
import re as _brz_v20_re

BRIZZAH_V20_LOOKBACK_DIAS = int(os.environ.get("BRIZZAH_V20_LOOKBACK_DIAS", "60") or "60")
BRIZZAH_V20_TOP_POOL = int(os.environ.get("BRIZZAH_V20_TOP_POOL", "25") or "25")


def _brz_v20_norm(txt):
    try:
        return _brz_v18_norm(txt)
    except Exception:
        try:
            return _brz_norm(txt)
        except Exception:
            ...
            s = str(txt or "").lower()
            s = "".join(c for c in _ud.normalize("NFD", s) if _ud.category(c) != "Mn")
            s = _brz_v20_re.sub(r"[^a-z0-9\s]", " ", s)
            return _brz_v20_re.sub(r"\s+", " ", s).strip()


def _brz_v20_source(p):
    try:
        return _brz_v10_source((p or {}).get("source"))
    except Exception:
        return str((p or {}).get("source") or "").lower().strip()


def _brz_v20_link_key(p):
    try:
        return _brz_v18_link_base((p or {}).get("affiliate_url") or (p or {}).get("product_url") or "")
    except Exception:
        return _brz_v20_norm((p or {}).get("affiliate_url") or (p or {}).get("product_url") or "")


def _brz_v20_name_key(p):
    try:
        return _brz_v18_nome_chave((p or {}).get("name") or "")
    except Exception:
        n = _brz_v20_norm((p or {}).get("name") or "")
        n = _brz_v20_re.sub(r"\b(3[0-9]|4[0-9]|pp|p|m|g|gg|xg|xxg|preto|branco|azul|verde|rosa|cinza)\b", " ", n)
        return " ".join(n.split()[:8])


def _brz_v20_cat(p):
    try:
        return _brz_v18_categoria(p)
    except Exception:
        n = _brz_v20_norm(" ".join([str((p or {}).get("name") or ""), str((p or {}).get("category") or "")]))
        if any(x in n for x in ["perfume", "colonia", "lattafa", "fragrancia"]): return "perfume"
        if any(x in n for x in ["tenis", "sneaker", "calcado", "olympikus", "new balance", "running"]): return "tenis"
        if any(x in n for x in ["camiseta", "camisa", "polo", "bermuda", "short", "cueca", "calca", "moletom"]): return "moda"
        if any(x in n for x in ["fone", "jbl", "xiaomi", "smartwatch", "relogio", "bluetooth"]): return "tech"
        if any(x in n for x in ["creatina", "whey", "fitness", "academia", "legging"]): return "fitness"
        if any(x in n for x in ["mochila", "bolsa", "oculos", "cinto"]): return "acessorios"
        if any(x in n for x in ["casa", "cozinha", "panela", "toalha", "organizador"]): return "casa"
        return "geral"


def _brz_v20_brand(p):
    try:
        return _brz_v18_marca(p)
    except Exception:
        n = _brz_v20_norm(" ".join([str((p or {}).get("name") or ""), str((p or {}).get("brand") or "")]))
        for m in ["new balance", "olympikus", "adidas", "nike", "puma", "mizuno", "fila", "jbl", "xiaomi", "lattafa", "casio"]:
            if m in n:
                return m
        return ""
...
def _brz_v20_ensure():
    try:
        with get_db() as c:
            c.execute("""
                CREATE TABLE IF NOT EXISTS brizzah_used_links (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    external_product_id INTEGER,
                    link_key TEXT DEFAULT '',
                    name_key TEXT DEFAULT '',
            ...
            """)
            c.execute("CREATE INDEX IF NOT EXISTS idx_brz_used_link ON brizzah_used_links(link_key)")
            c.execute("CREATE INDEX IF NOT EXISTS idx_brz_used_name ON brizzah_used_links(name_key)")
            c.execute("CREATE INDEX IF NOT EXISTS idx_brz_used_at ON brizzah_used_links(used_at)")
    except Exception as e:
        try: log("WARN", f"[V20] Falha ao criar histórico: {str(e)[:80]}")
        except Exception: pass


def _brz_v20_usados():
    _brz_v20_ensure()
    usados = {"links": set(), "names": set(), "cats": [], "brands": []}
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT link_key, name_key, category_key, brand_key
                FROM brizzah_used_links
                WHERE used_at >= datetime('now', ?)
                ORDER BY id DESC
            """, (f"-{BRIZZAH_V20_LOOKBACK_DIAS} days",)).fetchall()
            for r in rows:
                d = dict(r)
                if d.get("link_key"): usados["links"].add(d["link_key"])
                if d.get("name_key"): usados["names"].add(d["name_key"])
                if d.get("category_key"): usados["cats"].append(d["category_key"])
                if d.get("brand_key"): usados["brands"].append(d["brand_key"])
            rows2 = c.execute("""
                SELECT name, affiliate_url, product_url, shop_name
                FROM products
            ...
            for r in rows2:
                d = dict(r)
                fake = {"name": d.get("name"), "affiliate_url": d.get("affiliate_url"), "product_url": d.get("product_url"), "source": d.get("shop_name")}
                lk, nk = _brz_v20_link_key(fake), _brz_v20_name_key(fake)
                if lk: usados["links"].add(lk)
                if nk: usados["names"].add(nk)
                usados["cats"].append(_brz_v20_cat(fake))
                b = _brz_v20_brand(fake)
                if b: usados["brands"].append(b)
    except Exception:
        pass
    return usados


def _brz_v20_registrar(p):
    _brz_v20_ensure()
    try:
        with get_db() as c:
            c.execute("""
                INSERT INTO brizzah_used_links
                (external_product_id, link_key, name_key, category_key, brand_key, source, product_name, used_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now','localtime'))
            """, (
                (p or {}).get("id") or 0,
                _brz_v20_link_key(p),
                ...
                _brz_v20_brand(p),
                _brz_v20_source(p),
                ((p or {}).get("name") or "")[:180],
            ))
    except Exception as e:
        try: log("WARN", f"[V20] Falha registrar usado: {str(e)[:80]}")
        except Exception: pass


def _brz_v20_ordem():
    return ["moda_feminina", "tenis_masculino", "perfume", "tech", "moda_masculina", "tenis_feminino", "fitness", "casa", "acessorios", "tenis", "moda", "geral"]


def _brz_v20_alvo():
    ordem = _brz_v20_ordem()
    try: idx = int(cfg("_brz_v20_idx", "0") or 0)
    except Exception: idx = 0
    recentes = _brz_v20_usados()["cats"][:4]
    for i in range(len(ordem)):
        cat = ordem[(idx+i) % len(ordem)]
        if cat not in recentes[:2]: return cat
    return ordem[idx % len(ordem)]


def _brz_v20_avancar():
    try:
        cfg_set("_brz_v20_idx", str(int(cfg("_brz_v20_idx", "0") or 0) + 1))
    except Exception:
        pass


def _brz_v20_score(p, alvo):
    score = 0
    try: score += int(score_externo(p))
    except Exception: pass
    cat, brand, source = _brz_v20_cat(p), _brz_v20_brand(p), _brz_v20_source(p)
    if cat == alvo: score += 100
    elif alvo: score -= 10
    if source == "netshoes": score += 25
    elif source == "mercadolivre": score += 22
    elif source == "amazon": score += 10
    elif source == "shopee": score -= 999
    if brand: score += 8
    usados = _brz_v20_usados()
    if cat in usados["cats"][:3]: score -= 60
    if brand and brand in usados["brands"][:5]: score -= 80
    return score + _brz_v20_random.randint(0, 30)
...
def _brz_v20_banco():
    try:
        with get_db() as c:
            ...
            """).fetchall()
        produtos = [dict(r) for r in rows]
        _brz_v20_random.shuffle(produtos)
        return produtos
    except Exception as e:
        try: log("ERROR", f"[V20] Erro carregando banco: {str(e)[:100]}")
        except Exception: pass
        return []


def buscar_external_products_aprovados(limit=30):
    """V20: produtos diferentes e links diferentes até esgotar o banco."""
    _brz_v20_ensure()
    try: qtd = max(1, int(limit or cfg("products_per_cycle", "1") or 1))
    except Exception: qtd = 1
    todos = _brz_v20_banco()
    usados = _brz_v20_usados()
    candidatos = []
    for p in todos:
        try:
            p["source"] = _brz_v20_source(p)
            if p["source"] == "shopee": continue
            lk, nk = _brz_v20_link_key(p), _brz_v20_name_key(p)
            if (lk and lk in usados["links"]) or (nk and nk in usados["names"]): continue
            try:
                if "_brz_v10_produto_ruim" in globals() and _brz_v10_produto_ruim(p): continue
            except Exception: pass
            candidatos.append(p)
        except Exception:
            continue
    if not candidatos and todos:
        try:
            with get_db() as c: c.execute("DELETE FROM brizzah_used_links")
            log("INFO", "[V20] Todos os links/produtos já foram usados; histórico zerado para novo giro completo.")
        except Exception: pass
        usados = _brz_v20_usados()
        candidatos = [p for p in todos if _brz_v20_source(p) != "shopee"]
    escolhidos = []
    for _ in range(qtd):
        if not candidatos: break
        alvo = _brz_v20_alvo()
        pool = [p for p in candidatos if _brz_v20_cat(p) == alvo]
        if not pool:
            recentes_cats = set(_brz_v20_usados()["cats"][:4])
            pool = [p for p in candidatos if _brz_v20_cat(p) not in recentes_cats] or candidatos
        ordenados = sorted(pool, key=lambda p: _brz_v20_score(p, alvo), reverse=True)
        top = ordenados[:max(3, min(BRIZZAH_V20_TOP_POOL, len(ordenados)))]
        escolhido = _brz_v20_random.choice(top)
        try:
            if "_brz_v19_refresh_produto_externo" in globals():
                escolhido = _brz_v19_refresh_produto_externo(escolhido)
        except Exception: pass
        escolhidos.append(escolhido)
        _brz_v20_registrar(escolhido)
        _brz_v20_avancar()
        lk, nk, brand, cat = _brz_v20_link_key(escolhido), _brz_v20_name_key(escolhido), _brz_v20_brand(escolhido), _brz_v20_cat(escolhido)
        candidatos = [p for p in candidatos if _brz_v20_link_key(p) != lk and _brz_v20_name_key(p) != nk and _brz_v20_cat(p) != cat and not (brand and _brz_v20_brand(p) == brand)]
        try: log("INFO", f"[V20] Escolhido | alvo={alvo} | cat={cat} | marca={brand} | nome={(escolhido.get('name') or '')[:70]}")
        except Exception: pass
    return escolhidos[:qtd]

try:
    log("INFO", "[V20] Ativo: feed inteligente definitivo, sem repetir links/produtos até esgotar banco + refresh preço V19")
except Exception:
    pass
# ============================================================
# FIM BRIZZAH V20
...
# ============================================================
# BRIZZAH V21 — RESPEITA "PRODUTOS POR ENVIO" + REMOVE CUPOM FALSO
# Correções:
# - Remove travas antigas que forçavam 1 produto por ciclo.
# - O seletor passa a ignorar limit=1 antigo e usa cfg("products_per_cycle").
# - Seleciona produtos diferentes no mesmo envio.
# - Não exibe cupom quando ele não veio confiável do anúncio/banco.
# ============================================================

BRIZZAH_V21_MAX_PRODUTOS_POR_ENVIO = int(os.environ.get("BRIZZAH_MAX_PRODUTOS_POR_ENVIO", "5") or "5")


def _brz_v21_qtd_por_ciclo():
    try:
        q = int(cfg("products_per_cycle", "1") or "1")
    except Exception:
        q = 1
    if q < 1:
        q = 1
    if q > BRIZZAH_V21_MAX_PRODUTOS_POR_ENVIO:
        q = BRIZZAH_V21_MAX_PRODUTOS_POR_ENVIO
    return q


def _brz_v21_cupom_confiavel(cupom):
    c = str(cupom or "").strip().upper()
    falsos = {"", "SPAN", "NULL", "NONE", "N/A", "NA", "-", "0", "SEM CUPOM", "CUPOM"}
    if c in falsos:
        return ""
    if len(c) < 4 or len(c) > 25:
        return ""
    import re as _re
    if not _re.fullmatch(r"[A-Z0-9][A-Z0-9_\-]{3,24}", c):
        return ""
    return c


# guarda seletor V20/anterior
try:
    _brz_v21_buscar_external_products_aprovados_anterior = buscar_external_products_aprovados
except Exception:
    _brz_v21_buscar_external_products_aprovados_anterior = None


def buscar_external_products_aprovados(limit=30):
    """
    V21: usa a quantidade do painel, não o limit=1 herdado.
    Se o painel estiver em 2, retorna 2 produtos diferentes.
    """
    qtd = _brz_v21_qtd_por_ciclo()
    produtos = []
    if _brz_v21_buscar_external_products_aprovados_anterior:
        try:
            produtos = _brz_v21_buscar_external_products_aprovados_anterior(limit=qtd) or []
        except Exception as e:
            try: log("WARN", f"[V21] seletor anterior falhou: {str(e)[:80]}")
            except Exception: pass
            produtos = []

    # Remove duplicados dentro do mesmo ciclo
    saida = []
    vistos = set()
    for p in produtos:
        try:
            lk = _brz_v20_link_key(p) if "_brz_v20_link_key" in globals() else (p.get("affiliate_url") or p.get("product_url") or "")
            nk = _brz_v20_name_key(p) if "_brz_v20_name_key" in globals() else (p.get("name") or "").lower().strip()
            chave = (lk, nk)
            if chave in vistos:
                continue
            vistos.add(chave)
    ...
    # Completa direto do banco se o seletor anterior devolveu menos que o painel
    if len(saida) < qtd:
        try:
            todos = _brz_v20_banco() if "_brz_v20_banco" in globals() else []
            import random as _random
            _random.shuffle(todos)
            usados_lk = set()
            usados_nk = set()
            for p in saida:
                try:
                    usados_lk.add(_brz_v20_link_key(p) if "_brz_v20_link_key" in globals() else (p.get("affiliate_url") or p.get("product_url") or ""))
                    usados_nk.add(_brz_v20_name_key(p) if "_brz_v20_name_key" in globals() else (p.get("name") or "").lower().strip())
                except Exception:
                    pass
            for p in todos:
                if len(saida) >= qtd:
                    break
                try:
                    source = _brz_v20_source(p) if "_brz_v20_source" in globals() else str(p.get("source") or "").lower().strip()
                    if source == "shopee":
                        continue
                    lk = _brz_v20_link_key(p) if "_brz_v20_link_key" in globals() else (p.get("affiliate_url") or p.get("product_url") or "")
                    nk = _brz_v20_name_key(p) if "_brz_v20_name_key" in globals() else (p.get("name") or "").lower().strip()
                    if lk in usados_lk or nk in usados_nk:
                        continue
                    try:
                        if "_brz_v19_refresh_produto_externo" in globals():
                            p = _brz_v19_refresh_produto_externo(p)
            ...
            try: log("WARN", f"[V21] falha completando produtos do banco: {str(e)[:80]}")
            except Exception: pass

    try:
        log("INFO", f"[V21] Produtos por envio configurado={qtd} | selecionados={len(saida[:qtd])}")
    except Exception:
        pass
...
# Guarda captions anteriores para filtrar cupom falso sem alterar todo layout
try:
    _brz_v21_caption_wa_anterior = _caption_wa_externo
except Exception:
    _brz_v21_caption_wa_anterior = None

try:
    _brz_v21_caption_ig_anterior = _caption_ig_externo
except Exception:
    _brz_v21_caption_ig_anterior = None


def _caption_wa_externo(p):
    if isinstance(p, dict):
        p = dict(p)
        p["coupon"] = _brz_v21_cupom_confiavel(p.get("coupon"))
    if _brz_v21_caption_wa_anterior:
        txt = _brz_v21_caption_wa_anterior(p)
        # Remove linhas de cupom inválido que algum trecho antigo ainda gere
        linhas = []
        for linha in str(txt).splitlines():
            if linha.strip().lower().startswith("cupom:"):
                cup = linha.split(":", 1)[-1].replace("*", "").replace("⚠️", "").strip()
                if not _brz_v21_cupom_confiavel(cup):
                    continue
            linhas.append(linha)
        return "\n".join(linhas)
    return str((p or {}).get("name") or "Oferta selecionada")
...
def _caption_ig_externo(p):
    if isinstance(p, dict):
        p = dict(p)
        p["coupon"] = _brz_v21_cupom_confiavel(p.get("coupon"))
    if _brz_v21_caption_ig_anterior:
        return _brz_v21_caption_ig_anterior(p)
    return str((p or {}).get("name") or "Oferta selecionada")


try:
    log("INFO", "[V21] Ativo: respeita 2+ produtos por envio e remove cupom falso")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V22 — LEGENDAS INTELIGENTES POR TIPO DE PRODUTO
# Corrige pantufa/chinelo/sandália sendo chamados de tênis.
# ============================================================

import random as _brz_v22_random
import re as _brz_v22_re

def _brz_v22_norm(txt):
    try:
        return _brz_v20_norm(txt)
    except Exception:
        try:
            return _brz_v18_norm(txt)
        except Exception:
            try:
                ...
                s = str(txt or "").lower()
                s = "".join(c for c in _ud.normalize("NFD", s) if _ud.category(c) != "Mn")
                s = _brz_v22_re.sub(r"[^a-z0-9\s]", " ", s)
                return _brz_v22_re.sub(r"\s+", " ", s).strip()

def _brz_v22_tipo_produto(p):
    texto = _brz_v22_norm(" ".join([
        str((p or {}).get("name") or ""),
        str((p or {}).get("category") or ""),
        str((p or {}).get("brand") or ""),
        str((p or {}).get("notes") or ""),
        str((p or {}).get("product_url") or ""),
    ]))

    # ordem importa: pantufa/chinelo antes de tênis
    if any(x in texto for x in ["pantufa", "chinelo forrada", "chinelo de quarto", "slipper", "home riscos", "inverno by"]):
        return "pantufa"
    if any(x in texto for x in ["chinelo", "havaianas", "slide", "sandalia slide"]):
        return "chinelo"
    if any(x in texto for x in ["sandalia", "rasteira", "rasteirinha"]):
        return "sandalia"
    if any(x in texto for x in ["bota", "coturno"]):
        return "bota"
    if any(x in texto for x in ["tenis", "sneaker", "running", "corrida", "fresh foam", "court", "run", "olympikus", "adidas", "nike", "puma", "mizuno", "new balance", "fila", "asics"]):
        return "tenis"

    if any(x in texto for x in ["camiseta", "camisa", "polo", "regata"]):
        return "camiseta"
    if any(x in texto for x in ["bermuda", "short"]):
        return "short"
    if any(x in texto for x in ["cueca", "boxer"]):
        return "cueca"
    if any(x in texto for x in ["calca", "legging", "top academia", "cropped academia", "conjunto academia", "dryfit", "dry fit"]):
        return "fitness_roupa"
    if any(x in texto for x in ["perfume", "colonia", "fragrancia", "lattafa", "egeo", "boticario", "granado", "eau de parfum", "deo parfum"]):
        return "perfume"
    if any(x in texto for x in ["creatina", "whey", "suplemento", "protein", "coqueteleira"]):
        return "suplemento"
    if any(x in texto for x in ["fone", "bluetooth", "jbl", "xiaomi", "smartwatch", "relogio", "g shock", "casio", "caixa de som", "boombox"]):
        return "tech"
    if any(x in texto for x in ["mochila", "bolsa", "mala"]):
        return "mochila"
    if any(x in texto for x in ["toalha", "panela", "airfryer", "espelho", "organizador", "casa", "cozinha", "cama", "mesa"]):
        return "casa"
    if any(x in texto for x in ["pet", "cachorro", "gato", "racao", "coleira", "caminha pet"]):
        return "pet"
    return "geral"

def _brz_v22_marca(p):
    texto = _brz_v22_norm(" ".join([
        str((p or {}).get("name") or ""),
        str((p or {}).get("brand") or "")
    ]))
    for m in ["new balance", "olympikus", "adidas", "nike", "puma", "mizuno", "fila", "asics", "reebok", "jbl", "xiaomi", "casio", "lattafa", "granado", "boticario"]:
        if m in texto:
            return m
    return ""

def _brz_v22_nome_marca(marca):
    return {
        ...
        "granado": "Granado",
        "boticario": "Boticário",
    }.get(marca, marca.title() if marca else "")

def _headline_promo_externo(p):
    """Headline final V22: identifica produto antes de escrever a chamada."""
    p = p if isinstance(p, dict) else {"name": str(p)}
    tipo = _brz_v22_tipo_produto(p)
    marca_nome = _brz_v22_nome_marca(_brz_v22_marca(p))

    def pick(opts):
        return _brz_v22_random.choice(opts)

    if tipo == "pantufa":
        return pick(["Pantufa confortável para usar em casa 🏠", "Boa oferta nessa pantufa confortável 🏠", "Achado quentinho para o inverno 🏠", "Conforto para ficar em casa gastando pouco 🏠"])
    if tipo == "chinelo":
        return pick(["Chinelo confortável para o dia a dia 👊", "Boa oferta nesse chinelo 👊", "Conforto simples que todo mundo usa 👊"])
    if tipo == "sandalia":
        return pick(["Boa oferta nessa sandália 👡", "Sandália confortável para o dia a dia 👡", "Achado leve para usar muito 👡"])
    if tipo == "bota":
        return pick(["Boa oferta nessa bota 👢", "Bota bonita para compor o look 👢", "Achado interessante nessa bota 👢"])
    if tipo == "tenis":
        if marca_nome:
            return pick([f"Ótima oferta nesse {marca_nome} 👟", f"{marca_nome} confortável para o dia a dia 👟", f"Esse {marca_nome} vale conferir 👟", f"{marca_nome} com preço interessante 👟"])
        return pick(["Ótima oferta nesse tênis 👟", "Tênis confortável para o dia a dia 👟", "Esse tênis vale conferir 👟", "Tênis com preço interessante 👟"])
    if tipo == "camiseta":
        if marca_nome:
            return pick([f"Camiseta {marca_nome} para usar muito 👕", f"Boa oferta nessa camiseta {marca_nome} 👕", f"{marca_nome} básica para o dia a dia 👕"])
        return pick(["Camiseta boa para usar muito 👕", "Peça básica para o dia a dia 👕", "Boa oferta nessa camiseta 👕"])
    if tipo == "short":
        return pick(["Short versátil com preço bom 👊", "Bermuda confortável para o dia a dia 👊", "Boa oferta nesse short 👊"])
    if tipo == "cueca":
        return pick(["Boa oferta nesse kit de cuecas 👊", "Kit básico que vale ter 👊", "Para renovar a gaveta sem gastar muito 👊"])
    if tipo == "fitness_roupa":
        return pick(["Roupa de treino com preço interessante 🏋️", "Boa oferta para quem treina 🏋️", "Achado fitness para usar muito 🏋️"])
    if tipo == "perfume":
        return pick(["Perfume com preço interessante ✨", "Achado perfumado do dia ✨", "Boa oferta para quem gosta de perfume ✨"])
    if tipo == "suplemento":
        return pick(["Boa oferta para quem treina 👊", "Suplemento com preço interessante 👊", "Achado fitness do dia 👊"])
    if tipo == "tech":
        return pick(["Achado tech com preço bom ⚡", "Boa oferta em tecnologia ⚡", "Gadget útil para o dia a dia ⚡"])
    if tipo == "mochila":
        if marca_nome:
            return pick([f"Mochila {marca_nome} para o dia a dia 🎒", f"Boa oferta nessa mochila {marca_nome} 🎒", "Mochila prática para usar muito 🎒"])
        return pick(["Mochila prática para o dia a dia 🎒", "Boa oferta nessa mochila 🎒", "Achado útil para carregar tudo 🎒"])
    if tipo == "casa":
        return pick(["Achado útil para casa 🏠", "Boa oferta para o dia a dia 🏠", "Produto prático com preço bom 🏠"])
    if tipo == "pet":
        return pick(["Achado para pet com preço bom 🐾", "Boa oferta para quem tem pet 🐾"])
    return pick(["Boa oferta para aproveitar hoje 👊", "Achado interessante do dia 🔥", "Preço interessante para conferir 🔥", "Garimpo Brizzah para economizar 👊"])

try:
    log("INFO", "[V22] Ativo: legendas inteligentes por tipo de produto; pantufa/chinelo não vira tênis")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V23 — PREÇO INTELIGENTE + GATILHOS PROFISSIONAIS
# - Mostra "de R$ X por R$ Y" somente quando o preço anterior for confiável.
# - Se não houver preço anterior, mostra "por R$ Y" + "Já com desconto no anúncio".
# - Se não confirmar preço, mostra "Confira o valor atualizado no link".
# - Não mostra cupom se não existir cupom real.
# ============================================================

import random as _brz_v23_random

def _brz_v23_brl(v):
    try:
        return _format_brl(v)
    except Exception:
        try:
            v = float(v or 0)
            return f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
...
def _brz_v23_gatilho(p, desc=0, tem_preco_antigo=False):
    """Gatilhos curtos, sem prometer o que não foi validado."""
    nome = ""
    try:
        nome = _brz_v22_norm(p.get("name") or "")
    except Exception:
        try:
            nome = _brz_norm(p.get("name") or "")
        except Exception:
            nome = str(p.get("name") or "").lower()

    if tem_preco_antigo and desc >= 35:
        return _brz_v23_random.choice([
            "💥 Oferta forte hoje",
            "🔥 Preço bem abaixo do normal",
            "💸 Desconto pesado encontrado",
        ])

    if tem_preco_antigo and desc >= 15:
        return _brz_v23_random.choice([
            "🔥 Ótima oportunidade no link",
            "💸 Preço interessante hoje",
            "🔥 Achado com desconto real",
        ])

    if any(x in nome for x in ["perfume", "lattafa", "colonia", "fragrancia"]):
        return _brz_v23_random.choice([
            "✨ Achado perfumado do dia",
            "✨ Boa oferta em perfume",
            "✨ Preço interessante nesse perfume",
        ])

    if any(x in nome for x in ["tenis", "nike", "adidas", "new balance", "olympikus", "puma", "mizuno", "fila"]):
        return _brz_v23_random.choice([
            "👟 Boa oferta em calçado",
            "👟 Preço interessante nesse modelo",
            "👟 Vale conferir essa oferta",
        ])

    return _brz_v23_random.choice([
        "🔥 Boa oferta para aproveitar hoje",
        "💸 Preço interessante para conferir",
        "🛍️ Achado Brizzah do dia",
        "🔥 Garimpo para economizar",
    ])

def _brz_v23_cupom_valido(cupom):
    """Evita cupom fake / lixo de HTML como SPAN, DIV, NULL."""
    c = str(cupom or "").strip().upper()
    if not c:
        return ""
    invalidos = {
        "SPAN", "DIV", "NULL", "NONE", "N/A", "NA", "FALSE", "TRUE",
        "CUPOM", "DESCONTO", "PROMO", "PROMOCAO", "PROMOÇÃO"
    }
    if c in invalidos:
        return ""
    if len(c) < 4 or len(c) > 24:
        return ""
    # precisa ter letra ou número e não pode ser só tag/ruído
    import re as _re
    if not _re.fullmatch(r"[A-Z0-9_\-]{4,24}", c):
        return ""
    return c

def _caption_wa_externo(p):
    """Legenda WhatsApp V23 com preço mais chamativo e confiável."""
    try:
        if "_brz_v19_refresh_produto_externo" in globals():
            p = _brz_v19_refresh_produto_externo(p)
    except Exception:
    ...
    try:
        ...
        if not (orig and preco and orig > preco):
            orig = 0

    desc = int((1 - preco / orig) * 100) if orig and preco and orig > preco else 0
    link = p.get("affiliate_url") or p.get("product_url") or ""
    cupom = _brz_v23_cupom_valido(p.get("coupon"))
    installments = (p.get("installments") or "").strip()

    tem_preco_antigo = bool(orig and preco and orig > preco)
    headline = _headline_promo_externo(p)
    gatilho = _brz_v23_gatilho(p, desc=desc, tem_preco_antigo=tem_preco_antigo)

    linhas = [f"*{headline}*", "", nome, ""]

    if preco:
        if tem_preco_antigo:
            linhas.append(f"de {_brz_v23_brl(orig)} por *{_brz_v23_brl(preco)}* 👊")
            if desc >= 10:
                linhas.append(f"🔥 {desc}% OFF")
        else:
            linhas.append(f"por *{_brz_v23_brl(preco)}* 👊")
            linhas.append("🔥 Já com desconto no anúncio")
        if installments:
            linhas.append(f"💳 {installments}")
    else:
        linhas.append("🔥 Oferta com desconto no anúncio")
        linhas.append("⚠️ Confira o valor atualizado no link")

    if cupom:
        linhas.append(f"🏷 Cupom: *{cupom}*")

    # Gatilho abaixo do preço, sem exagero
    if gatilho:
        linhas.insert(0, f"*{gatilho}*")
        linhas.insert(1, "")
...
def _caption_ig_externo(p):
    """Legenda Instagram V23 seguindo a mesma lógica de preço."""
    try:
        if "_brz_v19_refresh_produto_externo" in globals():
            p = _brz_v19_refresh_produto_externo(p)
    except Exception:
    ...
    if preco:
        if tem_preco_antigo:
            ...
            linhas.append(f"Preço de hoje: {_brz_v23_brl(preco)}")
            linhas.append("Já com desconto no anúncio")
    else:
        linhas.append("Confira o valor atualizado no link 👇")

    linhas += [
        "",
        f"Origem: {fonte}",
        "Link na bio ou direto no grupo VIP 👆",
        link[:120],
        "",
        "#achadinhos #ofertas #brizzah #promocao"
    ]
    return "\n".join(linhas)

try:
    log("INFO", "[V23] Ativo: preço inteligente, desconto confiável e sem cupom fake")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V24 — CORREÇÃO URGENTE: 2+ PRODUTOS POR CICLO
# - Corrige leitura de produtos por envio.
# - Força o seletor a buscar a quantidade configurada.
# - Adiciona envio complementar se o ciclo antigo postar só 1.
# - Logs profissionais para diagnóstico.
# ============================================================

import time as _brz_v24_time

BRIZZAH_V24_MAX_PRODUTOS = int(os.environ.get("BRIZZAH_MAX_PRODUTOS_POR_CICLO", "4") or "4")
BRIZZAH_V24_DELAY_SEG = int(os.environ.get("BRIZZAH_DELAY_ENTRE_POSTS", "15") or "15")

def _brz_v24_int(v, default=1):
    try:
        import re as _re
        s = str(v or "").strip()
        m = _re.search(r"\d+", s)
...
def _brz_v24_get_raw_config(keys):
    # 1) tenta via cfg()
    for k in keys:
        try:
            v = cfg(k, "")
    ...
    # 2) tenta direto no banco
    try:
        with get_db() as c:
            for k in keys:
                row = c.execute("SELECT value FROM config WHERE key=?", (k,)).fetchone()
                if row and str(row["value"]).strip():
                    return str(row["value"]).strip(), k
    except Exception:
        pass

    # 3) procura chaves parecidas
    try:
        with get_db() as c:
            rows = c.execute("SELECT key, value FROM config").fetchall()
        for r in rows:
            k = str(r["key"] or "").lower()
            v = str(r["value"] or "").strip()
            if any(x in k for x in ["product", "produto", "envio", "cycle", "ciclo", "quantidade", "qtd"]):
                n = _brz_v24_int(v, 0)
                if 1 <= n <= BRIZZAH_V24_MAX_PRODUTOS:
                    return v, str(r["key"])
    except Exception:
        pass
...
def _brz_v24_produtos_por_envio():
    keys = [
        "products_per_cycle",
        "produtos_por_envio",
        "wa_products_per_cycle",
        "whatsapp_products_per_cycle",
        "auto_products_per_cycle",
        "qtd_produtos_envio",
        "quantidade_produtos",
        "wa_qtd_produtos",
        "produtos_por_ciclo",
    ]
    raw, origem = _brz_v24_get_raw_config(keys)
    qtd = _brz_v24_int(raw, 0)

    # Se nada estiver salvo, usar 2 como padrão profissional
    if not qtd:
        qtd = _brz_v24_int(os.environ.get("BRIZZAH_PRODUTOS_POR_ENVIO", "2"), 2)
        origem = "default_2"

    qtd = max(1, min(BRIZZAH_V24_MAX_PRODUTOS, qtd))

    # normaliza em chaves oficiais
    try:
        cfg_set("products_per_cycle", str(qtd))
        cfg_set("produtos_por_envio", str(qtd))
    except Exception:
        try:
            with get_db() as c:
                c.execute("INSERT OR REPLACE INTO config(key,value) VALUES(?,?)", ("products_per_cycle", str(qtd)))
                c.execute("INSERT OR REPLACE INTO config(key,value) VALUES(?,?)", ("produtos_por_envio", str(qtd)))
        except Exception:
            pass

    try:
        log("INFO", f"[V24] Produtos por envio carregado={qtd} | origem={origem} | raw={raw}")
    except Exception:
        pass

    return qtd

# preserva seletor anterior
try:
    _brz_v24_buscar_anterior = buscar_external_products_aprovados
except Exception:
    _brz_v24_buscar_anterior = None

def buscar_external_products_aprovados(limit=30):
    """
    V24: impede que versões antigas passem limit=1 e travem o ciclo.
    """
    qtd = _brz_v24_produtos_por_envio()
    try:
        limit_antigo = int(limit or 0)
    except Exception:
        limit_antigo = 0
    limit_final = max(qtd, limit_antigo, 1)

    produtos = []
    if _brz_v24_buscar_anterior:
        try:
            produtos = _brz_v24_buscar_anterior(limit=limit_final) or []
        except TypeError:
            try:
                produtos = _brz_v24_buscar_anterior(limit_final) or []
            except Exception:
                produtos = []
        except Exception as e:
            try: log("ERROR", f"[V24] seletor anterior falhou: {str(e)[:120]}")
            except Exception: pass

    # Se ainda veio menos que o configurado, completa direto do banco.
    if len(produtos or []) < limit_final:
        try:
            usados_ids = {str(p.get("id")) for p in produtos if isinstance(p, dict)}
            with get_db() as c:
                rows = c.execute("""
                    SELECT * FROM external_products
                    WHERE (status='approved' OR status='active' OR status IS NULL OR status='')
            ...
            for r in rows:
                ...
                try:
                    if "_brz_v20_usados" in globals() and "_brz_v20_link_key" in globals():
                        usados = _brz_v20_usados()
                        lk = _brz_v20_link_key(p)
                        nk = _brz_v20_name_key(p)
                        if (lk and lk in usados.get("links", set())) or (nk and nk in usados.get("names", set())):
                            continue
                except Exception:
                    pass
                ...
                produtos.append(p)
                usados_ids.add(str(p.get("id")))
                if len(produtos) >= limit_final:
                    break
        except Exception as e:
            try: log("WARN", f"[V24] complemento direto do banco falhou: {str(e)[:120]}")
            except Exception: pass

    produtos = (produtos or [])[:limit_final]

    try:
        log("INFO", f"[V24] Seleção final | configurado={qtd} | solicitado={limit_final} | selecionados={len(produtos)}")
    except Exception:
        pass

    return produtos

# preserva ciclo antigo
try:
    _brz_v24_executar_anterior = executar_ciclo
except Exception:
    _brz_v24_executar_anterior = None

def _brz_v24_tentar_envio_produto(p, idx=1, total=1):
    """
    Tenta usar funções existentes de envio. Se nenhuma existir, registra log.
    """
    nome = ""
    try:
        nome = (p.get("name") or p.get("title") or "Produto")[:90]
    except Exception:
        nome = "Produto"

    try: log("INFO", f"[V24] Envio complementar {idx}/{total} | {nome}")
    except Exception: pass

    for fn_name in [
        "postar_externo_whatsapp",
        "postar_produto_externo_whatsapp",
        "enviar_produto_externo_whatsapp",
        "postar_no_whatsapp_externo",
        "send_external_to_whatsapp",
        "postar_produto_whatsapp",
    ]:
        fn = globals().get(fn_name)
        if callable(fn):
            try:
                ok = fn(p)
                try: log("INFO", f"[V24] {idx}/{total} enviado via {fn_name} | resultado={ok}")
                except Exception: pass
                return ok
            except Exception as e:
                try: log("WARN", f"[V24] {fn_name} falhou no {idx}/{total}: {str(e)[:100]}")
                except Exception: pass

    try:
        log("WARN", f"[V24] Nenhuma função direta de envio encontrada para complementar {idx}/{total}.")
    except Exception:
        pass
    return False
...
def executar_ciclo():
    """
    V24: se o ciclo antigo postar só 1, envia complementares até atingir a quantidade.
    """
    qtd = _brz_v24_produtos_por_envio()
    ...
    try:
        with get_db() as c:
            r = c.execute("SELECT id FROM products ORDER BY id DESC LIMIT 1").fetchone()
            antes = int(r["id"]) if r else 0
    except Exception:
        pass

    resultado = 0
    if _brz_v24_executar_anterior:
        try:
            resultado = _brz_v24_executar_anterior()
        except Exception as e:
            try: log("ERROR", f"[V24] ciclo anterior falhou: {str(e)[:140]}")
            except Exception: pass

    enviados = 0
    try:
        with get_db() as c:
            row = c.execute("SELECT COUNT(*) AS n FROM products WHERE id > ?", (antes,)).fetchone()
            enviados = int(row["n"] or 0) if row else 0
    except Exception:
        enviados = int(resultado or 0) if resultado else 0

    faltam = max(0, qtd - enviados)

    try:
        log("INFO", f"[V24] Pós-ciclo | configurado={qtd} | enviados_detectados={enviados} | faltam={faltam}")
    except Exception:
        pass

    if faltam > 0:
        produtos = buscar_external_products_aprovados(limit=faltam)
        produtos = (produtos or [])[:faltam]
        for i, p in enumerate(produtos, start=enviados + 1):
            if i > 1:
                _brz_v24_time.sleep(BRIZZAH_V24_DELAY_SEG)
            ok = _brz_v24_tentar_envio_produto(p, idx=i, total=qtd)
            try:
                if "_brz_v20_registrar_usado" in globals():
                    _brz_v20_registrar_usado(p)
            except Exception:
                pass
            if ok:
                enviados += 1
    ...
    try:
        log("INFO", f"[V24] Ciclo finalizado | configurado={qtd} | enviados_total_estimado={enviados}")
    except Exception:
        pass

    return enviados or resultado

try:
    log("INFO", "[V24] Ativo: correção urgente de 2+ produtos por ciclo e leitura robusta da configuração")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V25 — FIX DEFINITIVO: PAINEL 2 PRODUTOS -> MOTOR 2 PRODUTOS
# Problema encontrado:
# - Painel "Automação WhatsApp" salva quantidade em wa_auto_qtd.
# - Ciclo antigo lia products_per_cycle=1.
# Solução:
# - Priorizar wa_auto_qtd.
# - Normalizar products_per_cycle para o mesmo valor.
# - Complementar envio usando whatsapp_post() real do app.
# ============================================================

import time as _brz_v25_time

BRIZZAH_V25_MAX_PRODUTOS = int(os.environ.get("BRIZZAH_MAX_PRODUTOS_POR_CICLO", "4") or "4")
BRIZZAH_V25_DELAY_SEG = int(os.environ.get("BRIZZAH_DELAY_ENTRE_POSTS", "15") or "15")

def _brz_v25_int(v, default=2):
    try:
        import re as _re
        s = str(v or "").strip()
        m = _re.search(r"\d+", s)
...
def _brz_v25_read_cfg(key, default=""):
    try:
        v = cfg(key, default)
        if str(v).strip():
            return str(v).strip()
...
def _brz_v25_produtos_por_envio():
    """
    Leitura definitiva:
    1) wa_auto_qtd, que é o campo do painel Automação WhatsApp.
    2) whatsapp_products_per_cycle / wa_products_per_cycle.
    3) products_per_cycle apenas se não houver wa_auto_qtd.
    """
    prioridade = [
        "wa_auto_qtd",
        "wa_products_per_cycle",
    ...
    for k in prioridade:
        v = _brz_v25_read_cfg(k, "")
        n = _brz_v25_int(v, 0)
        if 1 <= n <= BRIZZAH_V25_MAX_PRODUTOS:
            raw = v
            origem = k
            qtd = n
            break
    else:
        qtd = _brz_v25_int(os.environ.get("BRIZZAH_PRODUTOS_POR_ENVIO", "2"), 2)
        raw = str(qtd)
        origem = "default_2"

    # Se existe wa_auto_qtd=2, ele SEMPRE vence products_per_cycle=1
    wa_raw = _brz_v25_read_cfg("wa_auto_qtd", "")
    wa_qtd = _brz_v25_int(wa_raw, 0)
    if 1 <= wa_qtd <= BRIZZAH_V25_MAX_PRODUTOS:
        qtd = wa_qtd
        raw = wa_raw
        origem = "wa_auto_qtd"

    qtd = max(1, min(BRIZZAH_V25_MAX_PRODUTOS, int(qtd)))

    # Normaliza TODAS as chaves usadas pelo sistema antigo e novo
    for k in ["wa_auto_qtd", "products_per_cycle", "produtos_por_envio", "wa_products_per_cycle", "whatsapp_products_per_cycle"]:
        try:
            cfg_set(k, str(qtd))
        except Exception:
            try:
                with get_db() as c:
                    c.execute("INSERT OR REPLACE INTO config(key,value) VALUES(?,?)", (k, str(qtd)))
            except Exception:
                pass

    try:
        log("INFO", f"[V25] Produtos por envio FINAL={qtd} | origem={origem} | raw={raw}")
    except Exception:
        pass

    return qtd

# Redefine V24 também, para qualquer chamada antiga cair na lógica nova
_brz_v24_produtos_por_envio = _brz_v25_produtos_por_envio

try:
    _brz_v25_buscar_anterior = buscar_external_products_aprovados
except Exception:
    _brz_v25_buscar_anterior = None

def buscar_external_products_aprovados(limit=30):
    qtd = _brz_v25_produtos_por_envio()
    try:
        lim = int(limit or 0)
    except Exception:
        lim = 0

    limit_final = max(qtd, lim, 1)
    produtos = []

    if _brz_v25_buscar_anterior:
        try:
            produtos = _brz_v25_buscar_anterior(limit=limit_final) or []
        except TypeError:
            try:
                produtos = _brz_v25_buscar_anterior(limit_final) or []
            except Exception:
                produtos = []
        except Exception as e:
            try:
                log("ERROR", f"[V25] Seletor anterior falhou: {str(e)[:120]}")
            except Exception:
                pass

    if len(produtos or []) < limit_final:
        try:
            ids = {str(p.get("id")) for p in produtos if isinstance(p, dict)}
            with get_db() as c:
                rows = c.execute("""
                    SELECT * FROM external_products
                    WHERE (status='approved' OR status='active' OR status IS NULL OR status='')
            ...
            try:
                log("WARN", f"[V25] Complemento banco falhou: {str(e)[:120]}")
            except Exception:
                pass
    ...
    try:
        log("INFO", f"[V25] Seleção final | configurado={qtd} | solicitado={limit_final} | selecionados={len(produtos)}")
    except Exception:
        pass
...
try:
    _brz_v25_executar_anterior = executar_ciclo
except Exception:
    _brz_v25_executar_anterior = None

def _brz_v25_send_produto(p, idx=1, total=1):
    """
    Envio complementar usando a função real whatsapp_post(image_url, caption, instance_id, token, group_id)
    existente no app.
    """
    try:
        nome = (p.get("name") or p.get("title") or "Produto")[:90]
    ...
    try:
        log("INFO", f"[V25] Envio complementar {idx}/{total} iniciando | {nome}")
    except Exception:
        pass

    # Primeiro tenta funções dedicadas, se existirem
    for fn_name in [
        "postar_externo_whatsapp",
        "postar_produto_externo_whatsapp",
        "enviar_produto_externo_whatsapp",
        ...
        if callable(fn):
            try:
                ok = fn(p)
                try:
                    log("INFO", f"[V25] {idx}/{total} enviado via {fn_name} | resultado={ok}")
                except Exception:
                    pass
                return bool(ok if not isinstance(ok, tuple) else ok[0])
            except Exception as e:
                try:
                    log("WARN", f"[V25] {fn_name} falhou no {idx}/{total}: {str(e)[:100]}")
                except Exception:
                    pass

    # Fallback principal: whatsapp_post real
    try:
        caption = _caption_wa_externo(p) if "_caption_wa_externo" in globals() else str(p.get("name") or "Oferta Brizzah")
        img = p.get("image_url") or p.get("image") or p.get("img_url") or ""
        ...
        if callable(globals().get("whatsapp_post")):
            ok, res = whatsapp_post(img, caption, wa_inst, wa_token, wa_group)
            try:
                log("INFO", f"[V25] {idx}/{total} whatsapp_post | ok={ok} | res={str(res)[:80]}")
            except Exception:
                pass
            return bool(ok)
    except Exception as e:
        try:
            log("ERROR", f"[V25] fallback whatsapp_post falhou {idx}/{total}: {str(e)[:140]}")
        except Exception:
            pass
...
def executar_ciclo():
    ...
    resultado = 0
    if _brz_v25_executar_anterior:
        try:
            resultado = _brz_v25_executar_anterior()
        except Exception as e:
            try:
                log("ERROR", f"[V25] ciclo anterior falhou: {str(e)[:140]}")
            except Exception:
                pass
    ...
    try:
        log("INFO", f"[V25] Pós-ciclo | configurado={qtd} | enviados_detectados={enviados} | faltam={faltam}")
    except Exception:
        pass
    ...
    if faltam > 0:
        ...
        for offset, p in enumerate(produtos, start=1):
            idx = enviados + offset
            if idx > 1:
                _brz_v25_time.sleep(BRIZZAH_V25_DELAY_SEG)

            ok = _brz_v25_send_produto(p, idx=idx, total=qtd)

            try:
                if ok and "_brz_v20_registrar_usado" in globals():
                    _brz_v20_registrar_usado(p)
            except Exception:
                pass
            ...
            if ok:
                ...
                try:
                    log("WARN", f"[V25] Complementar {idx}/{qtd} não confirmou envio")
                except Exception:
                    pass

    try:
        log("INFO", f"[V25] Ciclo finalizado | configurado={qtd} | enviados_total_estimado={enviados}")
    except Exception:
        pass
...
try:
    log("INFO", "[V25] Ativo: wa_auto_qtd agora vence products_per_cycle; complemento usa whatsapp_post real")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V26 MVP PREMIUM — CENTRAL MANUAL + BANCO PRO
# Entrega segura:
# - Não altera o scheduler V25.
# - Adiciona rotas isoladas /manual-pro e /ofertas-pro.
# - Permite colar link, gerar prévia, editar legenda e postar no WhatsApp.
# - Permite listar, editar, ativar/desativar e excluir produtos do banco.
# ============================================================

def _brz_v26_safe_float(v, default=0.0):
    try:
...
def _brz_v26_detect_source(url):
    u = (url or "").lower()
    if "mercadolivre" in u or "meli.la" in u:
        return "mercadolivre"
    if "netshoes" in u:
        return "netshoes"
...
def _brz_v26_build_preview(url, source=""):
    source = source or _brz_v26_detect_source(url)
    try:
        dados = extrair_dados_link_externo(url, source)
    except Exception as e:
        try: log("WARN", f"[V26] Falha preview link: {str(e)[:100]}")
        except Exception: pass
        dados = {"name":"Oferta selecionada Brizzah", "price":0, "old_price":0, "image_url":"", "product_url":url}

    p = {
        "id": 0,
        "source": source,
        "name": dados.get("name") or "Oferta selecionada Brizzah",
        "price": _brz_v26_safe_float(dados.get("price"), 0),
        "old_price": _brz_v26_safe_float(dados.get("old_price"), 0),
        "image_url": dados.get("image_url") or "",
        "product_url": dados.get("product_url") or url,
        "affiliate_url": ajustar_link_afiliado_externo(source, url) if "ajustar_link_afiliado_externo" in globals() else url,
        "coupon": dados.get("coupon") or "",
        "installments": dados.get("installments") or "",
        "category": "",
        "brand": "",
        "notes": "",
        "priority": 5,
    ...
    try:
        if "_brz_v19_refresh_produto_externo" in globals():
            p = _brz_v19_refresh_produto_externo(p, force=True)
    except Exception:
        pass

    try:
        caption = _caption_wa_externo(p)
    except Exception:
        caption = f"*Boa oferta para conferir 👊*\n\n{p['name']}\n\n🔗 {p['affiliate_url'] or p['product_url']}\n\n⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.\n_Brizzah | Achados inteligentes_ 🔥"

    p["caption"] = caption
    return p

def _brz_v26_save_external(p):
    with get_db() as c:
        cur = c.execute("""
            INSERT INTO external_products
            (source, name, price, old_price, image_url, product_url, affiliate_url, category, brand, coupon, notes, priority, status, is_active)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'approved', 1)
        """, (
            p.get("source","manual"),
            p.get("name",""),
            _brz_v26_safe_float(p.get("price"),0),
            _brz_v26_safe_float(p.get("old_price"),0),
            p.get("image_url",""),
            p.get("product_url",""),
            p.get("affiliate_url","") or p.get("product_url",""),
            p.get("category",""),
            p.get("brand",""),
            p.get("coupon",""),
            p.get("notes",""),
            int(p.get("priority",5) or 5),
        ))
        return cur.lastrowid

def _brz_v26_post_whatsapp(image_url, caption):
    inst = cfg("whatsapp_instance_id", "brizzah-bot")
    token = cfg("whatsapp_token", "Brizzah@2025!")
    group = cfg("whatsapp_group_id", "120363407236556172@g.us")
    ok, res = whatsapp_post(image_url or "", caption or "", inst, token, group)
    return ok, res

@app.route("/manual-pro", methods=["GET", "POST"])
@login_required
def brizzah_manual_pro():
    preview = None
    msg = ""
    if request.method == "POST":
        action = request.form.get("action", "")
        url = (request.form.get("url") or "").strip()
        source = request.form.get("source", "") or _brz_v26_detect_source(url)

        if action == "preview":
            if not url:
                msg = "Cole um link para gerar a prévia."
            else:
                preview = _brz_v26_build_preview(url, source)
                msg = "Prévia gerada. Revise antes de postar."

        elif action == "post":
            p = {
                "source": source,
                "name": request.form.get("name",""),
                "price": _brz_v26_safe_float(request.form.get("price"),0),
                "old_price": _brz_v26_safe_float(request.form.get("old_price"),0),
                "image_url": request.form.get("image_url",""),
                "product_url": request.form.get("product_url") or url,
                "affiliate_url": request.form.get("affiliate_url") or url,
            ...
            if request.form.get("save_bank") == "1":
                try:
                    _brz_v26_save_external(p)
                    log("INFO", f"[V26] Produto manual salvo no banco: {(p.get('name') or '')[:80]}")
                except Exception as e:
                    log("ERROR", f"[V26] Falha salvar manual banco: {str(e)[:120]}")
            try:
                ok, res = _brz_v26_post_whatsapp(p.get("image_url",""), p.get("caption",""))
                if ok:
                    msg = "✅ Postado no WhatsApp com sucesso."
                    log("INFO", f"[V26] Post manual WhatsApp OK: {(p.get('name') or '')[:80]}")
                else:
                    msg = "⚠️ A API respondeu com possível falha. Confira o WhatsApp e os logs."
                    log("WARN", f"[V26] Post manual WhatsApp retorno: {str(res)[:140]}")
            except Exception as e:
                msg = f"Erro ao postar: {str(e)[:120]}"
                log("ERROR", f"[V26] Erro post manual: {str(e)[:160]}")
            preview = p

    html = """
    <!doctype html><html lang="pt-br"><head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Brizzah Pro | Postagem Manual</title>
    <style>
      body{margin:0;font-family:Arial,Helvetica,sans-serif;background:#08100c;color:#f4fff8}
      .wrap{max-width:1100px;margin:0 auto;padding:24px}
      .top{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px}
      .brand{font-size:24px;font-weight:800;color:#b6ff4d}
      .grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}
      .card{background:#101a14;border:1px solid #223226;border-radius:18px;padding:18px;box-shadow:0 10px 30px rgba(0,0,0,.25)}
      label{font-size:13px;color:#b9c9bd;display:block;margin:12px 0 6px}
      input,textarea,select{width:100%;box-sizing:border-box;border:1px solid #2d4033;border-radius:12px;background:#07110b;color:#fff;padding:12px;font-size:14px}
      textarea{min-height:180px;resize:vertical}
      button{border:0;border-radius:12px;padding:12px 16px;font-weight:800;cursor:pointer}
      .primary{background:#b6ff4d;color:#08100c}
      .secondary{background:#243326;color:#eaffea}
      .danger{background:#ff615c;color:#fff}
      .row{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}
      .msg{background:#17251b;border-left:4px solid #b6ff4d;padding:12px;border-radius:10px;margin:12px 0;color:#eaffea}
      img.preview{max-width:100%;border-radius:14px;background:#000;margin-top:10px}
      a{color:#b6ff4d;text-decoration:none}
      .muted{color:#93a596;font-size:13px}
      @media(max-width:850px){.grid{grid-template-columns:1fr}.wrap{padding:14px}}
    </style></head><body><div class="wrap">
      <div class="top">
        <div><div class="brand">Brizzah Pro</div><div class="muted">Central Manual Premium</div></div>
        <div><a href="/">Dashboard</a> · <a href="/ofertas-pro">Banco de Ofertas Pro</a></div>
      </div>
      {% if msg %}<div class="msg">{{msg}}</div>{% endif %}
      <div class="grid">
        <div class="card">
          <h2>🔗 Colar link e gerar prévia</h2>
          <form method="post">
            <input type="hidden" name="action" value="preview">
            <label>Link do produto</label>
            <input name="url" placeholder="Cole aqui link Mercado Livre, Netshoes, Amazon, Shopee..." value="{{preview.product_url if preview else ''}}">
            <label>Origem</label>
            <select name="source">
            ...
            <div class="row"><button class="primary" type="submit">👀 Gerar prévia / atualizar preço</button></div>
          </form>
          {% if preview and preview.image_url %}
            <img class="preview" src="{{preview.image_url}}">
          {% endif %}
        </div>
        <div class="card">
          <h2>✏️ Revisar e postar</h2>
          <form method="post">
            <input type="hidden" name="action" value="post">
            <label>Nome / título</label>
            <input name="name" value="{{preview.name if preview else ''}}">
            <label>Preço atual</label>
            <input name="price" value="{{preview.price if preview else ''}}">
            <label>Preço anterior</label>
            <input name="old_price" value="{{preview.old_price if preview else ''}}">
            <label>Imagem URL</label>
            <input name="image_url" value="{{preview.image_url if preview else ''}}">
            <label>Link produto</label>
            <input name="product_url" value="{{preview.product_url if preview else ''}}">
            <label>Link afiliado</label>
            <input name="affiliate_url" value="{{preview.affiliate_url if preview else ''}}">
            <label>Categoria</label>
            <input name="category" value="{{preview.category if preview else ''}}" placeholder="tenis, perfume, tech, moda...">
            <label>Marca</label>
            <input name="brand" value="{{preview.brand if preview else ''}}">
            <label>Prioridade</label>
            <input name="priority" value="{{preview.priority if preview else '5'}}">
            <label>Legenda editável</label>
            <textarea name="caption">{{preview.caption if preview else ''}}</textarea>
            <label><input type="checkbox" name="save_bank" value="1" style="width:auto"> Salvar também no banco de ofertas</label>
            <div class="row">
              <button class="primary" type="submit">📲 Postar no WhatsApp</button>
              <a class="secondary" style="padding:12px 16px;border-radius:12px" href="/ofertas-pro">Abrir Banco Pro</a>
            </div>
          </form>
        </div>
      </div>
    ...
    """
    return render_template_string(html, preview=preview, msg=msg)

@app.route("/ofertas-pro")
@login_required
def brizzah_ofertas_pro():
    q = (request.args.get("q") or "").strip()
    with get_db() as c:
        if q:
            rows = c.execute("""
                SELECT * FROM external_products
                WHERE name LIKE ? OR source LIKE ? OR category LIKE ? OR brand LIKE ?
                ORDER BY id DESC LIMIT 120
            """, (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%")).fetchall()
        else:
            rows = c.execute("SELECT * FROM external_products ORDER BY id DESC LIMIT 120").fetchall()
    produtos = [dict(r) for r in rows]
    html = """
    <!doctype html><html lang="pt-br"><head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Brizzah Pro | Banco de Ofertas</title>
    <style>
      body{margin:0;font-family:Arial,Helvetica,sans-serif;background:#08100c;color:#f4fff8}
      .wrap{max-width:1250px;margin:0 auto;padding:22px}
      .brand{font-size:24px;font-weight:900;color:#b6ff4d}
      a{color:#b6ff4d;text-decoration:none}
      .card{background:#101a14;border:1px solid #223226;border-radius:18px;padding:16px;margin:14px 0}
      input,select{border:1px solid #2d4033;border-radius:10px;background:#07110b;color:#fff;padding:10px}
      button{border:0;border-radius:10px;padding:10px 12px;font-weight:800;cursor:pointer}
      .primary{background:#b6ff4d;color:#08100c}.danger{background:#ff615c;color:#fff}.soft{background:#243326;color:#fff}
      table{width:100%;border-collapse:collapse;margin-top:12px;font-size:13px}
      th,td{border-bottom:1px solid #223226;padding:8px;text-align:left;vertical-align:top}
      img{width:58px;height:58px;object-fit:cover;border-radius:10px;background:#000}
      .top{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap}
      .muted{color:#9db0a1}
      @media(max-width:900px){table{font-size:12px}.hide-mobile{display:none}}
    </style></head><body><div class="wrap">
      <div class="top"><div><div class="brand">Banco de Ofertas Pro</div><div class="muted">Editar, ativar/desativar, excluir e postar manualmente.</div></div>
      <div><a href="/">Dashboard</a> · <a href="/manual-pro">Central Manual</a></div></div>
      <div class="card">
        <form method="get">
          <input name="q" placeholder="Buscar produto, origem, categoria..." value="{{q}}" style="width:70%">
          <button class="primary" type="submit">Buscar</button>
        </form>
      </div>
      <div class="card">
        <table>
          <tr><th>Img</th><th>Produto</th><th>Preço</th><th>Origem</th><th>Status</th><th>Ações</th></tr>
          {% for p in produtos %}
          <tr>
            <td>{% if p.image_url %}<img src="{{p.image_url}}">{% endif %}</td>
            <td><b>{{p.name}}</b><br><span class="muted">ID {{p.id}} · {{p.category or ''}} · {{p.brand or ''}}</span></td>
            <td>Atual: {{p.price}}<br>Antigo: {{p.old_price}}</td>
            <td>{{p.source}}<br><span class="muted hide-mobile">{{(p.affiliate_url or p.product_url or '')[:55]}}</span></td>
            <td>{{'ativo' if p.is_active else 'inativo'}}<br>{{p.status}}</td>
            <td>
              <a class="soft" href="/ofertas-pro/{{p.id}}/edit" style="padding:9px 10px;border-radius:10px;display:inline-block">Editar</a>
              <form method="post" action="/ofertas-pro/{{p.id}}/toggle" style="display:inline"><button class="soft" type="submit">{{'Desativar' if p.is_active else 'Ativar'}}</button></form>
              <form method="post" action="/ofertas-pro/{{p.id}}/post" style="display:inline"><button class="primary" type="submit">Postar</button></form>
              <form method="post" action="/ofertas-pro/{{p.id}}/delete" style="display:inline" onsubmit="return confirm('Excluir este produto?')"><button class="danger" type="submit">Excluir</button></form>
            </td>
          </tr>
          {% endfor %}
        </table>
    ...
    """
    return render_template_string(html, produtos=produtos, q=q)

@app.route("/ofertas-pro/<int:pid>/edit", methods=["GET","POST"])
@login_required
def brizzah_oferta_edit(pid):
    msg = ""
    if request.method == "POST":
        data = {
            "name": request.form.get("name",""),
        ...
        with get_db() as c:
            c.execute("""
                UPDATE external_products
                SET source=?, name=?, price=?, old_price=?, image_url=?, product_url=?, affiliate_url=?, category=?, brand=?, coupon=?, notes=?, priority=?
                WHERE id=?
            """, (data["source"], data["name"], data["price"], data["old_price"], data["image_url"], data["product_url"], data["affiliate_url"], data["category"], data["brand"], data["coupon"], data["notes"], data["priority"], pid))
        log("INFO", f"[V26] Oferta editada ID={pid}")
        return redirect(url_for("brizzah_ofertas_pro"))

    with get_db() as c:
        r = c.execute("SELECT * FROM external_products WHERE id=?", (pid,)).fetchone()
    if not r:
        return "Produto não encontrado", 404
    p = dict(r)
    html = """
    <!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Editar Oferta</title><style>
    body{font-family:Arial;background:#08100c;color:#fff;margin:0}.wrap{max-width:850px;margin:0 auto;padding:22px}
    input,textarea{width:100%;box-sizing:border-box;margin:5px 0 12px;padding:11px;border-radius:10px;border:1px solid #2d4033;background:#07110b;color:#fff}
    textarea{min-height:110px}button{background:#b6ff4d;color:#08100c;border:0;border-radius:10px;padding:12px 16px;font-weight:900}a{color:#b6ff4d}
    </style></head><body><div class="wrap"><h1>Editar Oferta #{{p.id}}</h1><a href="/ofertas-pro">← voltar</a>
    <form method="post">
      {% for campo in ['source','name','price','old_price','image_url','product_url','affiliate_url','category','brand','coupon','priority'] %}
        <label>{{campo}}</label><input name="{{campo}}" value="{{p[campo] if p[campo] is not none else ''}}">
      {% endfor %}
      <label>notes / descrição interna</label><textarea name="notes">{{p.notes or ''}}</textarea>
      <button type="submit">Salvar alterações</button>
    </form></div></body></html>
    """
    return render_template_string(html, p=p)

@app.route("/ofertas-pro/<int:pid>/toggle", methods=["POST"])
@login_required
def brizzah_oferta_toggle(pid):
    with get_db() as c:
        r = c.execute("SELECT is_active FROM external_products WHERE id=?", (pid,)).fetchone()
        novo = 0 if (r and int(r["is_active"] or 0)==1) else 1
        c.execute("UPDATE external_products SET is_active=? WHERE id=?", (novo, pid))
    log("INFO", f"[V26] Oferta toggle ID={pid} ativo={novo}")
    return redirect(url_for("brizzah_ofertas_pro"))

@app.route("/ofertas-pro/<int:pid>/delete", methods=["POST"])
@login_required
def brizzah_oferta_delete(pid):
    with get_db() as c:
        c.execute("DELETE FROM external_products WHERE id=?", (pid,))
    log("INFO", f"[V26] Oferta excluída ID={pid}")
    return redirect(url_for("brizzah_ofertas_pro"))

@app.route("/ofertas-pro/<int:pid>/post", methods=["POST"])
@login_required
def brizzah_oferta_post(pid):
    with get_db() as c:
        r = c.execute("SELECT * FROM external_products WHERE id=?", (pid,)).fetchone()
    if not r:
        return "Produto não encontrado", 404
    ...
    try:
        ...
        pass
    caption = _caption_wa_externo(p)
    ok, res = _brz_v26_post_whatsapp(p.get("image_url",""), caption)
    log("INFO" if ok else "WARN", f"[V26] Post Banco Pro ID={pid} ok={ok} res={str(res)[:100]}")
    return redirect(url_for("brizzah_ofertas_pro"))

try:
    log("INFO", "[V26] MVP Premium ativo: /manual-pro e /ofertas-pro")
except Exception:
    pass

# ============================================================
# FIM BRIZZAH V26 MVP PREMIUM
# ============================================================


# ============================================================
# BRIZZAH V27 — CORREÇÃO PREÇO FAKE R$ 9,90 NO MANUAL PRO
# - Não exibe preço quando Mercado Livre/Amazon/Netshoes retornam fallback baixo.
# - Mantém imagem, título, link e legenda profissional.
# - Evita salvar preço falso no Banco de Ofertas Pro.
# ============================================================

def _brz_v27_preco_confiavel(preco, source="", nome=""):
    """Valida preço capturado por scraping. Evita fallback falso como R$ 9,90."""
    try:
        v = _safe_float(preco, 0)
    except Exception:
        v = 0
    ...
    # O erro observado no manual-pro: Mercado Livre social/links externos caem em R$ 9,90.
    # Para fontes externas, valores muito baixos geralmente são frete, parcela, taxa ou fallback.
    if s in ("mercadolivre", "amazon", "netshoes"):
        if v <= 10.0:
            return False
        if abs(v - 9.90) < 0.02:
            return False

    # Para Shopee, ainda permitimos preços baixos, mas bloqueamos exatamente o fallback clássico.
    if abs(v - 9.90) < 0.02 and s != "shopee":
        return False

    return True


def _brz_v27_sanitizar_preco_produto(p):
    """Remove preço/antigo não confiável antes de montar preview, legenda ou salvar."""
    try:
        if not isinstance(p, dict):
            return p
        p = dict(p)
        source = p.get("source") or _brz_v26_detect_source(p.get("product_url") or p.get("affiliate_url") or "") if "_brz_v26_detect_source" in globals() else ""
        preco = _safe_float(p.get("price"), 0)
        antigo = _safe_float(p.get("old_price"), 0)

        if not _brz_v27_preco_confiavel(preco, source, p.get("name")):
            p["price"] = 0
            p["old_price"] = 0
            p["_price_hidden_reason"] = "preco_nao_confirmado"
            try:
                log("WARN", f"[V27] Preço ocultado por baixa confiança | origem={source} | preço={preco} | nome={(p.get('name') or '')[:70]}")
            except Exception:
                pass
            return p

        # Nunca mostrar preço antigo estranho ou menor/igual ao atual.
        if not antigo or antigo <= preco:
            p["old_price"] = 0
        return p
    except Exception:
        return p
...
# Guarda versões anteriores para aproveitar toda a inteligência de legenda já existente.
try:
    _brz_v27_caption_wa_anterior = _caption_wa_externo
except Exception:
    _brz_v27_caption_wa_anterior = None
try:
    _brz_v27_caption_ig_anterior = _caption_ig_externo
except Exception:
    _brz_v27_caption_ig_anterior = None


def _caption_wa_externo(p):
    p = _brz_v27_sanitizar_preco_produto(p)
    if _brz_v27_caption_wa_anterior:
        txt = _brz_v27_caption_wa_anterior(p)
    else:
        emoji, fonte = _fonte_emoji_externo(p.get("source")) if "_fonte_emoji_externo" in globals() else ("🛍️", "Produto")
        nome = p.get("name") or "Oferta selecionada Brizzah"
        link = p.get("affiliate_url") or p.get("product_url") or ""
        txt = f"*ACHADO BRIZZAH DO DIA*\n\n{nome}\n\n💰 Confira o valor atualizado no link 👇\n\n{emoji} Origem: *{fonte}*\n🔗 {link}\n\n⚠️ Os preços podem mudar a qualquer momento sem aviso prévio.\n_Brizzah | Achados inteligentes_ 🔥"

    # Segurança extra: se algum trecho antigo ainda gerar R$ 9,90, troca por frase confiável.
    if isinstance(p, dict) and p.get("_price_hidden_reason"):
        import re as _re
        linhas = []
        remove_next_discount = False
        for linha in str(txt).splitlines():
            l = linha.strip().lower()
            if "r$ 9,90" in l or "r$9,90" in l or l.startswith("por *r$ 9,90") or l.startswith("preço de hoje: r$ 9,90"):
                linhas.append("💰 Confira o valor atualizado no link 👇")
                remove_next_discount = True
                continue
            if remove_next_discount and ("já com desconto" in l or "% off" in l):
                continue
            linhas.append(linha)
        txt = "\n".join(linhas)
    return txt
...
def _caption_ig_externo(p):
    p = _brz_v27_sanitizar_preco_produto(p)
    if _brz_v27_caption_ig_anterior:
        txt = _brz_v27_caption_ig_anterior(p)
    else:
        nome = p.get("name") or "Oferta selecionada Brizzah"
        link = p.get("affiliate_url") or p.get("product_url") or ""
        txt = f"🛍️ ACHADO BRIZZAH DO DIA\n\n{nome}\n\nConfira o preço atualizado no link 👇\n{link}\n\n#achadinhos #ofertas #brizzah #promocao"
    if isinstance(p, dict) and p.get("_price_hidden_reason"):
        txt = str(txt).replace("Preço de hoje: R$ 9,90", "Confira o preço atualizado no link 👇")
        txt = txt.replace("por R$ 9,90", "confira o preço atualizado no link")
    return txt


# Recria o preview V26 com sanitização V27 antes de aparecer na tela.
def _brz_v26_build_preview(url, source=""):
    source = source or _brz_v26_detect_source(url)
    try:
        dados = extrair_dados_link_externo(url, source)
    except Exception as e:
        try: log("WARN", f"[V27] Falha preview link: {str(e)[:100]}")
        except Exception: pass
        dados = {"name":"Oferta selecionada Brizzah", "price":0, "old_price":0, "image_url":"", "product_url":url}
...
# Evita gravar preço falso no banco quando salvar a oferta manual.
def _brz_v26_save_external(p):
    p = _brz_v27_sanitizar_preco_produto(p)
    with get_db() as c:
        cur = c.execute("""
...
try:
    log("INFO", "[V27] Ativo: bloqueia preço fake R$ 9,90 no Manual Pro e Banco Pro")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V26.1 HOTFIX — PREÇO ML + RETORNO WHATSAPP
# Objetivo:
# - Melhorar extração de preço em links Mercado Livre/meli.la.
# - Não exibir 0.0 como preço confirmado.
# - Tratar HTTP 200/201/PENDING da Evolution como envio aceito.
# - Manter V25 e V26 preservadas.
# ============================================================

def _brz_v261_extract_price_ml_html(html):
    import re as _re
    html = html or ""
    candidatos = []

    # JSON/estado interno do Mercado Livre e metatags comuns
    patterns = [
        r'"price"\s*:\s*\{\s*"amount"\s*:\s*([0-9]+(?:\.[0-9]+)?)',
        r'"current_price"\s*:\s*\{\s*"value"\s*:\s*([0-9]+(?:\.[0-9]+)?)',
    ...
    # Evita parcelas pequenas demais: pega valor mais recorrente/menor plausível acima de 5.
    candidatos = sorted(set(round(x, 2) for x in candidatos))
    return candidatos[0]

def _brz_v261_extract_old_price_ml_html(html, price=0):
    import re as _re
    html = html or ""
    candidatos = []
    patterns = [
        r'"original_price"\s*:\s*([0-9]+(?:\.[0-9]+)?)',
        r'"regular_amount"\s*:\s*([0-9]+(?:\.[0-9]+)?)',
        r'"wasPrice"\s*:\s*"?([0-9]+(?:[\.,][0-9]{1,2})?)"?',
...
try:
    _brz_v261_extrair_original = extrair_dados_link_externo
except Exception:
    _brz_v261_extrair_original = None

def extrair_dados_link_externo(url, source=""):
    """
    V26.1: wrapper do importador externo com reforço específico para ML/meli.la.
    """
    source = (source or "").lower().strip() or _brz_v26_detect_source(url) if "_brz_v26_detect_source" in globals() else (source or "").lower().strip()
    dados = {}
    if _brz_v261_extrair_original:
        try:
            dados = _brz_v261_extrair_original(url, source) or {}
        except Exception as e:
            try: log("WARN", f"[V26.1] Importador original falhou: {str(e)[:100]}")
            except Exception: pass
            dados = {}

    if not dados:
        dados = {"name":"Oferta selecionada Brizzah","price":0,"old_price":0,"image_url":"","product_url":url,"coupon":"","installments":""}

    # Reforço Mercado Livre
    try:
        u = (url or dados.get("product_url") or "").lower()
        if "mercadolivre" in u or "meli.la" in u or source == "mercadolivre":
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124 Safari/537.36",
                "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8",
                "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
            ...
            r = requests.get(url, headers=headers, timeout=18, allow_redirects=True)
            html = r.text or ""
            final_url = r.url or dados.get("product_url") or url
            if final_url:
                dados["product_url"] = final_url

            price = _brz_v261_extract_price_ml_html(html)
            if price:
                dados["price"] = price
                oldp = _brz_v261_extract_old_price_ml_html(html, price)
                dados["old_price"] = oldp if oldp and oldp > price else 0

            # Nome/imagem por meta se o original falhou
            import re as _re
            def _meta(prop):
                pats = [
                    r'<meta[^>]+property=["\']' + _re.escape(prop) + r'["\'][^>]+content=["\']([^"\']+)',
            ...
            if not dados.get("image_url"):
                img = _meta("og:image") or _meta("twitter:image")
                if img:
                    dados["image_url"] = _fix_img(img)

            if not dados.get("name") or _nome_generico_externo(dados.get("name")):
                title = _meta("og:title") or _meta("twitter:title")
                if title:
                    dados["name"] = _limpar_nome_produto_ext(title, source)

            # Cupom: manter só se explícito no HTML. Não inventar.
            cupom = ""
            for pat in [r'Cupom\s*[:\-]?\s*([A-Z0-9]{4,20})', r'cupom\s+([A-Z0-9]{4,20})']:
                m = _re.search(pat, html, flags=_re.I|_re.S)
                if m:
                    cupom = m.group(1).upper()
                    break
            dados["coupon"] = cupom or dados.get("coupon","")

    except Exception as e:
        try: log("WARN", f"[V26.1] Reforço ML não confirmou preço: {str(e)[:100]}")
        except Exception: pass

    return dados

def _brz_v261_caption_manual(p):
    """
    Legenda manual sem preço 0.0. Se preço não confirmado, orienta consultar no link.
    """
    try:
        p = dict(p or {})
        preco = _safe_float(p.get("price"), 0)
        oldp = _safe_float(p.get("old_price"), 0)
        p["old_price"] = _brz_preco_antigo_confiavel(preco, oldp, p.get("source")) if "_brz_preco_antigo_confiavel" in globals() else oldp
        if preco:
            return _caption_wa_externo(p)
        emoji, fonte = _fonte_emoji_externo(p.get("source"))
        nome = _nome_profissional_produto(_nome_apresentavel_externo(p))[:115]
        ...
        return str(p.get("caption") or "")

try:
    _brz_v261_build_preview_original = _brz_v26_build_preview
except Exception:
    _brz_v261_build_preview_original = None

def _brz_v26_build_preview(url, source=""):
    """
    V26.1: prévia com preço reforçado e sem 0.0 como preço confirmado.
    """
    source = source or _brz_v26_detect_source(url)
    dados = extrair_dados_link_externo(url, source)
    p = {
    ...
    # Só tenta refresh se o link já tiver preço ou se for fonte que responde melhor.
    try:
        if p.get("price") and "_brz_v19_refresh_produto_externo" in globals():
            p = _brz_v19_refresh_produto_externo(p, force=True)
    except Exception:
        pass

    p["price"] = _safe_float(p.get("price"), 0)
    p["old_price"] = _safe_float(p.get("old_price"), 0)
    p["caption"] = _brz_v261_caption_manual(p)
    return p

try:
    _brz_v261_post_original = _brz_v26_post_whatsapp
except Exception:
    _brz_v261_post_original = None

def _brz_v26_post_whatsapp(image_url, caption):
    """
    V26.1: considera HTTP 200/201 e PENDING como aceito pela Evolution.
    """
    inst = cfg("whatsapp_instance_id", "brizzah-bot")
    token = cfg("whatsapp_token", "Brizzah@2025!")
    group = cfg("whatsapp_group_id", "120363407236556172@g.us")
    ok, res = whatsapp_post(image_url or "", caption or "", inst, token, group)
    txt = str(res)
    if ok or "HTTP 200" in txt or "HTTP 201" in txt or "PENDING" in txt or "'status': 'PENDING'" in txt or '"status":"PENDING"' in txt:
        return True, res
    return bool(ok), res

try:
    log("INFO", "[V26.1] Hotfix ativo: preço ML reforçado + WhatsApp HTTP 201/PENDING tratado como OK")
except Exception:
    pass

# ============================================================
# FIM BRIZZAH V26.1 HOTFIX
# ============================================================



# ============================================================
# BRIZZAH V27 — DASHBOARD PREMIUM + QUALIDADE DO FEED
# Entrega segura:
# - Não altera o motor automático V25/V26.
# - Cria /dashboard-pro com visual premium e atalhos operacionais.
# - Cria /qualidade-feed com diagnóstico do banco de ofertas.
# - Cria /logs-pro com leitura mais limpa dos logs.
# - Corrige interpretação visual de HTTP 201/PENDING como aceito.
# ============================================================

def _brz_v27_count(sql, params=()):
    try:
        with get_db() as c:
...
def _brz_v27_cfg_status():
    try:
        return {
            "auto": cfg("auto_enabled", "false"),
            "wa_auto": cfg("wa_auto_ativo", "false"),
            "qtd": cfg("wa_auto_qtd", cfg("products_per_cycle", "2")),
            "last": cfg("last_auto_run", ""),
            "schedule": cfg("auto_schedule", "")[:120],
            "warmup": os.environ.get("BRIZZAH_VITRINE_WARMUP", "false"),
            "scheduler_disabled": os.environ.get("BRIZZAH_DISABLE_SCHEDULER", "false"),
            "workers": os.environ.get("WEB_CONCURRENCY", "1"),
        }
    except Exception:
        return {}

def _brz_v27_quality_data():
    total = _brz_v27_count("SELECT COUNT(*) FROM external_products")
    ativos = _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=1")
    inativos = _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=0")
    sem_preco = _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=1 AND (price IS NULL OR price<=0)")
    sem_img = _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=1 AND (image_url IS NULL OR image_url='')")
    sem_cat = _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=1 AND (category IS NULL OR category='')")
    repetidos = 0
    try:
        with get_db() as c:
            r = c.execute("""
                SELECT COUNT(*) FROM (
                    SELECT affiliate_url, COUNT(*) n FROM external_products
                    WHERE affiliate_url IS NOT NULL AND affiliate_url <> ''
                    GROUP BY affiliate_url HAVING n > 1
                )
            """).fetchone()
            repetidos = int((r[0] if r else 0) or 0)
    except Exception:
        pass
    return {
        "total": total, "ativos": ativos, "inativos": inativos,
        "sem_preco": sem_preco, "sem_img": sem_img, "sem_cat": sem_cat,
        "repetidos": repetidos
    }

def _brz_v27_recent_posts(limit=8):
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT id,name,price,source,last_posted,image_url,affiliate_url,product_url,category
                FROM external_products
                ORDER BY COALESCE(last_posted,'') DESC, id DESC
                LIMIT ?
            """, (limit,)).fetchall()
            return [dict(r) for r in rows]
    except Exception:
...
def _brz_v27_recent_logs(limit=25):
    out = []
    try:
        with get_db() as c:
            rows = c.execute("SELECT level,message,created_at FROM logs ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        for r in rows:
            d = dict(r)
            msg = str(d.get("message") or "")
            level = str(d.get("level") or "INFO")
            visual = level
            if "HTTP 201" in msg or "PENDING" in msg or "status': 'PENDING" in msg:
                visual = "OK"
            if "failed" in msg.lower() and ("HTTP 201" in msg or "PENDING" in msg):
                visual = "OK"
            d["visual_level"] = visual
            out.append(d)
    except Exception:
        pass
    return out
...
@login_required
def brizzah_dashboard_pro():
    stats = {
        "products_today": _brz_v27_count("SELECT COUNT(*) FROM products WHERE date(posted_at)=date('now')"),
        "external_total": _brz_v27_count("SELECT COUNT(*) FROM external_products"),
        "external_active": _brz_v27_count("SELECT COUNT(*) FROM external_products WHERE is_active=1"),
        "clicks": _brz_v27_count("SELECT COUNT(*) FROM external_clicks"),
        "logs_warn": _brz_v27_count("SELECT COUNT(*) FROM logs WHERE level IN ('WARN','ERROR')"),
    }
    cfgs = _brz_v27_cfg_status()
    quality = _brz_v27_quality_data()
    recent = _brz_v27_recent_posts(8)
    logs = _brz_v27_recent_logs(12)
    html = """
    <!doctype html><html lang="pt-br"><head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Brizzah Pro Dashboard</title>
    <style>
      :root{--bg:#07110b;--card:#101b14;--card2:#15251a;--line:#29402f;--green:#b6ff4d;--text:#f4fff6;--muted:#9bad9f;--red:#ff5d57;--blue:#5da8ff}
      *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top left,#16381f,#07110b 38%,#050805);color:var(--text);font-family:Arial,Helvetica,sans-serif}
      .wrap{max-width:1180px;margin:0 auto;padding:22px}
      .hero{display:grid;grid-template-columns:1.15fr .85fr;gap:18px;margin-bottom:18px}
      .card{background:linear-gradient(160deg,var(--card),#0b140e);border:1px solid var(--line);border-radius:22px;padding:18px;box-shadow:0 18px 44px rgba(0,0,0,.35)}
      .brand{font-size:30px;font-weight:900;color:var(--green);letter-spacing:-1px}.sub{color:var(--muted);margin-top:4px}
      .pill{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:999px;background:#203323;color:#dbffe0;font-weight:800;font-size:13px;margin:5px 5px 0 0}
      .grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:14px 0}.metric{background:var(--card2);padding:16px;border-radius:18px;border:1px solid var(--line)}
      .metric b{font-size:25px;display:block;color:#fff}.metric span{color:var(--muted);font-size:13px}
      .actions{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-top:14px}
      a.btn{display:block;text-decoration:none;background:var(--green);color:#08100c;padding:14px 16px;border-radius:16px;font-weight:900;text-align:center}
      a.btn.dark{background:#203323;color:#fff}a.btn.blue{background:var(--blue);color:#06101f}a.btn.red{background:#ff714d;color:#fff}
      .section{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:18px}
      table{width:100%;border-collapse:collapse;font-size:13px}td,th{padding:9px;border-bottom:1px solid #26382b;text-align:left;vertical-align:top}
      .ok{color:var(--green);font-weight:900}.warn{color:#ffd15c;font-weight:900}.err{color:var(--red);font-weight:900}.muted{color:var(--muted)}
      img.thumb{width:54px;height:54px;object-fit:cover;border-radius:12px;background:#000}.top{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}
      @media(max-width:900px){.hero,.section{grid-template-columns:1fr}.grid{grid-template-columns:repeat(2,1fr)}.actions{grid-template-columns:1fr}.wrap{padding:14px}.brand{font-size:26px}}
    </style></head><body><div class="wrap">
      <div class="hero">
        <div class="card">
          <div class="top">
            <div><div class="brand">⚡ Brizzah Pro</div><div class="sub">Central premium de operação, curadoria e vendas</div></div>
            <div><a class="btn dark" href="/">Dashboard antigo</a></div>
          </div>
          <div style="margin-top:12px">
            <span class="pill">WhatsApp Auto: {{'ON' if cfgs.auto=='true' or cfgs.wa_auto=='true' else 'OFF'}}</span>
            <span class="pill">{{cfgs.qtd}} produto(s) por ciclo</span>
            <span class="pill">Workers: {{cfgs.workers}}</span>
            <span class="pill">Warmup: {{cfgs.warmup}}</span>
          </div>
          <div class="grid">
            <div class="metric"><b>{{stats.products_today}}</b><span>posts hoje</span></div>
            <div class="metric"><b>{{stats.external_active}}</b><span>ofertas ativas</span></div>
            <div class="metric"><b>{{stats.external_total}}</b><span>ofertas no banco</span></div>
            <div class="metric"><b>{{stats.clicks}}</b><span>cliques externos</span></div>
          </div>
          <div class="actions">
            <a class="btn" href="/manual-pro">🔗 Central Manual Premium</a>
            <a class="btn blue" href="/ofertas-pro">⭐ Banco de Ofertas Pro</a>
            <a class="btn dark" href="/qualidade-feed">🧠 Qualidade do Feed</a>
            <a class="btn red" href="/logs-pro">📋 Logs Pro</a>
          </div>
        </div>
        <div class="card">
          <h2>🧠 Qualidade rápida</h2>
          <table>
            <tr><td>Sem preço confirmado</td><td><b class="{{'warn' if quality.sem_preco else 'ok'}}">{{quality.sem_preco}}</b></td></tr>
            <tr><td>Sem imagem</td><td><b class="{{'warn' if quality.sem_img else 'ok'}}">{{quality.sem_img}}</b></td></tr>
            <tr><td>Sem categoria</td><td><b class="{{'warn' if quality.sem_cat else 'ok'}}">{{quality.sem_cat}}</b></td></tr>
            <tr><td>Links duplicados</td><td><b class="{{'warn' if quality.repetidos else 'ok'}}">{{quality.repetidos}}</b></td></tr>
            <tr><td>Inativos</td><td><b>{{quality.inativos}}</b></td></tr>
          </table>
          <p class="muted">Prioridade: corrigir preço, imagem e categoria antes de escalar tráfego.</p>
        </div>
      </div>
      <div class="section">
        <div class="card">
          <h2>🛍️ Últimas ofertas do banco</h2>
          <table>
            {% for p in recent %}
            <tr>
              <td>{% if p.image_url %}<img class="thumb" src="{{p.image_url}}">{% endif %}</td>
              <td><b>{{(p.name or '')[:70]}}</b><br><span class="muted">{{p.source}} · {{p.category or 'sem categoria'}} · R$ {{p.price}}</span></td>
              <td><a href="/ofertas-pro/{{p.id}}/edit">Editar</a></td>
            </tr>
            {% endfor %}
          </table>
        </div>
        <div class="card">
          ...
          <table>
            {% for l in logs %}
            <tr>
              <td><span class="{{'ok' if l.visual_level=='OK' else ('err' if l.visual_level=='ERROR' else ('warn' if l.visual_level=='WARN' else 'muted'))}}">{{l.visual_level}}</span></td>
              <td>{{(l.message or '')[:105]}}<br><span class="muted">{{l.created_at}}</span></td>
            </tr>
            {% endfor %}
          </table>
        </div>
    ...
    """
    return render_template_string(html, stats=stats, cfgs=cfgs, quality=quality, recent=recent, logs=logs)

@app.route("/qualidade-feed")
@login_required
def brizzah_qualidade_feed():
    quality = _brz_v27_quality_data()
    try:
        with get_db() as c:
            sem_preco = [dict(r) for r in c.execute("SELECT id,name,source,price,image_url FROM external_products WHERE is_active=1 AND (price IS NULL OR price<=0) ORDER BY id DESC LIMIT 40").fetchall()]
            sem_img = [dict(r) for r in c.execute("SELECT id,name,source,price,image_url FROM external_products WHERE is_active=1 AND (image_url IS NULL OR image_url='') ORDER BY id DESC LIMIT 40").fetchall()]
            sem_cat = [dict(r) for r in c.execute("SELECT id,name,source,price,image_url FROM external_products WHERE is_active=1 AND (category IS NULL OR category='') ORDER BY id DESC LIMIT 40").fetchall()]
    except Exception:
        sem_preco, sem_img, sem_cat = [], [], []
    html = """
    <!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Qualidade do Feed</title>
    <style>body{font-family:Arial;background:#07110b;color:#f4fff6;margin:0}.wrap{max-width:1100px;margin:auto;padding:20px}a{color:#b6ff4d;text-decoration:none}.card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:16px;margin:14px 0}table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #29402f;padding:9px;text-align:left}.brand{font-size:28px;font-weight:900;color:#b6ff4d}.metric{display:inline-block;background:#17251b;padding:12px 16px;border-radius:16px;margin:6px}</style></head>
    <body><div class="wrap"><div class="brand">🧠 Qualidade do Feed</div><p><a href="/dashboard-pro">← Dashboard Pro</a> · <a href="/ofertas-pro">Banco Pro</a></p>
    <div class="card">
      <span class="metric">Sem preço: <b>{{quality.sem_preco}}</b></span>
      <span class="metric">Sem imagem: <b>{{quality.sem_img}}</b></span>
      <span class="metric">Sem categoria: <b>{{quality.sem_cat}}</b></span>
      <span class="metric">Duplicados: <b>{{quality.repetidos}}</b></span>
    </div>
    {% for titulo, lista in [('Sem preço confirmado',sem_preco),('Sem imagem',sem_img),('Sem categoria',sem_cat)] %}
    <div class="card"><h2>{{titulo}}</h2><table>
      {% for p in lista %}
      <tr><td>#{{p.id}}</td><td>{{p.name}}</td><td>{{p.source}}</td><td>R$ {{p.price}}</td><td><a href="/ofertas-pro/{{p.id}}/edit">corrigir</a></td></tr>
      {% endfor %}
    </table></div>
    {% endfor %}
    </div></body></html>
    """
    return render_template_string(html, quality=quality, sem_preco=sem_preco, sem_img=sem_img, sem_cat=sem_cat)

@app.route("/logs-pro")
@login_required
def brizzah_logs_pro():
    logs = _brz_v27_recent_logs(120)
    html = """
    <!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Logs Pro</title>
    <style>body{font-family:Arial;background:#07110b;color:#f4fff6;margin:0}.wrap{max-width:1100px;margin:auto;padding:20px}a{color:#b6ff4d}.card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:16px;margin:14px 0}table{width:100%;border-collapse:collapse;font-size:13px}td,th{border-bottom:1px solid #29402f;padding:8px;text-align:left;vertical-align:top}.brand{font-size:28px;font-weight:900;color:#b6ff4d}.ok{color:#b6ff4d;font-weight:900}.warn{color:#ffd15c;font-weight:900}.err{color:#ff5d57;font-weight:900}.muted{color:#9bad9f}</style></head>
    <body><div class="wrap"><div class="brand">📋 Logs Pro</div><p><a href="/dashboard-pro">← Dashboard Pro</a></p><div class="card"><table>
      {% for l in logs %}
      <tr><td>{{l.created_at}}</td><td><span class="{{'ok' if l.visual_level=='OK' else ('err' if l.visual_level=='ERROR' else ('warn' if l.visual_level=='WARN' else 'muted'))}}">{{l.visual_level}}</span></td><td>{{l.message}}</td></tr>
      {% endfor %}
...
try:
    log("INFO", "[V27] Ativo: Dashboard Premium + Qualidade do Feed + Logs Pro")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V27.1 — HOTFIX LINKS CURTOS MELI.LA
# Corrige links colados com vírgula, parêntese, markdown, espaços,
# quebra de linha ou caracteres extras no final.
# Exemplo:
# https://meli.la/15Uxk8X,
# https://meli.la/15Uxk8X)
# ============================================================

def _brz_v271_clean_any_link(raw):
    import re as _re
    import urllib.parse as _up
    ...
    # Remove markdown comum: [texto](url)
    m = _re.search(r"\((https?://[^)]+)\)", s)
    if m:
        s = m.group(1).strip()

    # Se veio texto junto, pega a primeira URL.
    m = _re.search(r"https?://[^\s<>\]\)]+", s)
    if m:
        s = m.group(0).strip()

    # Remove pontuações acidentais no fim.
    s = s.strip(" \t\r\n,.;)]}>\"'")

    # Corrige quando o usuário cola sem protocolo.
    if s.startswith("meli.la/"):
        s = "https://" + s
    if s.startswith("www.mercadolivre"):
        s = "https://" + s

    # Normaliza meli.la preservando caminho e removendo fragmento ruim.
    try:
        p = _up.urlparse(s)
        host = (p.netloc or "").lower()
        if "meli.la" in host:
            s = _up.urlunparse((p.scheme or "https", p.netloc, p.path, "", p.query, ""))
    except Exception:
        pass
...
try:
    _brz_v271_limpar_url_original = _limpar_url_compartilhamento
except Exception:
    _brz_v271_limpar_url_original = None

def _limpar_url_compartilhamento(url):
    """
    V27.1: limpeza reforçada antes da regra antiga.
    """
    url2 = _brz_v271_clean_any_link(url)
    if _brz_v271_limpar_url_original:
        try:
            return _brz_v271_limpar_url_original(url2)
        except Exception:
            return url2
    return url2

try:
    _brz_v271_detect_original = _brz_v26_detect_source
except Exception:
    _brz_v271_detect_original = None

def _brz_v26_detect_source(url):
    """
    V27.1: detecta Mercado Livre mesmo em links curtos meli.la.
    """
    u = _brz_v271_clean_any_link(url).lower()
    ...
    if "shopee" in u:
        return "shopee"
    if _brz_v271_detect_original:
        try:
            return _brz_v271_detect_original(u)
        except Exception:
            pass
    return "manual"

try:
    _brz_v271_build_preview_original = _brz_v26_build_preview
except Exception:
    _brz_v271_build_preview_original = None

def _brz_v26_build_preview(url, source=""):
    """
    V27.1: prévia sempre recebe link limpo.
    """
    url_limpo = _brz_v271_clean_any_link(url)
    source = source or _brz_v26_detect_source(url_limpo)
    if _brz_v271_build_preview_original:
        return _brz_v271_build_preview_original(url_limpo, source)
    return {
        "name": "Oferta Brizzah",
        "product_url": url_limpo,
        "affiliate_url": url_limpo,
        "source": source,
        "price": 0,
        "old_price": 0,
        "image_url": "",
        "caption": f"Confira no link: {url_limpo}",
    }

try:
    log("INFO", "[V27.1] Ativo: links meli.la aceitos mesmo com vírgula/parêntese/markdown")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V27.2 — HOTFIX IMPORTADOR EM MASSA MELI.LA
# Corrige /external/paste_bulk:
# - aceita meli.la curto;
# - limpa vírgula/parêntese/markdown;
# - detecta fonte automaticamente;
# - salva no banco mesmo se preço não confirmar;
# - mostra detalhes reais do que foi ignorado.
# ============================================================

def _brz_v272_source_from_link(link, selected=""):
    selected = (selected or "").strip().lower()
    if selected and selected not in ("auto", "detectar", "detect"):
        return selected
    try:
        return _brz_v26_detect_source(link)
    except Exception:
        u = str(link or "").lower()
        if "meli.la" in u or "mercadolivre" in u:
            return "mercadolivre"
...
def _brz_v272_nome_importado(dados, source, link):
    try:
        nome = dados.get("name") or ""
        if not nome or _nome_generico_externo(nome):
            if source == "mercadolivre":
                nome = "Oferta Mercado Livre"
            elif source == "netshoes":
                nome = "Oferta Netshoes"
            elif source == "amazon":
                nome = "Oferta Amazon"
            elif source == "shopee":
                nome = "Oferta Shopee"
                ...
                nome = "Oferta Brizzah"
        return _corrigir_portugues_produto(_limpar_nome_produto_ext(nome, source))[:140]
    except Exception:
        return "Oferta Brizzah"

def _brz_v272_import_one_external(raw, selected_source="", priority=8):
    """
    Importa 1 link externo com tolerância máxima.
    Retorna: (ok:bool, status:str, detalhe:str)
    """
    original = str(raw or "").strip()
    link = _brz_v271_clean_any_link(original) if "_brz_v271_clean_any_link" in globals() else _limpar_url_compartilhamento(original)
    link = _limpar_url_compartilhamento(link)

    if not link:
        return False, "ignorado", "linha vazia"

    if not (link.startswith("http://") or link.startswith("https://")):
        return False, "erro", f"link inválido após limpeza: {link}"

    source = _brz_v272_source_from_link(link, selected_source)

    try:
        dados = extrair_dados_link_externo(link, source) or {}
    except Exception as e:
        try: log("WARN", f"[V27.2] Enriquecimento falhou, salvando fallback: {str(e)[:90]}")
        except Exception: pass
        dados = {}

    product_url = _limpar_url_compartilhamento(dados.get("product_url") or link)
    affiliate_url = ajustar_link_afiliado_externo(source, link) if "ajustar_link_afiliado_externo" in globals() else link
    affiliate_url = _limpar_url_compartilhamento(affiliate_url)

    nome = _brz_v272_nome_importado(dados, source, link)
    price = _safe_float(dados.get("price"), 0)
    old_price = _safe_float(dados.get("old_price"), 0)
    image_url = dados.get("image_url") or ""
    coupon = dados.get("coupon") or ""

    try:
        with get_db() as c:
            # duplicidade mais tolerante: compara link original limpo, destino e afiliado.
            exists = c.execute("""
                SELECT id FROM external_products
                WHERE product_url IN (?, ?, ?)
                   OR affiliate_url IN (?, ?, ?)
                LIMIT 1
            """, (link, product_url, affiliate_url, link, product_url, affiliate_url)).fetchone()

            if exists:
                return False, "duplicado", f"já existe no banco ID {exists['id']}"
            ...
            """, (
                ...
                image_url,
                product_url or link,
                affiliate_url or link,
                coupon,
                int(priority or 8)
            ))

        try:
            log("INFO", f"[V27.2] Link importado em massa OK | source={source} | nome={nome[:70]}")
        except Exception:
            pass

        detalhe = f"{source} | {nome}"
        if not price:
            detalhe += " | preço não confirmado"
        return True, "adicionado", detalhe

    except Exception as e:
        try: log("ERROR", f"[V27.2] Falha ao salvar link em massa: {str(e)[:140]}")
        except Exception: pass
        return False, "erro", str(e)[:140]

def external_paste_bulk_v272():
    if request.method == "POST":
        selected_source = (request.form.get("source") or "").strip().lower()
        try:
            priority = int(request.form.get("priority") or 8)
        except Exception:
            priority = 8
        ...
        raw_links = request.form.get("links", "") or ""
        # Aceita 1 por linha, mas também separa quando vier com espaços.
        import re as _re
        links = []
        for line in raw_links.splitlines():
            ...
            if not line:
                continue
            # Se vier texto com várias URLs na mesma linha, captura todas.
            found = _re.findall(r"https?://[^\s<>\]\)]+", line)
            if found:
                links.extend(found)
            else:
                links.append(line)
        ...
        for i, raw in enumerate(links, start=1):
            ok, status, detalhe = _brz_v272_import_one_external(raw, selected_source, priority)
            if ok:
                added += 1
                details.append(f"✅ Linha {i}: {detalhe}")
            else:
                skipped += 1
                details.append(f"⚠️ Linha {i}: {status} — {detalhe}")

        detalhe_html = ""
        if details:
            detalhe_html = "<h3>Detalhes</h3><ul>" + "".join(f"<li>{d}</li>" for d in details[:80]) + "</ul>"
        ...
        <style>
          body{{font-family:Arial;background:#07110b;color:#f4fff6;padding:22px}}
          a{{color:#b6ff4d}} .card{{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:18px;max-width:850px}}
          .ok{{color:#b6ff4d;font-weight:900}} .warn{{color:#ffd15c;font-weight:900}}
        </style></head><body><div class='card'>
        <h1>Importação concluída</h1>
        <p><b class='ok'>{added}</b> link(s) adicionados.</p>
        <p><b class='warn'>{skipped}</b> ignorado(s), duplicado(s) ou com erro.</p>
        {detalhe_html}
        <p><a href='/external'>Ver Produtos Externos</a></p>
        <p><a href='/external/paste_bulk'>Colar mais links</a></p>
        <p><a href='/ofertas-pro'>Abrir Banco Pro</a></p>
        </div></body></html>
        """
    ...
    <!doctype html><html lang='pt-br'><head>
    <meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>
    <title>Colar links em massa</title>
    <style>
      body{font-family:Arial;background:#07110b;color:#f4fff6;padding:22px}
      a{color:#b6ff4d}.card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:18px;max-width:850px}
      textarea,select,input{width:100%;box-sizing:border-box;padding:12px;border-radius:12px;border:1px solid #29402f;background:#06100a;color:#fff;margin:8px 0 14px}
      button{background:#b6ff4d;color:#06100a;border:0;border-radius:12px;padding:13px 18px;font-weight:900}
      .muted{color:#9bad9f}
    </style></head><body><div class='card'>
    <h1>Colar links em massa</h1>
    <p class='muted'>Aceita links Mercado Livre curtos tipo <b>https://meli.la/15Uxk8X</b>, mesmo com vírgula ou parêntese no fim.</p>
    <form method='post'>
      Fonte:
      <select name='source'>
      ...
      Prioridade:
      <input name='priority' value='8'>
      Cole os links afiliados ou links normais:
      <textarea name='links' rows='14' placeholder='1 link por linha'></textarea>
      <button>Salvar no banco</button>
    </form>
    <p><a href='/dashboard-pro'>Dashboard Pro</a> · <a href='/ofertas-pro'>Banco Pro</a></p>
    </div></body></html>
    """

try:
    app.view_functions["external_paste_bulk"] = external_paste_bulk_v272
    log("INFO", "[V27.2] Ativo: importador em massa aceita meli.la e salva fallback sem preço")
except Exception as e:
    try: log("ERROR", f"[V27.2] Falha ao ativar override paste_bulk: {str(e)[:120]}")
    except Exception: pass
...
# ============================================================
# BRIZZAH V28 — MERCADO LIVRE RESOLVER INTELIGENTE
# Objetivo:
# - Resolver links curtos meli.la antes de importar.
# - Evitar salvar produto quebrado com nome tipo "2EaTvha".
# - Evitar preço falso como R$ 9,90 quando o preço não foi confirmado.
# - Salvar fallback seguro sem preço apenas quando tiver nome/imagem aceitáveis.
# - Melhorar importação em massa e prévia manual.
# ============================================================

def _brz_v28_is_meli_url(url):
    u = str(url or "").lower()
    return "meli.la" in u or "mercadolivre" in u or "mercadolibre" in u

def _brz_v28_slug_ruim(nome):
    import re as _re
    n = str(nome or "").strip()
    if not n:
        return True
    if len(n) <= 10 and _re.fullmatch(r"[A-Za-z0-9]+", n):
        return True
    if n.lower() in {"produto importado", "oferta mercado livre", "mercado livre", "oferta brizzah"}:
        return True
    return False

def _brz_v28_resolve_url(url):
    """
    Resolve URL final de meli.la/mercadolivre com HEAD+GET leve.
    """
    url = _brz_v271_clean_any_link(url) if "_brz_v271_clean_any_link" in globals() else _limpar_url_compartilhamento(url)
    headers = {
        "User-Agent": "Mozilla/5.0 (Linux; Android 12; Mobile) AppleWebKit/537.36 Chrome/124 Safari/537.36",
        "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
        "Cache-Control": "no-cache",
    }
    ...
    try:
        ...
        try: log("WARN", f"[V28] Resolve URL falhou: {str(e)[:90]}")
        except Exception: pass
    return final_url, html

def _brz_v28_meta_from_html(html, prop):
    import re as _re
    pats = [
        r'<meta[^>]+property=["\']' + _re.escape(prop) + r'["\'][^>]+content=["\']([^"\']+)',
        r'<meta[^>]+name=["\']' + _re.escape(prop) + r'["\'][^>]+content=["\']([^"\']+)',
    ...
    for pat in pats:
        m = _re.search(pat, html or "", flags=_re.I | _re.S)
        if m:
            return _html_unescape(m.group(1))
    return ""

def _brz_v28_title_from_html(html):
    import re as _re
    title = (
        _brz_v28_meta_from_html(html, "og:title")
        or _brz_v28_meta_from_html(html, "twitter:title")
        or ""
    )
    if not title:
        m = _re.search(r"<title[^>]*>(.*?)</title>", html or "", flags=_re.I | _re.S)
        if m:
            title = _html_unescape(_re.sub(r"\s+", " ", m.group(1)).strip())
    title = _limpar_nome_produto_ext(title, "mercadolivre")
    # Remove frases comuns
    title = title.replace(" | Mercado Livre", "").replace(" - Mercado Livre", "").strip()
    return title[:140]

def _brz_v28_image_from_html(html):
    img = (
        _brz_v28_meta_from_html(html, "og:image")
        or _brz_v28_meta_from_html(html, "twitter:image")
        or _brz_v28_meta_from_html(html, "image")
        or ""
    )
    return _fix_img(img)

def _brz_v28_price_from_html(html):
    """
    Extrator mais conservador: só aceita preço se houver sinais fortes.
    """
    import re as _re, json as _json
    html = html or ""
    candidatos = []
    ...
    try:
        for prod in _extract_jsonld_products(html):
            ...
            if isinstance(offers, dict):
                ...
                price = _safe_float(val, 0)
                if 5 <= price <= 50000:
                    candidatos.append(("jsonld", price))
    except Exception:
        pass

    patterns = [
        ("meta_product", r'property=["\']product:price:amount["\'][^>]+content=["\']([0-9]+(?:[\.,][0-9]{1,2})?)'),
        ("itemprop", r'itemprop=["\']price["\'][^>]+content=["\']([0-9]+(?:[\.,][0-9]{1,2})?)'),
        ("price_amount", r'"priceAmount"\s*:\s*"?([0-9]+(?:[\.,][0-9]{1,2})?)"?'),
        ("andes_fraction", r'andes-money-amount__fraction[^>]*>\s*([0-9\.]{1,8})\s*<'),
        ("api_price", r'"price"\s*:\s*([0-9]{2,6}(?:\.[0-9]{1,2})?)'),
        ("amount", r'"amount"\s*:\s*([0-9]{2,6}(?:\.[0-9]{1,2})?)'),
    ]
    for source, pat in patterns:
        for m in _re.finditer(pat, html, flags=_re.I | _re.S):
            price = _safe_float(m.group(1), 0)
            if 5 <= price <= 50000:
                candidatos.append((source, price))

    # Remove preço fake muito comum quando falha.
    vals = [round(v, 2) for _, v in candidatos if v and round(v, 2) not in (9.90, 0.0)]
    if not vals:
        return 0

    # Preferir menor preço plausível, mas não parcela minúscula se houver preço maior recorrente.
    vals_sorted = sorted(vals)
    # Se tem valores acima de 30, descarta valores muito baixos que podem ser parcela/frete.
    if any(v >= 30 for v in vals_sorted):
        vals_sorted = [v for v in vals_sorted if v >= 20]
    return vals_sorted[0] if vals_sorted else 0

def _brz_v28_old_price_from_html(html, price):
    import re as _re
    if not price:
        return 0
    candidatos = []
    pats = [
        r'"original_price"\s*:\s*([0-9]{2,6}(?:\.[0-9]{1,2})?)',
        r'"regular_amount"\s*:\s*([0-9]{2,6}(?:\.[0-9]{1,2})?)',
        r'"listPrice"\s*:\s*"?([0-9]{2,6}(?:[\.,][0-9]{1,2})?)"?',
        r'de\s*R\$\s*([0-9]{1,3}(?:\.[0-9]{3})*,[0-9]{2})',
    ]
    for pat in pats:
        for m in _re.finditer(pat, html or "", flags=_re.I | _re.S):
            val = _safe_float(m.group(1), 0)
            if val > price and val <= 50000:
                candidatos.append(val)
    return max(candidatos) if candidatos else 0

try:
    _brz_v28_extrair_original = extrair_dados_link_externo
except Exception:
    _brz_v28_extrair_original = None

def extrair_dados_link_externo(url, source=""):
    """
    V28: resolve Mercado Livre/meli.la antes e valida qualidade.
    """
    source = (source or "").lower().strip()
    clean = _brz_v271_clean_any_link(url) if "_brz_v271_clean_any_link" in globals() else _limpar_url_compartilhamento(url)
    if not source:
        source = _brz_v26_detect_source(clean) if "_brz_v26_detect_source" in globals() else ""

    if source == "mercadolivre" or _brz_v28_is_meli_url(clean):
        final_url, html = _brz_v28_resolve_url(clean)
        title = _brz_v28_title_from_html(html)
        img = _brz_v28_image_from_html(html)
        price = _brz_v28_price_from_html(html)
        oldp = _brz_v28_old_price_from_html(html, price)

        # Fallback slug da URL final se título veio ruim.
        _, slug = _url_destino_e_nome_slug_externo(final_url or clean)
        name = title if not _brz_v28_slug_ruim(title) else slug
        if _brz_v28_slug_ruim(name):
            name = "Oferta Mercado Livre"

        dados = {
            "name": _corrigir_portugues_produto(name),
            "price": price if price and price != 9.90 else 0,
            "old_price": oldp if oldp and oldp > price else 0,
            "image_url": img,
            "product_url": final_url or clean,
        ...
        try:
            log("INFO", f"[V28] ML resolvido | nome={dados['name'][:55]} | preco={dados['price']} | url_final={(final_url or clean)[:80]}")
        except Exception:
            pass

        return dados

    # Demais origens seguem importador anterior.
    if _brz_v28_extrair_original:
        return _brz_v28_extrair_original(clean, source)
    return {"name":"Oferta Brizzah","price":0,"old_price":0,"image_url":"","product_url":clean,"coupon":"","installments":""}

def _brz_v28_should_block_broken_ml(dados, source):
    """
    Bloqueia somente quando o produto está realmente quebrado:
    sem nome bom e sem imagem. Se tiver imagem/nome, permite salvar sem preço.
    """
    if source != "mercadolivre":
        return False, ""
    nome = dados.get("name") or ""
    img = dados.get("image_url") or ""
    price = _safe_float(dados.get("price"), 0)

    if price == 9.90:
        dados["price"] = 0

    if _brz_v28_slug_ruim(nome) and not img:
        return True, "Mercado Livre não retornou título/imagem suficientes. Use a Central Manual para revisar."
    return False, ""

try:
    _brz_v28_import_one_original = _brz_v272_import_one_external
except Exception:
    _brz_v28_import_one_original = None

def _brz_v272_import_one_external(raw, selected_source="", priority=8):
    """
    V28: importação em massa com resolução ML e trava anti-produto quebrado.
    """
    original = str(raw or "").strip()
    link = _brz_v271_clean_any_link(original) if "_brz_v271_clean_any_link" in globals() else _limpar_url_compartilhamento(original)
    link = _limpar_url_compartilhamento(link)
    ...
    source = _brz_v272_source_from_link(link, selected_source) if "_brz_v272_source_from_link" in globals() else _brz_v26_detect_source(link)

    try:
        dados = extrair_dados_link_externo(link, source) or {}
    except Exception as e:
        return False, "erro", f"não consegui ler o link: {str(e)[:90]}"

    block, reason = _brz_v28_should_block_broken_ml(dados, source)
    if block:
        return False, "qualidade", reason

    product_url = _limpar_url_compartilhamento(dados.get("product_url") or link)
    affiliate_url = ajustar_link_afiliado_externo(source, link) if "ajustar_link_afiliado_externo" in globals() else link
    affiliate_url = _limpar_url_compartilhamento(affiliate_url)

    nome = _brz_v272_nome_importado(dados, source, link) if "_brz_v272_nome_importado" in globals() else _corrigir_portugues_produto(dados.get("name") or "Oferta Brizzah")
    price = _safe_float(dados.get("price"), 0)
    if price == 9.90 and source == "mercadolivre":
        price = 0
    old_price = _safe_float(dados.get("old_price"), 0)
    image_url = dados.get("image_url") or ""
    coupon = dados.get("coupon") or ""
    ...
    try:
        with get_db() as c:
            ...
            c.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,'approved',1)
            """, (
                source, nome, price, old_price, image_url,
                product_url or link, affiliate_url or link, coupon, int(priority or 8)
            ))
...
try:
    _brz_v28_build_preview_old = _brz_v26_build_preview
except Exception:
    _brz_v28_build_preview_old = None

def _brz_v26_build_preview(url, source=""):
    """
    V28: prévia manual Mercado Livre com resolução final e sem preço fake.
    """
    clean = _brz_v271_clean_any_link(url) if "_brz_v271_clean_any_link" in globals() else _limpar_url_compartilhamento(url)
    source = source or (_brz_v26_detect_source(clean) if "_brz_v26_detect_source" in globals() else "")
    dados = extrair_dados_link_externo(clean, source)
    ...
    p = {
        ...
        "old_price": _safe_float(dados.get("old_price"), 0),
        "image_url": dados.get("image_url") or "",
        "product_url": dados.get("product_url") or clean,
        "affiliate_url": ajustar_link_afiliado_externo(source, clean) if "ajustar_link_afiliado_externo" in globals() else clean,
        "coupon": dados.get("coupon") or "",
        "installments": dados.get("installments") or "",
        "category": "",
        "brand": "",
    ...
    if p["price"] == 9.90 and source == "mercadolivre":
        p["price"] = 0
    p["caption"] = _brz_v261_caption_manual(p) if "_brz_v261_caption_manual" in globals() else _caption_wa_externo(p)
    return p

try:
    log("INFO", "[V28] Ativo: Mercado Livre Resolver inteligente, anti-preço fake e anti-produto quebrado")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V29 — ENRIQUECEDOR EM LOTE MERCADO LIVRE
# Painel: /ml-enriquecer
# ============================================================

def _brz_v29_is_ml(p):
    txt = " ".join([str(p.get("source") or ""), str(p.get("product_url") or ""), str(p.get("affiliate_url") or "")]).lower()
    return "mercadolivre" in txt or "meli.la" in txt

def _brz_v29_need(p):
    if not _brz_v29_is_ml(p):
        return False
    price = _safe_float(p.get("price"), 0)
    img = str(p.get("image_url") or "").strip()
    name = str(p.get("name") or "").strip()
    if not price or price == 9.90:
        return True
    if not img:
        return True
    try:
        if "_brz_v28_slug_ruim" in globals() and _brz_v28_slug_ruim(name):
            return True
    except Exception:
        if len(name) <= 10:
            return True
...
def _brz_v29_enrich_one(p):
    pid = p.get("id")
    link = p.get("affiliate_url") or p.get("product_url") or ""
    if not link:
        return {"id": pid, "ok": False, "status": "sem_link", "msg": "sem link"}

    try:
        dados = extrair_dados_link_externo(link, "mercadolivre") or {}
        name_new = _corrigir_portugues_produto(dados.get("name") or p.get("name") or "")
        price_new = _safe_float(dados.get("price"), 0)
        old_new = _safe_float(dados.get("old_price"), 0)
        img_new = dados.get("image_url") or p.get("image_url") or ""
        product_url = dados.get("product_url") or p.get("product_url") or link

        if price_new == 9.90:
            price_new = 0
        if old_new and price_new and old_new <= price_new:
            old_new = 0

        name_final = name_new if name_new and len(name_new) > 10 else (p.get("name") or "")
        price_final = price_new if price_new else _safe_float(p.get("price"), 0)
        old_final = old_new if old_new else _safe_float(p.get("old_price"), 0)
        img_final = img_new if img_new else (p.get("image_url") or "")

        with get_db() as c:
            c.execute("""
                UPDATE external_products
                SET name=?, price=?, old_price=?, image_url=?, product_url=?, source='mercadolivre'
                WHERE id=?
            """, (name_final, price_final, old_final, img_final, product_url, pid))

        status = "atualizado_com_preco" if price_final else "atualizado_sem_preco"
        try:
            log("INFO", f"[V29] ML enriquecido ID={pid} | {status} | preço={price_final}")
        except Exception:
            pass
        return {"id": pid, "ok": True, "status": status, "msg": f"{name_final[:80]} | preço={price_final or 'não confirmado'}"}

    except Exception as e:
        try:
            log("WARN", f"[V29] Falha enriquecer ID={pid}: {str(e)[:100]}")
        except Exception:
            pass
        return {"id": pid, "ok": False, "status": "erro", "msg": str(e)[:120]}

def _brz_v29_enrich_batch(limit=20, only_pending=True):
    ...
    try:
        with get_db() as c:
            rows = c.execute("""
                ...
                WHERE is_active=1 AND (
                    LOWER(source)='mercadolivre'
                    OR LOWER(product_url) LIKE '%meli.la%'
                    OR LOWER(product_url) LIKE '%mercadolivre%'
                    OR LOWER(affiliate_url) LIKE '%meli.la%'
                    OR LOWER(affiliate_url) LIKE '%mercadolivre%'
                )
                ORDER BY CASE WHEN price IS NULL OR price<=0 OR price=9.9 THEN 0 ELSE 1 END, id DESC
                LIMIT ?
            """, (limit * 4,)).fetchall()
        ...
        for r in rows:
            p = dict(r)
            if (not only_pending) or _brz_v29_need(p):
                candidates.append(p)
            if len(candidates) >= limit:
                break

        for p in candidates:
            results.append(_brz_v29_enrich_one(p))
            time.sleep(1)
    except Exception as e:
        results.append({"id": 0, "ok": False, "status": "erro_geral", "msg": str(e)[:160]})
    return results

@app.route("/ml-enriquecer", methods=["GET", "POST"])
@login_required
def brizzah_ml_enriquecer():
    results = []
    msg = ""
    if request.method == "POST":
        limit = _safe_int(request.form.get("limit"), 20)
        only = request.form.get("somente_pendentes", "1") == "1"
        results = _brz_v29_enrich_batch(limit=limit, only_pending=only)
        msg = f"Processamento concluído: {len(results)} produto(s)."

    total_ml = sem_preco = 0
    try:
        with get_db() as c:
            total_ml = c.execute("""
                SELECT COUNT(*) n FROM external_products
                WHERE LOWER(source)='mercadolivre' OR LOWER(product_url) LIKE '%meli.la%' OR LOWER(affiliate_url) LIKE '%meli.la%'
            """).fetchone()["n"]
            sem_preco = c.execute("""
                SELECT COUNT(*) n FROM external_products
                WHERE is_active=1 AND (
                    LOWER(source)='mercadolivre' OR LOWER(product_url) LIKE '%meli.la%' OR LOWER(affiliate_url) LIKE '%meli.la%'
                ) AND (price IS NULL OR price<=0 OR price=9.9)
            """).fetchone()["n"]
    except Exception:
        pass
    ...
    html = """
    <!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Brizzah Pro | Enriquecer ML</title>
    <style>
      body{font-family:Arial;background:#07110b;color:#f4fff6;margin:0}.wrap{max-width:1050px;margin:auto;padding:22px}
      .brand{font-size:28px;font-weight:900;color:#b6ff4d}.card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:18px;margin:14px 0}
      a{color:#b6ff4d;text-decoration:none}input{padding:12px;border-radius:12px;border:1px solid #29402f;background:#06100a;color:#fff}
      button{background:#b6ff4d;color:#06100a;border:0;border-radius:12px;padding:13px 18px;font-weight:900}
      table{width:100%;border-collapse:collapse;margin-top:12px;font-size:13px}td,th{border-bottom:1px solid #29402f;padding:9px;text-align:left}
      .ok{color:#b6ff4d;font-weight:900}.warn{color:#ffd15c;font-weight:900}.muted{color:#9bad9f}.metric{display:inline-block;background:#17251b;border:1px solid #29402f;border-radius:16px;padding:12px 16px;margin:6px}
    </style></head><body><div class="wrap">
      <div class="brand">🟡 Enriquecedor Mercado Livre</div>
      <p><a href="/dashboard-pro">Dashboard Pro</a> · <a href="/ofertas-pro">Banco Pro</a> · <a href="/qualidade-feed">Qualidade do Feed</a></p>
      <div class="card"><span class="metric">Total ML: <b>{{total_ml}}</b></span><span class="metric">Sem preço: <b>{{sem_preco}}</b></span></div>
      <div class="card"><h2>Atualizar preço/título/imagem em lote</h2><p class="muted">Use lotes pequenos no Render Free. Recomendo 10 a 20 por vez.</p>
      <form method="post"><label>Quantidade neste lote</label><br><input name="limit" value="20" type="number" min="1" max="50"><br><br>
      <label><input type="checkbox" name="somente_pendentes" value="1" checked> Apenas produtos sem preço/imagem/título ruim</label><br><br>
      <button type="submit">🔄 Enriquecer agora</button></form></div>
      {% if msg %}<div class="card"><b>{{msg}}</b></div>{% endif %}
      {% if results %}<div class="card"><h2>Resultado</h2><table><tr><th>ID</th><th>Status</th><th>Detalhe</th></tr>
      {% for r in results %}<tr><td>{{r.id}}</td><td><span class="{{'ok' if r.ok else 'warn'}}">{{r.status}}</span></td><td>{{r.msg}}</td></tr>{% endfor %}
      </table></div>{% endif %}
    </div></body></html>
    """
    return render_template_string(html, results=results, msg=msg, total_ml=total_ml, sem_preco=sem_preco)

try:
    log("INFO", "[V29] Ativo: enriquecedor em lote Mercado Livre em /ml-enriquecer")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V30 — ENGINE INTELIGENTE DE LEGENDA E QUALIDADE
# - Corrige legenda contextual errada.
# - Produto de festa/decoracao nao vira pet.
# - Anti-vazamento de template antigo.
# - Penaliza produtos quebrados no score.
# ============================================================

def _brz_v30_text_norm(txt):
    try:
        import unicodedata, re
        s = str(txt or "").lower()
        s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
...
def _brz_v30_detect_category(p):
    nome = _brz_v30_text_norm((p or {}).get("name") or "")
    cat = _brz_v30_text_norm((p or {}).get("category") or "")
    txt = f"{cat} {nome}"

    if any(k in txt for k in ["pet", "cachorro", "gato", "racao", "coleira", "caminha pet"]):
        return "pet"
    if any(k in txt for k in ["balao", "aniversario", "festa", "cortina", "faixa decoracao", "decoracao", "mesa posta", "enfeite", "buffet", "cafe", "cozinha", "casa", "organizador", "tapete", "cadeira", "jardim"]):
        return "casa_decoracao"
    if any(k in txt for k in ["licor", "vinho", "whisky", "vodka", "cerveja", "amarula", "gin"]):
        return "bebida"
    if any(k in txt for k in ["tenis", "chinelo", "sandalia", "sapato", "bota", "calcado", "calcados"]):
        return "calcados"
    if any(k in txt for k in ["camiseta", "camisa", "bermuda", "short", "calca", "cueca", "meia", "moletom", "vestido", "blusa", "moda", "legging"]):
        return "moda"
    if any(k in txt for k in ["notebook", "fone", "bluetooth", "smartwatch", "relogio inteligente", "camera", "carregador", "mouse", "teclado", "jbl", "xiaomi", "tech"]):
        return "tech"
    if any(k in txt for k in ["perfume", "creme", "shampoo", "skincare", "maquiagem", "beleza", "colageno"]):
        return "beleza"
    if any(k in txt for k in ["whey", "creatina", "fitness", "academia", "treino", "suplemento"]):
        return "fitness"
    if any(k in txt for k in ["brinquedo", "infantil", "crianca", "bebe", "escolar"]):
        return "infantil"
    return "geral"

def _brz_v30_headline(p):
    cat = _brz_v30_detect_category(p)
    nome = _brz_v30_text_norm((p or {}).get("name") or "")

    if cat == "pet":
        return "🐶 Oferta boa para quem cuida do pet"
    if cat == "casa_decoracao":
        if any(k in nome for k in ["balao", "aniversario", "festa"]):
            return "✨ Pra deixar a festa mais bonita gastando menos"
        return "✨ Achado lindo para casa e decoração"
    if cat == "bebida":
        return "🥃 Oferta boa para aproveitar hoje"
    if cat == "calcados":
        return "👟 Garimpo Brizzah para calçar bem pagando menos"
    if cat == "moda":
        return "👕 Garimpo Brizzah para renovar o visual"
    if cat == "tech":
        return "⚡ Achado tech com preço interessante"
    if cat == "beleza":
        return "💄 Achado de beleza para aproveitar"
    if cat == "fitness":
        return "💪 Oferta boa para o projeto fitness"
    if cat == "infantil":
        return "🎁 Achado legal para os pequenos"
    return "🔥 Boa oferta para aproveitar hoje"

def _brz_v30_quality_ok(p):
    nome = str((p or {}).get("name") or "").strip()
    img = str((p or {}).get("image_url") or "").strip()
    price = _safe_float((p or {}).get("price"), 0)
    source = str((p or {}).get("source") or "").lower()

    if not nome or len(nome) < 8:
        return False, "nome muito curto"
    if source == "mercadolivre" and price == 9.90:
        return False, "preço fake 9,90"
    if not img:
        return False, "sem imagem"
    return True, "ok"

try:
    _brz_v30_caption_wa_old = _caption_wa_externo
except Exception:
    _brz_v30_caption_wa_old = None
...
def _caption_wa_externo(p):
    try:
        ...
        headline = _brz_v30_headline(p)
        linhas = [f"*{headline}*", "", nome, ""]

        if preco and preco != 9.90:
            if orig and orig > preco:
                linhas.append(f"de {_format_brl(orig)} por *{_format_brl(preco)}* 👊")
                if desc >= 10:
                    linhas.append(f"🔥 {desc}% OFF")
        ...
        if _brz_v30_caption_wa_old:
            return _brz_v30_caption_wa_old(p)
        return "🔥 Oferta Brizzah\n\nConfira no link."

try:
    _brz_v30_caption_ig_old = _caption_ig_externo
except Exception:
    _brz_v30_caption_ig_old = None
...
def _caption_ig_externo(p):
    try:
        ...
        linhas = [f"{emoji} {_brz_v30_headline(p)}", "", nome, ""]
        if preco and preco != 9.90:
            linhas.append(f"De {_format_brl(orig)} por {_format_brl(preco)} 🔥 {desc}% OFF" if desc >= 10 else f"Preço de hoje: {_format_brl(preco)}")
        else:
            linhas.append("Confira o preço atualizado no link 👇")
        ...
        if _brz_v30_caption_ig_old:
            return _brz_v30_caption_ig_old(p)
        return "Oferta Brizzah"

try:
    _brz_v30_score_old = score_externo
except Exception:
    _brz_v30_score_old = None

def score_externo(p):
    try:
        base = _brz_v30_score_old(p) if _brz_v30_score_old else 0
        ok, motivo = _brz_v30_quality_ok(p)
        if not ok:
            base -= 20
        cat = _brz_v30_detect_category(p)
        if cat in ("moda", "calcados", "tech", "beleza", "casa_decoracao"):
            base += 2
        return base
    except Exception:
        return 0
...
try:
    log("INFO", "[V30] Ativo: legenda contextual, anti-vazamento e qualidade de feed")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V30.1 — HOTFIX ANTI-CUPOM FAKE
# Remove cupons falsos como SPAN/APP/OFF/PROMO capturados por HTML.
# Rota extra: /limpar-cupons-fake
# ============================================================

def _brz_v301_coupon_valido(cupom):
    import re as _re
    c = str(cupom or "").strip().upper()
    ...
    fake = {
        "SPAN", "DIV", "HTML", "BODY", "SCRIPT", "STYLE", "CLASS", "BUTTON",
        "APP", "OFF", "PROMO", "PROMOCAO", "DESCONTO", "CUPOM", "LINK",
        "MERCADO", "LIVRE", "MELI", "ML", "BRIZZAH", "OFERTA", "PRICE",
        "PRECO", "COMPRAR", "ANUNCIO", "FRETE", "FULL", "SUPER"
    }
    ...
    if c.isalpha() and len(c) <= 6:
        return ""
    if c.isalpha() and not any(x in c for x in ["BR", "VIP", "MAIS", "OFF"]):
        return ""

    return c

try:
    _brz_v301_caption_wa_old = _caption_wa_externo
except Exception:
    _brz_v301_caption_wa_old = None

def _caption_wa_externo(p):
    try:
        p = dict(p or {})
        p["coupon"] = _brz_v301_coupon_valido(p.get("coupon"))
        ...
        link = p.get("affiliate_url") or p.get("product_url") or ""
        installments = (p.get("installments") or "").strip()
        headline = _brz_v30_headline(p) if "_brz_v30_headline" in globals() else _headline_promo_externo(p)
        ...
        if p.get("coupon"):
            linhas.append(f"Cupom: *{p.get('coupon')}* ⚠️")
        ...
        if _brz_v301_caption_wa_old:
            try:
                p2 = dict(p or {})
                p2["coupon"] = _brz_v301_coupon_valido(p2.get("coupon"))
                return _brz_v301_caption_wa_old(p2)
            except Exception:
                return _brz_v301_caption_wa_old(p)
        return "🔥 Oferta Brizzah\n\nConfira no link."

try:
    _brz_v301_caption_ig_old = _caption_ig_externo
except Exception:
    _brz_v301_caption_ig_old = None

def _caption_ig_externo(p):
    try:
        p = dict(p or {})
        p["coupon"] = _brz_v301_coupon_valido(p.get("coupon"))
        if _brz_v301_caption_ig_old:
            return _brz_v301_caption_ig_old(p)
        return _caption_wa_externo(p)
    except Exception:
        return _brz_v301_caption_ig_old(p) if _brz_v301_caption_ig_old else "Oferta Brizzah"

try:
    _brz_v301_extract_old = extrair_dados_link_externo
except Exception:
    _brz_v301_extract_old = None

def extrair_dados_link_externo(url, source=""):
    dados = {}
    if _brz_v301_extract_old:
        dados = _brz_v301_extract_old(url, source) or {}
    else:
        dados = {"name": "Oferta Brizzah", "price": 0, "old_price": 0, "image_url": "", "product_url": url}
    dados["coupon"] = _brz_v301_coupon_valido(dados.get("coupon"))
    return dados

@app.route("/limpar-cupons-fake", methods=["GET", "POST"])
@login_required
def brizzah_limpar_cupons_fake():
    alterados = 0
    exemplos = []
    if request.method == "POST":
        try:
            with get_db() as c:
                rows = c.execute("SELECT id, coupon, name FROM external_products WHERE coupon IS NOT NULL AND coupon <> ''").fetchall()
                for r in rows:
                    atual = str(r["coupon"] or "")
                    limpo = _brz_v301_coupon_valido(atual)
                    if limpo != atual:
                        c.execute("UPDATE external_products SET coupon=? WHERE id=?", (limpo, r["id"]))
                        alterados += 1
                        exemplos.append(f"ID {r['id']}: {atual} → {limpo or 'removido'}")
            try:
                log("INFO", f"[V30.1] Cupons fake limpos no banco: {alterados}")
            except Exception:
                pass
        except Exception as e:
            exemplos.append(f"Erro: {str(e)[:140]}")

    html = """
    <!doctype html><html lang="pt-br"><head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Limpar Cupons Fake</title>
    <style>
      body{font-family:Arial;background:#07110b;color:#f4fff6;margin:0}.wrap{max-width:850px;margin:auto;padding:22px}
      .card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:18px;margin:14px 0}
      a{color:#b6ff4d}button{background:#b6ff4d;color:#06100a;border:0;border-radius:12px;padding:13px 18px;font-weight:900}
      li{margin:8px 0}.brand{font-size:28px;font-weight:900;color:#b6ff4d}
    </style></head><body><div class="wrap">
      <div class="brand">🧹 Limpar Cupons Fake</div>
      <p><a href="/dashboard-pro">Dashboard Pro</a> · <a href="/ofertas-pro">Banco Pro</a></p>
      <div class="card">
        <p>Remove cupons falsos como <b>SPAN</b>, <b>APP</b>, <b>OFF</b>, <b>PROMO</b> quando foram capturados por erro de HTML.</p>
        <form method="post"><button type="submit">Limpar agora</button></form>
      </div>
      {% if alterados %}
      <div class="card"><h2>{{alterados}} cupom(ns) ajustado(s)</h2><ul>{% for e in exemplos[:80] %}<li>{{e}}</li>{% endfor %}</ul></div>
      {% endif %}
      {% if exemplos and not alterados %}
      <div class="card"><ul>{% for e in exemplos[:80] %}<li>{{e}}</li>{% endfor %}</ul></div>
      {% endif %}
    </div></body></html>
    """
    return render_template_string(html, alterados=alterados, exemplos=exemplos)

try:
    log("INFO", "[V30.1] Ativo: anti-cupom fake + rota /limpar-cupons-fake")
except Exception:
    pass
...
# ============================================================
# BRIZZAH V30.2 — HOTFIX DEFINITIVO CATEGORIA/HEADLINE
# Nome real do produto manda mais que category antiga do banco.
# Beleza/maquiagem nunca vira pet.
# Rota: /corrigir-categorias-headlines
# ============================================================

def _brz_v302_norm(txt):
...
def _brz_v302_cat_nome(nome):
    n = _brz_v302_norm(nome)

    if any(k in n for k in ["mascara de cilios","cilios","rimel","maybelline","colossal","maquiagem","batom","base liquida","corretivo","delineador","skincare","perfume","hidratante","shampoo","condicionador","beleza","cosmetico"]):
        return "beleza"
    if any(k in n for k in ["licor","amarula","vinho","whisky","vodka","gin","cerveja","espumante"]):
        return "bebida"
    if any(k in n for k in ["balao","aniversario","festa","cortina","decoracao","caixa organizadora","organizador","guarda roupa","gaveta","buffet","cozinha","mesa","cadeira","tapete","casa","jardim"]):
        return "casa_decoracao"
    if any(k in n for k in ["notebook","teclado","mouse","fone","bluetooth","smartwatch","camera","carregador","usb","tablet","pc","ssd","monitor","roteador","wifi","xiaomi","jbl","samsung"]):
        return "tech"
    if any(k in n for k in ["tenis","chinelo","sandalia","sapato","bota","calcado"]):
        return "calcados"
    if any(k in n for k in ["camiseta","camisa","bermuda","short","calca","cueca","meia","moletom","vestido","blusa","legging","jaqueta","moda"]):
        return "moda"
    if any(k in n for k in ["whey","creatina","fitness","academia","treino","suplemento"]):
        return "fitness"
    if any(k in n for k in ["brinquedo","infantil","crianca","bebe","escolar"]):
        return "infantil"
    if any(k in n for k in ["racao","cachorro","gato","pet shop","coleira pet","coleira cachorro","cama pet","caminha pet","arranhador","comedouro pet","bebedouro pet","areia gato","brinquedo cachorro","brinquedo gato"]):
        return "pet"
    return ""

def _brz_v30_detect_category(p):
    p = dict(p or {})
    nome_cat = _brz_v302_cat_nome(p.get("name") or "")
    if nome_cat:
        return nome_cat

    cat = _brz_v302_norm(p.get("category") or "")
    mapa = {
        "beleza": "beleza", "cosmeticos": "beleza", "maquiagem": "beleza",
        "casa": "casa_decoracao", "decoracao": "casa_decoracao", "casa decoracao": "casa_decoracao", "casa_decoracao": "casa_decoracao",
        "bebida": "bebida", "bebidas": "bebida",
        "tech": "tech", "tecnologia": "tech", "eletronicos": "tech",
        "moda": "moda", "roupas": "moda", "vestuario": "moda",
        "calcados": "calcados", "tenis": "calcados",
        "fitness": "fitness", "infantil": "infantil", "pet": "pet", "pets": "pet",
    }
    return mapa.get(cat, "geral")

def _brz_v30_headline(p):
    cat = _brz_v30_detect_category(p)
    nome = _brz_v302_norm((p or {}).get("name") or "")

    if cat == "beleza":
        if any(k in nome for k in ["mascara de cilios","cilios","rimel","maybelline"]):
            return "💄 Achado de beleza para realçar o olhar"
        return "💄 Achado de beleza com preço interessante"
    if cat == "pet":
        return "🐶 Oferta boa para quem cuida do pet"
    if cat == "casa_decoracao":
        if any(k in nome for k in ["balao","aniversario","festa"]):
...
@app.route("/corrigir-categorias-headlines", methods=["GET", "POST"])
@login_required
def brizzah_corrigir_categorias_headlines():
    alterados = 0
    exemplos = []
    if request.method == "POST":
        try:
            with get_db() as c:
                rows = c.execute("SELECT id,name,category FROM external_products ORDER BY id DESC LIMIT 2000").fetchall()
                for r in rows:
                    nova = _brz_v302_cat_nome(r["name"] or "")
                    atual = r["category"] or ""
                    if nova and nova != _brz_v302_norm(atual):
                        c.execute("UPDATE external_products SET category=? WHERE id=?", (nova, r["id"]))
                        alterados += 1
                        if len(exemplos) < 100:
                            exemplos.append(f"ID {r['id']}: {atual or 'vazio'} → {nova} | {(r['name'] or '')[:70]}")
            log("INFO", f"[V30.2] Categorias corrigidas pelo nome real: {alterados}")
        except Exception as e:
            exemplos.append(f"Erro: {str(e)[:140]}")

    html = """
    <!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Corrigir Categorias</title><style>
    body{font-family:Arial;background:#07110b;color:#f4fff6;margin:0}.wrap{max-width:900px;margin:auto;padding:22px}
    .card{background:#101b14;border:1px solid #29402f;border-radius:18px;padding:18px;margin:14px 0}
    a{color:#b6ff4d}button{background:#b6ff4d;color:#06100a;border:0;border-radius:12px;padding:13px 18px;font-weight:900}
    li{margin:8px 0}.brand{font-size:28px;font-weight:900;color:#b6ff4d}</style></head>
    <body><div class="wrap"><div class="brand">🧠 Corrigir Categorias/Headlines</div>
    <p><a href="/dashboard-pro">Dashboard Pro</a> · <a href="/ofertas-pro">Banco Pro</a></p>
    <div class="card"><p>Corrige categoria pelo <b>nome real do produto</b>. Exemplo: máscara de cílios vira beleza, mesmo se o banco marcou pet.</p>
    <form method="post"><button type="submit">Corrigir agora</button></form></div>
    {% if alterados %}<div class="card"><h2>{{alterados}} produto(s) ajustado(s)</h2><ul>{% for e in exemplos %}<li>{{e}}</li>{% endfor %}</ul></div>{% endif %}
    {% if exemplos and not alterados %}<div class="card"><ul>{% for e in exemplos %}<li>{{e}}</li>{% endfor %}</ul></div>{% endif %}
    </div></body></html>
    """
    return render_template_string(html, alterados=alterados, exemplos=exemplos)

try:
    log("INFO", "[V30.2] Ativo: categoria pelo nome real + anti-headline pet indevida")
except Exception:
    pass
