...
# ===== BRIZZAH V9 FINAL — PORTUGUÊS DEFINITIVO + SHOPEE NA ROTAÇÃO + CTA WHATSAPP =====
LINK_GRUPO_BRIZZAH = "https://chat.whatsapp.com/Fb6kF0NXlwi8CzIVoPHMDr?mode=gi_t"

def _brz_norm_v9(txt):
    try:
        import unicodedata, re
        s = str(txt or "").lower()
        s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
...
def _corrigir_portugues_produto(texto):
    try:
        ...
        regras = [
            ...
            (r"\bversao\b", "versão"), (r"\bconfortavel\b", "confortável"),
            (r"\balca\b", "alça"), (r"\bpeca\b", "peça"), (r"\bpecas\b", "peças"),
            (r"\brelogio\b", "relógio"), (r"\boculos\b", "óculos"),
            (r"\beletronico\b", "eletrônico"), (r"\beletronicos\b", "eletrônicos"),
            (r"\bdecoracao\b", "decoração"), (r"\borganizacao\b", "organização"),
            (r"\butilidades domesticas\b", "utilidades domésticas"),
            (r"\bpre treino\b", "pré-treino"), (r"\bpos treino\b", "pós-treino"),
            (r"\bcaes\b", "cães"), (r"\bcao\b", "cão"), (r"\baco\b", "aço"),
            (r"\badnet\b", "Adnet"), (r"\bolympikus\b", "Olympikus"),
            (r"\badidas\b", "Adidas"), (r"\bpuma\b", "Puma"), (r"\bnike\b", "Nike"),
            (r"\bfila\b", "Fila"), (r"\bmizuno\b", "Mizuno"),
            (r"\bjbl\b", "JBL"), (r"\bxiaomi\b", "Xiaomi"),
            (r"\bmercado livre\b", "Mercado Livre"),
            (r"\bshopee\b", "Shopee"), (r"\bnetshoes\b", "Netshoes"),
            (r"\bamazon\b", "Amazon"), (r"\bessentials\b", "Essentials"),
            (r"\b3\s*stripes\b", "3-Stripes"), (r"\boriginal\s+nf\b", "original"),
        ]
        for pat, repl in regras:
...
def _brz_cta_grupo_whatsapp_v9():
    import random
    opcoes = [
        f"🚨 Quer receber ofertas assim todos os dias?\n👉 Entre no grupo VIP:\n{LINK_GRUPO_BRIZZAH}\n\n🔥 Manda pra aquele amigo que ama economizar!",
        f"💰 Só entra quem gosta de pagar barato!\n👉 Entre no grupo agora:\n{LINK_GRUPO_BRIZZAH}",
...
def _brz_categoria_v9(p):
    nome = _brz_norm_v9((p.get("name") or "") + " " + (p.get("category") or "") + " " + (p.get("source") or ""))
    if any(x in nome for x in ["infantil", "crianca", "menino", "menina", "bebe", "kids"]):
        return "infantil"
    if any(x in nome for x in ["feminina", "feminino", "mulher", "vestido", "saia", "bolsa feminina", "legging"]):
        return "feminino"
    if any(x in nome for x in ["masculina", "masculino", "homem", "polo", "bermuda masculina"]):
        return "masculino"
    if any(x in nome for x in ["tenis", "sneaker", "calcado", "sapato", "sandalia", "chinelo", "bota"]):
        return "tenis"
    if any(x in nome for x in ["pet", "cachorro", "cao", "caes", "gato", "gatos", "caminha", "coleira", "racao"]):
        return "pet"
    if any(x in nome for x in ["casa", "cozinha", "toalha", "espelho", "decoracao", "organizador", "banheiro", "sala"]):
        return "casa"
    if any(x in nome for x in ["fone", "jbl", "xiaomi", "bluetooth", "caixa de som", "smartwatch", "carregador"]):
        return "tech"
    return "geral"

try:
    _brz_buscar_external_original_v9 = buscar_external_products_aprovados
except Exception:
    _brz_buscar_external_original_v9 = None

try:
    _brz_buscar_multiplas_original_v9 = buscar_multiplas_plataformas
except Exception:
    _brz_buscar_multiplas_original_v9 = None

def _brz_turno_fonte_v9():
    try:
        atual = int(cfg("_brz_v9_turno", "0") or 0)
    except Exception:
        atual = 0
    sequencia = ["externo", "shopee", "externo", "shopee"]
    fonte = sequencia[atual % len(sequencia)]
    try:
        cfg_set("_brz_v9_turno", str(atual + 1))
    except Exception:
        pass
    return fonte
...
def buscar_external_products_aprovados(limit=30):
    import random
    turno = _brz_turno_fonte_v9()
    if turno == "shopee":
        try:
            cfg_set("_brz_force_shopee_v9", "1")
            log("INFO", "[V9] Turno Shopee: pulando links manuais para forçar Shopee na rotação.")
        except Exception:
            pass
        return []
    try:
        if _brz_buscar_external_original_v9:
            produtos = _brz_buscar_external_original_v9(limit=200)
        else:
            with get_db() as c:
                rows = c.execute("""
                    SELECT * FROM external_products
        ...
        try:
            log("WARN", f"[V9] Falha ao buscar externos: {str(e)[:100]}")
        except Exception:
            pass
        produtos = []
    if not produtos:
        return []
    try:
        hoje = _agora_brasil().date().isoformat()
    except Exception:
        hoje = datetime.now().date().isoformat()
    nao_postados_hoje = [p for p in produtos if not str(p.get("last_posted") or "").startswith(hoje)]
    base = nao_postados_hoje if nao_postados_hoje else produtos
    grupos = {}
    for p in base:
        grupos.setdefault(_brz_categoria_v9(p), []).append(p)
    for cat in grupos:
        random.shuffle(grupos[cat])
    ordem = ["masculino", "feminino", "infantil", "tenis", "casa", "pet", "tech", "geral"]
    random.shuffle(ordem)
    resultado = []
    while any(grupos.values()) and len(resultado) < limit:
        for cat in ordem:
            if grupos.get(cat):
                resultado.append(grupos[cat].pop(0))
                if len(resultado) >= limit:
                    break
    try:
        log("INFO", f"[V9] Turno links manuais: {len(resultado)} produto(s) variados selecionados.")
    except Exception:
        pass
    return resultado[:limit]

def buscar_multiplas_plataformas(keyword, limit_cada=6):
    if cfg("_brz_force_shopee_v9", "") == "1":
        try:
            cfg_set("_brz_force_shopee_v9", "")
        except Exception:
            pass
        produtos = []
        try:
            pool = buscar_pool_produtos_top(nicho_base=keyword)
            if pool:
                produtos.extend(pool[:max(limit_cada, 10)])
        except Exception as e:
            try:
                log("WARN", f"[V9] Pool Shopee falhou: {str(e)[:100]}")
            except Exception:
                pass
        if not produtos:
            try:
                produtos = shopee_search(keyword, limit=max(limit_cada, 10))
            except Exception as e:
                try:
                    log("WARN", f"[V9] Shopee search falhou: {str(e)[:100]}")
                except Exception:
                    pass
                produtos = []
        for p in produtos:
            try:
                ...
                pass
        try:
            log("INFO", f"[V9] Shopee priorizada: {len(produtos)} produto(s).")
        except Exception:
            pass
        return produtos
    if _brz_buscar_multiplas_original_v9:
        return _brz_buscar_multiplas_original_v9(keyword, limit_cada=limit_cada)
    return []
...
def _caption_wa_externo(p):
    ...
    if cupom:
        linhas.append(f"Cupom: *{cupom}* ⚠️")
    linhas += ["", f"{emoji} Origem: *{fonte}*", f"🔗 {link}", "", _brz_cta_grupo_whatsapp_v9(), "", "_Brizzah | Achados inteligentes_ 🔥"]
    return "\n".join(linhas)

def _gerar_caption_wa(produto):
    nome = _nome_profissional_produto(produto.get("name", ""))[:115]
    preco = _safe_float(produto.get("price", 0), 0)
    orig = produto.get("original_price", 0) or produto.get("old_price", 0)
    try:
        orig = _brz_preco_antigo_confiavel(preco, orig, produto.get("source"))
    except Exception:
    ...
    stars = float(produto.get("rating", 0) or 0)
    try:
        emoji, fonte = _fonte_emoji(produto)
    except Exception:
        emoji, fonte = "🟠", "Shopee"
    linhas = [f"{emoji} *{_headline_promo_externo({'name': nome, 'source': produto.get('source', 'shopee')})}*", "", nome, ""]
    if desc >= 10 and orig:
        linhas.append(f"de {_format_brl(orig)} por *{_format_brl(preco)}* 🔥 *{desc}% OFF*")
    elif preco:
        linhas.append(f"💰 *{_format_brl(preco)}*")
    else:
        linhas.append("💰 Confira o preço atualizado no link 👇")
    if stars > 0:
        linhas.append(f"⭐ {stars:.1f}/5")
    if sold > 0:
        linhas.append(f"🛒 {sold:,}+ vendidos".replace(",", "."))
    try:
        linhas.append(f"💬 _{adicionar_opiniao(produto)}_")
    except Exception:
        pass
    linhas.append("🔥 Oferta com desconto no anúncio")
    linhas.append("⚠️ Preço pode mudar a qualquer momento")
    ...
    linhas.append("")
    linhas.append(_brz_cta_grupo_whatsapp_v9())
    linhas.append("")
    linhas.append("_Brizzah | Achados inteligentes_ 🔥")
    return "\n".join(linhas)

try:
    log("INFO", "[V9] Patch final carregado: português definitivo + Shopee na rotação + CTA WhatsApp.")
except Exception:
    pass
# ===== FIM BRIZZAH V9 FINAL =====
