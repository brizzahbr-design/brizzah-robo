...
# ============================================================
# BRIZZAH V16 — RODÍZIO INTELIGENTE DE PERFIS + ANTI-REPETIÇÃO FORTE
# Objetivo: automático não repetir anúncio e alternar perfis/categorias
# Ex.: moda feminina → moda masculina → tênis → perfume → fitness → tech...
# ============================================================
BRIZZAH_V16_REPOST_HORAS = int(os.environ.get("BRIZZAH_V16_REPOST_HORAS", "168") or "168")  # 7 dias
BRIZZAH_V16_LOOKBACK_DIAS = int(os.environ.get("BRIZZAH_V16_LOOKBACK_DIAS", "21") or "21")
BRIZZAH_V16_SEQ = [
    "moda_feminina", "moda_masculina", "calcados", "perfume",
    "fitness", "tech", "casa", "acessorios", "calcados", "moda_feminina"
]


def _brz_v16_norm(txt):
    try:
...
def _brz_v16_link_key(url):
    """Chave anti-duplicidade: remove rastreadores e preserva o caminho principal."""
    try:
        import urllib.parse as _up
        ...
        qs = _up.parse_qs(p.query or '')
        # Em links Linksynergy/afiliados, murl é o produto real.
        for k in ('murl', 'url', 'u', 'target'):
            if qs.get(k, [''])[0].startswith('http'):
                return _brz_v16_link_key(_up.unquote(qs.get(k, [''])[0]))
        # Mercado Livre costuma ter MLB no caminho; captura se existir.
        m = re.search(r'(MLB\d+)', url, flags=re.I)
        if m:
            return 'mlb:' + m.group(1).lower()
        return (host + path).lower()
    except Exception:
        return str(url or '').strip().split('?', 1)[0].lower()


def _brz_v16_name_key(nome):
    n = _brz_v16_norm(nome)
    # Remove termos genéricos para comparar melhor o produto real.
    stop = {'tenis','tênis','camiseta','camisa','masculino','masculina','feminino','feminina','adulto','original','preto','branco','azul','verde','vermelho','casual'}
    toks = [t for t in n.split() if len(t) >= 3 and t not in stop]
    return ' '.join(toks[:10]) or n[:80]


def _brz_v16_perfil(p):
    n = _brz_v16_norm(' '.join([
        str((p or {}).get('name') or ''),
        str((p or {}).get('category') or ''),
        str((p or {}).get('brand') or '')
    ]))
    if any(x in n for x in ['perfume','colonia','fragrancia','lattafa','egeo','granado','boticario','parfum']):
        return 'perfume'
    if any(x in n for x in ['tenis','sneaker','calcado','sapato','sandalia','chinelo','bota','running','fresh foam','court']):
        return 'calcados'
    if any(x in n for x in ['creatina','whey','suplemento','protein','academia','treino','dryfit','legging','regata fitness','camisa termica']):
        return 'fitness'
    if any(x in n for x in ['fone','bluetooth','jbl','xiaomi','smartwatch','relogio','g shock','casio','caixa de som','boombox','carregador','power bank']):
        return 'tech'
    if any(x in n for x in ['airfryer','panela','cozinha','toalha','cama mesa','organizador','espelho','tapete','casa','decoracao']):
        return 'casa'
    if any(x in n for x in ['mochila','bolsa','relogio','óculos','oculos','colar','corrente','pulseira','brinco','anel','cinto']):
        return 'acessorios'
    if any(x in n for x in ['cueca','boxer','bermuda masculina','short masculino','camiseta masculina','camisa masculina','polo masculina','masculino']):
        return 'moda_masculina'
    if any(x in n for x in ['vestido','cropped','blusa feminina','short feminino','camiseta feminina','legging feminina','saia','feminino','feminina','mulher']):
        return 'moda_feminina'
    if any(x in n for x in ['camiseta','camisa','short','bermuda','calca','moletom','polo']):
        return 'moda_masculina'
    return 'geral'


def _brz_v16_perfil_alvo():
    try:
        idx = int(cfg('_brz_v16_perfil_idx', '0') or 0)
    except Exception:
        idx = 0
    return BRIZZAH_V16_SEQ[idx % len(BRIZZAH_V16_SEQ)]


def _brz_v16_avancar_perfil(perfil_postado=''):
    try:
        idx = int(cfg('_brz_v16_perfil_idx', '0') or 0)
        cfg_set('_brz_v16_perfil_idx', str(idx + 1))
        if perfil_postado:
            cfg_set('_brz_v16_last_perfil', perfil_postado)
            hist = [x for x in str(cfg('_brz_v16_hist_perfis', '') or '').split(',') if x]
            hist.append(perfil_postado)
            cfg_set('_brz_v16_hist_perfis', ','.join(hist[-6:]))
    except Exception:
        pass


def _brz_v16_recent_products():
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT name, product_url, affiliate_url, posted_at
                FROM products
                WHERE posted_at >= datetime('now', ?)
                ORDER BY id DESC
                LIMIT 500
            """, (f'-{BRIZZAH_V16_LOOKBACK_DIAS} days',)).fetchall()
        return [dict(r) for r in rows]
    except Exception:
        return []


def _brz_v16_ja_postado(p):
    """Bloqueia mesmo anúncio, mesmo link, mesmo nome ou muito parecido."""
    try:
        nome = p.get('name') or ''
        nk = _brz_v16_name_key(nome)
        lk1 = _brz_v16_link_key(p.get('affiliate_url') or '')
        lk2 = _brz_v16_link_key(p.get('product_url') or '')

        # last_posted da própria fila externa.
        lp = str(p.get('last_posted') or '').strip()
        if lp:
            try:
                from datetime import timedelta
                dt = datetime.fromisoformat(lp.replace('Z','').split('.')[0])
                if datetime.now() - dt < timedelta(hours=BRIZZAH_V16_REPOST_HORAS):
                    return True
            except Exception:
                pass

        for r in _brz_v16_recent_products():
            rnk = _brz_v16_name_key(r.get('name') or '')
            rlk1 = _brz_v16_link_key(r.get('affiliate_url') or '')
            rlk2 = _brz_v16_link_key(r.get('product_url') or '')
            if lk1 and (lk1 == rlk1 or lk1 == rlk2):
                return True
            if lk2 and (lk2 == rlk1 or lk2 == rlk2):
                return True
            # Nome igual/quase igual bloqueia duplicidade mesmo com outro link/ID.
            if nk and rnk and (nk == rnk or nk in rnk or rnk in nk):
                if len(nk) >= 18 or len(rnk) >= 18:
                    return True
    except Exception:
        return False
    return False
...
def _brz_v16_score(p):
    base = 0
    try:
        if 'score_externo' in globals() and callable(score_externo):
            # Evita recursão se esta função virar score_externo.
            orig = globals().get('_brz_v16_score_externo_original')
            if orig:
                base = int(orig(p) or 0)
    except Exception:
        base = 0
    n = _brz_v16_norm(p.get('name') or '')
    fonte = _brz_v10_source(p.get('source')) if '_brz_v10_source' in globals() else str(p.get('source') or '').lower()
    perfil = _brz_v16_perfil(p)
    alvo = _brz_v16_perfil_alvo()
    hist = [x for x in str(cfg('_brz_v16_hist_perfis', '') or '').split(',') if x]

    s = base
    # Fonte: banco premium continua forte.
    if fonte == 'netshoes': s += 80
    elif fonte == 'mercadolivre': s += 70
    elif fonte == 'amazon': s += 55
    elif fonte == 'nike': s += 65
    elif fonte == 'shopee': s -= 200

    # Rodízio de perfil: escolhe algo diferente do que acabou de sair.
    if perfil == alvo: s += 180
    if hist and perfil == hist[-1]: s -= 220
    if len(hist) >= 2 and perfil == hist[-2]: s -= 120

    marcas = ['nike','adidas','puma','reebok','fila','olympikus','mizuno','new balance','asics','casio','g shock','jbl','philips','lattafa','granado','boticario']
    if any(m in n for m in marcas): s += 45

    # Penaliza produto genérico quando há opção de perfil claro.
    if perfil == 'geral': s -= 60
    return s


try:
    _brz_v16_score_externo_original = score_externo
except Exception:
    _brz_v16_score_externo_original = None


def score_externo(p):
    return _brz_v16_score(p)


try:
    _brz_v16_buscar_external_original = buscar_external_products_aprovados
except Exception:
    _brz_v16_buscar_external_original = None


def buscar_external_products_aprovados(limit=30):
    """V16: escolhe produto diferente, com rodízio de perfil e anti-repetição forte."""
    try:
        # Busca direto no banco para não herdar ordenação antiga que repetia o campeão.
        with get_db() as c:
            rows = c.execute("""
                SELECT * FROM external_products
                WHERE (status='approved' OR status='active' OR status IS NULL OR status='')
        ...
        try: log('WARN', f'[V16] Falha ao ler banco externo: {str(e)[:90]}')
        except Exception: pass
        produtos = []
    ...
    for p in produtos:
        try:
            p['source'] = _brz_v10_source(p.get('source')) if '_brz_v10_source' in globals() else str(p.get('source') or '').lower()
            # Automático V12: Shopee só manual.
            if '_brz_v12_is_auto_banco_only' in globals() and _brz_v12_is_auto_banco_only() and p['source'] == 'shopee':
                continue
            if '_brz_v10_produto_ruim' in globals() and _brz_v10_produto_ruim(p):
                continue
            if _brz_v16_ja_postado(p):
                continue
            validos.append(p)
        except Exception:
            continue
    ...
    alvo = _brz_v16_perfil_alvo()
    # 1ª tentativa: perfil da vez.
    pool_alvo = [p for p in validos if _brz_v16_perfil(p) == alvo]
    pool = pool_alvo or validos
    pool.sort(key=_brz_v16_score, reverse=True)
    escolhidos = pool[:max(1, int(limit or 1))]
    try:
        resumo = {}
        for p in escolhidos:
            resumo[_brz_v16_perfil(p)] = resumo.get(_brz_v16_perfil(p), 0) + 1
        log('INFO', f'[V16] Rodízio inteligente | alvo={alvo} | total={len(produtos)} | validos={len(validos)} | escolhidos={len(escolhidos)} | perfis={resumo}')
    except Exception:
        pass
    return escolhidos
...
try:
    _brz_v16_executar_ciclo_original = executar_ciclo
except Exception:
    _brz_v16_executar_ciclo_original = None


def executar_ciclo():
    """V16: após postar, avança o perfil do rodízio para variar o próximo produto."""
    antes_id = None
    try:
        with get_db() as c:
            r = c.execute('SELECT id FROM products ORDER BY id DESC LIMIT 1').fetchone()
            antes_id = r['id'] if r else None
    except Exception:
        pass

    resultado = _brz_v16_executar_ciclo_original() if _brz_v16_executar_ciclo_original else 0

    if resultado and int(resultado or 0) > 0:
        try:
            with get_db() as c:
                if antes_id:
                    r = c.execute('SELECT name, shop_name FROM products WHERE id>? ORDER BY id DESC LIMIT 1', (antes_id,)).fetchone()
                else:
                    r = c.execute('SELECT name, shop_name FROM products ORDER BY id DESC LIMIT 1').fetchone()
            if r:
                perfil = _brz_v16_perfil({'name': r['name'] or '', 'source': r['shop_name'] or ''})
                _brz_v16_avancar_perfil(perfil)
                try: log('INFO', f'[V16] Perfil postado={perfil} | próximo alvo={_brz_v16_perfil_alvo()}')
                except Exception: pass
            else:
                _brz_v16_avancar_perfil('')
        except Exception:
            _brz_v16_avancar_perfil('')
    return resultado

try:
    log('INFO', '[V16] Ativo: rodízio inteligente de perfil + anti-repetição forte por nome/link/histórico')
except Exception:
    pass
# ============================================================
# FIM BRIZZAH V16
