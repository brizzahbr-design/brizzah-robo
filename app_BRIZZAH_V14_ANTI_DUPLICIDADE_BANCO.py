...
# ============================================================
# BRIZZAH V14 — ANTI-DUPLICIDADE FORTE NETSHOES/ML
# Correção: evita postar o mesmo produto em sequência mesmo quando
# ele aparece duplicado no banco com IDs/links afiliados diferentes.
# ============================================================
import urllib.parse as _brz_v14_urlparse

BRIZZAH_V14_REPOST_HORAS = int(os.environ.get("BRIZZAH_REPOST_HORAS", "96") or "96")


def _brz_v14_norm(txt):
    try:
...
def _brz_v14_link_base(url):
    """Gera uma chave estável do produto, ignorando rastreios de afiliado."""
    try:
        url = (url or "").strip()
        if not url:
            return ""
        p = _brz_v14_urlparse.urlparse(url)
        host = (p.netloc or "").lower().replace("www.", "")
        path = _brz_v14_urlparse.unquote(p.path or "").lower().strip("/")
        # Remove partes de rastreio comuns, mantendo o caminho do produto.
        path = re.sub(r"/(?:ref|utm|tag|campaign|click|share)(?:/.*)?$", "", path)
        path = re.sub(r"[^a-z0-9/_-]", "", path)
        return f"{host}/{path}"[:220]
    except Exception:
        return _brz_v14_norm(url)[:220]


def _brz_v14_nome_base(nome):
    n = _brz_v14_norm(nome)
    # Remove ruídos e tamanhos/cores que podem variar no mesmo item.
    n = re.sub(r"\b(tamanho|tam|cor|preto|branco|azul|verde|vermelho|rosa|cinza|p|m|g|gg|xg|xxg)\b", " ", n)
    n = re.sub(r"\b[0-9]{1,3}\b", " ", n)
    return re.sub(r"\s+", " ", n).strip()[:90]


def _brz_v14_fingerprint(p):
    p = p or {}
    link = p.get("affiliate_url") or p.get("product_url") or p.get("link") or ""
    link_key = _brz_v14_link_base(link)
    nome_key = _brz_v14_nome_base(p.get("name") or "")
    fonte = _brz_v10_source(p.get("source")) if "_brz_v10_source" in globals() else str(p.get("source") or "")
    base = f"{fonte}|{link_key}|{nome_key}"
    return _brz_hashlib.sha1(base.encode("utf-8", errors="ignore")).hexdigest()


# Sobrepõe hash V10 para usar fingerprint mais estável.
def _brz_v10_hash_produto(p):
    return _brz_v14_fingerprint(p)


def _brz_v14_recent_products_fingerprints(horas=None):
    horas = int(horas or BRIZZAH_V14_REPOST_HORAS)
    fps = set()
    try:
        with get_db() as c:
            rows = c.execute("""
                SELECT name, product_url, affiliate_url, shop_name, posted_at
                FROM products
        ...
        for r in rows:
            d = dict(r)
            d["source"] = d.get("shop_name") or ""
            fps.add(_brz_v14_fingerprint(d))
    except Exception:
        pass
    return fps
...
def _brz_v10_ja_postado_hash(p, horas=None):
    """V14: bloqueia por fingerprint salvo e também pelo histórico da tabela products."""
    horas = int(horas or BRIZZAH_V14_REPOST_HORAS)
    fp = _brz_v14_fingerprint(p)
    try:
        with get_db() as c:
            r = c.execute(
                "SELECT created_at FROM posted_fingerprints WHERE fingerprint=? AND created_at >= datetime('now', ?)",
                (fp, f"-{horas} hours")
            ).fetchone()
            if r:
                return True
        ...
        pass
    return fp in _brz_v14_recent_products_fingerprints(horas)


def _brz_v10_marcar_postado_hash(p):
    """V14: registra fingerprint e marca duplicados do banco como recém-postados."""
    fp = _brz_v14_fingerprint(p)
    try:
        with get_db() as c:
            c.execute("""
                CREATE TABLE IF NOT EXISTS posted_fingerprints (
            ...
            """)
            c.execute("""
                INSERT OR REPLACE INTO posted_fingerprints(fingerprint,source,category,product_name,created_at)
                VALUES(?,?,?,?,datetime('now','localtime'))
            """, (fp, _brz_v10_source(p.get("source")), _brz_v10_categoria(p), (p.get("name") or "")[:140]))
    except Exception:
        pass


def _brz_v14_dedupe_lista(produtos, horas=None):
    horas = int(horas or BRIZZAH_V14_REPOST_HORAS)
    vistos = set()
    recentes = _brz_v14_recent_products_fingerprints(horas)
    saida = []
    for p in produtos or []:
        try:
            fp = _brz_v14_fingerprint(p)
            ...
            if fp in recentes:
                continue
            if _brz_v10_ja_postado_hash(p, horas=horas):
                continue
            vistos.add(fp)
            saida.append(p)
        except Exception:
...
try:
    _brz_v14_buscar_external_products_aprovados_original = buscar_external_products_aprovados
except Exception:
    _brz_v14_buscar_external_products_aprovados_original = None


def buscar_external_products_aprovados(limit=30):
    """V14: banco de links com anti-duplicidade por produto, não só por ID."""
    produtos = []
    if _brz_v14_buscar_external_products_aprovados_original:
        produtos = _brz_v14_buscar_external_products_aprovados_original(limit=max(int(limit or 30), 120))
    produtos = _brz_v14_dedupe_lista(produtos, horas=BRIZZAH_V14_REPOST_HORAS)
    try:
        produtos.sort(key=_brz_v10_score, reverse=True)
    except Exception:
        pass
    try:
        log("INFO", f"[V14] Anti-duplicidade banco aplicado | liberados={len(produtos)}")
    except Exception:
        pass
    return produtos[:max(1, int(limit or 1))]


# Marca todo produto externo como postado também por fingerprint, além do last_posted do ID.
try:
    _brz_v14_whatsapp_post_original = whatsapp_post
except Exception:
    _brz_v14_whatsapp_post_original = None


def whatsapp_post(img_url, caption, instance_id, token, group_id):
    ok, result = (False, "whatsapp_post original ausente")
    if _brz_v14_whatsapp_post_original:
        ok, result = _brz_v14_whatsapp_post_original(img_url, caption, instance_id, token, group_id)
    # O registro principal já é feito na tabela products pelo ciclo. Esta função fica neutra.
    return ok, result


try:
    log("INFO", "[V14] Ativo: anti-duplicidade forte por link/nome/histórico recente")
except Exception:
    pass
